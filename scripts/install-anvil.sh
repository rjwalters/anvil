...
#   .agents/skills/anvil-<name>/SKILL.md  Thin Codex registration shim (issue
#                                         #1003). Same naming and thin-pointer
#                                         content pattern as the Claude shim
#                                         above -- points back at the canonical
#                                         .anvil/skills/<name>/SKILL.md body,
#                                         never duplicates it. Codex CLI scans
#                                         .agents/skills at the repo root
#                                         (verified contract: see
#                                         docs/codex-skill-adapter.md); no
#                                         wrapping plugin manifest is required
#                                         for discovery.
...
# Write the thin Codex registration shim for a skill. Structured in parallel
# to write_shim() above -- same caller shape (happy-path Stage 7 install and
# the override-skip branch), same thin-pointer content pattern, same
# `anvil-$skill` naming. Issue #1003 (phase 2 of the #1000 epic); the target
# path and "no plugin manifest required" decision are the verified findings
# from #1002 (see docs/codex-skill-adapter.md section 6):
#
#   - Codex CLI scans `.agents/skills` from CWD up to the repo root; the
#     repo-root location ($TARGET/.agents/skills/anvil-<name>/) is the one an
#     installer writing a fixed path once should target.
#   - A bare `<scope>/.agents/skills/<name>/SKILL.md` is independently
#     discovered -- no `.codex-plugin/plugin.json` is required in v1.
#   - `.codex/skills/` is NOT part of the documented contract and must not be
#     used.
#
# All substitution happens inside the bash heredoc, so paths containing shell
# metacharacters are safe (no child-shell re-parse).
...
This file is a thin Codex registration shim generated by install-anvil.sh.
The canonical skill body and any consumer overrides live at .anvil/skills/$skill/.
SHIMEOF
}
...
for skill in "${SELECTED_SKILLS[@]}"; do
  ...
  # Codex counterpart (issue #1003): repo-root .agents/skills scan path
  # verified in docs/codex-skill-adapter.md (#1002). Same "anvil-<skill>"
  # naming as the Claude shim; independently discovered, no plugin manifest.
  ...
  # Always regenerate the thin Codex registration shim (issue #1003) —
  # tracks the Claude shim 1:1 for every install/upgrade/--skills= run.
