...
#   .agents/skills/anvil-<name>/SKILL.md  Thin Codex CLI registration shim
#                                         (issue #1003; format verified in
#                                         docs/codex-skill-adapter.md — Codex
#                                         scans repo-root .agents/skills/<name>/
#                                         for a SKILL.md; same minimal
#                                         name+description frontmatter contract
#                                         as Claude's shim, same
#                                         "canonical body stays under .anvil/"
#                                         thin-pointer content, same
#                                         anvil-<name> naming). Written and
#                                         pruned 1:1 alongside the Claude shim
#                                         above -- no wrapping plugin manifest.
...
# Write the thin Codex CLI registration shim for a skill (issue #1003).
# Structured in parallel to write_shim() above -- same two callers (the
# happy-path Stage 7 install and the override-skip branch), same
# unconditional-regenerate-every-run semantics (the shim is a generated
# pointer, not a consumer-override target, so it is never hash-tracked or
# skip-guarded the way a skill body is).
#
# Format per docs/codex-skill-adapter.md (research spike #1002, closed):
# Codex CLI scans repo-root `.agents/skills/<name>/SKILL.md` (NOT
# `.codex/skills/`), and its minimal SKILL.md frontmatter contract
# (`name` + `description`) is identical to Claude Code's. No wrapping
# `.codex-plugin/plugin.json` is required for discovery, so this mirrors
# write_shim()'s thin-pointer content verbatim at the Codex-side path --
# same anvil-$skill naming, same one-line pointer back at the canonical
# body under .anvil/skills/$skill/SKILL.md, never a copy of the body.
#
# All substitution happens inside the bash heredoc, so paths containing shell
# metacharacters are safe (no child-shell re-parse).
write_codex_shim() {
...
for skill in "${SELECTED_SKILLS[@]}"; do
  ...
  # Codex CLI counterpart (issue #1003): repo-root .agents/skills/<name>/
  # per docs/codex-skill-adapter.md -- same anvil-$skill naming, tracks the
  # Claude shim 1:1 at every call site below.
  codex_shim_dir="$TARGET/.agents/skills/anvil-$skill"
  codex_shim_file="$codex_shim_dir/SKILL.md"
  ...
  if [[ -d "$dst_skill" ]]; then
    if dirs_identical_body_only "$src_skill" "$dst_skill"; then
      ...
      if [[ -n "$recorded_hash" && "$recorded_hash" == "$current_dst_hash" ]]; then
        ...
        do_action "regenerate Codex registration shim at .agents/skills/anvil-$skill/SKILL.md" \
          write_codex_shim "$skill" "$codex_shim_dir" "$codex_shim_file"
        continue
      else
  ...
  # Always regenerate the thin Codex CLI registration shim (issue #1003) --
  # tracks the Claude shim 1:1 at this call site too.
  do_action "write Codex registration shim at .agents/skills/anvil-$skill/SKILL.md" \
    write_codex_shim "$skill" "$codex_shim_dir" "$codex_shim_file"
...
#
# For each removed name, all five installer-owned path families are removed
# (directory blow-away for the four dir families, glob removal for the agent
...
#   * .agents/skills/anvil-<name>/     (Codex registration shim, issue #1003)
  ...
  if [[ ${#REMOVED_SKILLS[@]} -gt 0 ]]; then
    if [[ "$DRY_RUN" == true ]]; then
      ...
      for removed in "${REMOVED_SKILLS[@]}"; do
        echo "  [dry-run]   remove .anvil/skills/$removed/, .anvil/anvil/skills/$removed/, .claude/skills/anvil-$removed/, .agents/skills/anvil-$removed/, .claude/agents/anvil-$removed-*.md"
      done
    else
      for removed in "${REMOVED_SKILLS[@]}"; do
        # Four directory families — blow away each if present. Each is
        ...
        rm -rf "$TARGET/.agents/skills/anvil-$removed"
      done
      ...
      note "pruned ${#REMOVED_SKILLS[@]} skill(s) removed from the registry since the last install: ${REMOVED_SKILLS[*]} (all five path families removed)"
    fi
  fi
fi
