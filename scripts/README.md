...
7. For each selected skill: byte-diff against source; skip if consumer-modified (unless `--force`);
   ...
   registration shim at `.claude/skills/anvil/<name>/SKILL.md`.
...
at `.claude/skills/anvil/<name>/SKILL.md`. This mirrors Loom's `.loom/` separation between
framework code and Claude registration, and preserves consumer-override semantics: edits to
`.anvil/skills/<name>/` are detected on re-install and preserved by default.
