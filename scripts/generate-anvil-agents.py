...
# Per-(skill, phase) extra responsibility paragraphs appended to the
# generated agent body (issue #472). The memo render step (memo-draft.md
# step 9.5 / memo-revise.md step 9.7) historically read as runtime
# narration to agent drivers — the studio canary produced four version
# dirs with no PDF because the subagent prompt never said the render was
# the agent's own post-write job. These paragraphs make the
# responsibility explicit in the prompt itself; the command files carry
# the full contract.
SKILL_PHASE_EXTRA_NOTES = {
    ("memo", "draft"): (
        "Render responsibility (issue #472): after you write the new version "
        "directory and record `phases.draft.state == done`, YOU must execute "
        "the render step (memo-draft.md step 9.5) yourself by running "
        "`python3 .anvil/skills/memo/lib/render_phase.py <version-dir>` — "
        "there is no other runtime that performs it; the CLI is non-blocking "
        "and always exits 0."
    ),
    ("memo", "revise"): (
        "Render responsibility (issue #472): after you write the revised "
        "version directory and `changelog.md`, YOU must execute the render "
        "step (memo-revise.md step 9.7) yourself by running "
        "`python3 .anvil/skills/memo/lib/render_phase.py <version-dir>` — "
        "there is no other runtime that performs it; the CLI is non-blocking "
        "and always exits 0."
    ),
...
def render_lifecycle_agent(skill: str, phase: str) -> str:
    ...
    extra_note = SKILL_PHASE_EXTRA_NOTES.get((skill, phase))
    extra_block = f"\n{extra_note}\n" if extra_note else ""
