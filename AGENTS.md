...
This guide describes the Loom orchestration workflows used to develop **Anvil itself** — the framework code, the per-skill commands, the shared libraries, and the install script. It does **not** describe how Anvil's *artifact-authoring agents* (drafter, reviewer, reviser, auditor, figurer, etc.) work; those are documented per-skill in `anvil/skills/<name>/SKILL.md`.

Anvil-the-framework is built using [Loom](https://github.com/rjwalters/loom), and the eight Loom agent archetypes below run on this repo's GitHub issues + PRs to ship features against a real-world canary consumer ([2AM Logic Studio](https://2amlogic.com)). Anvil-the-product orchestrates *artifact creation* using filesystem versioning as the coordination layer; Loom orchestrates Anvil's *own development* using GitHub as the coordination layer. The two patterns are deliberately analogous — `loom:issue` → `loom:building` → `loom:review-requested` → `loom:pr` is the GitHub-side mirror of Anvil's `EMPTY → DRAFTED → REVIEWED → READY → AUDITED`.

See `.loom/CLAUDE.md` for the Loom configuration shipped with this repo. See `ROADMAP.md` for what Anvil is, where it stands today, and where it's going. See `WORK_LOG.md` for the chronological record of merged PRs and closed issues. See `WORK_PLAN.md` for the current prioritized backlog.
