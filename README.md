...
**Status:** v0.10.1. Installable — see the Skills table below for the current list. Actively developed against a real-world canary consumer ([2AM Logic Studio](https://2amlogic.com)) — the framework is sharpened by being used, not by being designed in isolation.
...
| `anvil:spec` | Normative technical specifications maintained truthfully against an implementation (protocol whitepapers, wire-format specs, consensus rules; normative correctness as the owned dominant rubric dim on the ≥39 audit-grade band, an optional `code_ref` companion input — the mirror image of primer's `spec_ref` — feeding a spec↔implementation consistency audit, adoption-first, AUDITED-terminal; three-way audit verdict + implementation-status register per #707) | LaTeX → PDF |
| `anvil:memoir` | Chaptered narrative nonfiction reconstructed from a private evidentiary corpus (family memoirs, oral histories, biography-from-archive; sourcing fidelity as the owned dominant rubric dim on the ≥39 audit-grade band, dual-corpus claim provenance and dual voice tiers — narrator + reconstructed dialogue — active at once, an exhaustive `kind: tool_evidence` corpus-audit sibling, chapter-thread-native, photo-placement macros for `anvil:project-photos`, AUDITED-terminal; assembled into a book via `anvil:project-book`) | LaTeX → PDF |
...
| `anvil:help` | Strictly read-only orientation tool: introspects the installed skill set (from `.anvil/install-metadata.json`, falling back to a `.claude/skills/anvil-*/` scan) and prints a two-tier view — `anvil:help` overview (installed skills grouped artifact-vs-utility, the common lifecycle with its per-skill variations, a "start here" pointer), `anvil:help <skill>` deep-dive (real command set, rubric total/threshold, thread layout). Describes only what is installed; writes nothing | Terminal output (no files written) |
| `anvil:deslop` | Prose-cleanup tool: iterates arbitrary AI-drafted prose that lives outside any anvil-authored project (website copy, README fragments, existing markdown/HTML, pasted text) clean of AI-tell rhetoric and voice mismatch — deterministic rhetoric lint + LLM critique loop to convergence, honoring a consumer's declared `voice.rhetoric_rules` / voice-grounding docs when pointed at a project. Never edits the source | Cleaned text + per-change rationale + a ready-to-apply diff |
| `anvil:ip-search` | Prior-art search tool for the ip suite: derives queries from a thread's `BRIEF.md` inventive-feature inventory, queries a live patent corpus (PatentsView / USPTO Open Data Portal; Google Patents as a documented manual fallback), and writes cited reference summaries into `<thread>/prior-art/` in exactly the frontmatter shape the prior-art positioning critics already parse. Degrades gracefully with no API key (writes nothing, prints the manual query URLs, exits 0); writes `<thread>/prior-art/` and nothing else; a drafting aid, never a clearance search. Runs by hand, or automatically ahead of the prior-art positioning critic when a thread opts in via `--search` or `{ "prior_art_search": { "auto": true } }` in `<thread>/.anvil.json` (OFF by default — absent the knob there is no corpus query, no API-key read, and no network call; code-enforced via `anvil/skills/ip-search/lib/prior_art_step.py`) | `<thread>/prior-art/<slug>.md` reference summaries |
| `anvil:diff` | Local, ephemeral, read-only viewer tool: renders a word-level (not line-level) side-by-side HTML diff between two anvil version dirs, a deslop origin/`cleaned.txt` pair, or two arbitrary files, with an optional `.review/` sidecar + `rhetoric_lint` overlay anchored to the lines they describe. Stdlib-only (`http.server` + `difflib`), binds `127.0.0.1` on an ephemeral port, never writes to any input path | Localhost-served HTML page (+ optional `--out` file) |
...
Anvil installs into a target repository (the consumer repo) where you do the authoring work. The installer copies the framework + selected skills, writes thin Claude Code and Codex CLI skill registrations, generates a consumer-side `pyproject.toml`, and appends an Anvil section to the target's `CLAUDE.md` and `AGENTS.md`.
...
- `.anvil/` — the framework + skills, self-contained, with its own `pyproject.toml`. `uv sync --project .anvil` resolves against that file only, so the anvil venv stays independent of the monorepo's own uv project (root `pyproject.toml` / `uv.lock` are never read or written). It also ships a self-contained `.anvil/.gitignore` (patterns `__pycache__/`, `*.py[cod]`, `.venv/`, `*.egg-info/`) so the Python bytecode caches, the uv venv, and the setuptools egg-info metadata `uv sync`'s editable install materializes never dirty `git status`; that file is written once (skip-if-exists) and your root `.gitignore` is never touched. Re-installing an older install does not rewrite it — a pattern added to the template after you installed is *appended* if missing, leaving your own additions verbatim and never duplicating a pattern you already added yourself.
...
- `.agents/skills/anvil-<skill>/SKILL.md` — a thin per-skill pointer file for Codex CLI discovery, mirroring the Claude shim above; it points back at the canonical `.anvil/skills/<skill>/SKILL.md` body rather than duplicating it, and is pruned in lockstep with the Claude shim when a skill is removed via `--skills=`.
- `CLAUDE.md` and `AGENTS.md` — the only root-level files the installer writes. Each gets its own marker-bounded block (`<!-- BEGIN ANVIL --> … <!-- END ANVIL -->`) appended after existing content; re-installs replace that block in place rather than appending a duplicate. Everything outside the markers — including a Loom section — passes through verbatim. (One nuance: if the file ends in multiple blank lines, the first install normalizes that trailing run to a single blank line before the block.) The two files share the same runtime-neutral pointer prose; Claude's `/anvil:<skill>` slash-command syntax and Codex's `.agents/skills/anvil-<skill>/SKILL.md` path are isolated into their own per-runtime adapter line so neither leaks into the other file.
...
anvil/
  skills/        Per-artifact-type skills (memo, paper, report, deck, slides,
                 ...
                 datasheet, essay, primer, spec, memoir) plus the bridge-tool
                 and utility skills. Each has SKILL.md +
