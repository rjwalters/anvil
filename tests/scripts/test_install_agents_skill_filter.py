...
def test_source_ships_the_expected_agent_registry() -> None:
    """The source registry is 54 ``anvil-<skill>-<phase>.md`` files.
    ...
    lockstep — that's the intended coupling.
    """

    assert len(_all_source_agents()) == 54, (
        "expected 54 anvil-*.md source agents; the registry changed — update "
...
# ---------------------------------------------------------------------------
# Full install: no regression (all 54)
# ---------------------------------------------------------------------------


def test_full_install_still_ships_all_agents(tmp_path: Path) -> None:
    """A no-``--skills=`` install still copies every source agent (all 54)."""
