...
stamping landed by issue #346 and applied to the `paper` skill by issue
...
``test_paper_rubric_version_transition_doc.py`` so pytest does not collide
...
PUB_REVIEW_MD = SKILL_ROOT / "commands" / "paper-review.md"
...
# ---------------------------------------------------------------------------
# Doc-coverage: paper-review step 3 stamps `rubric_id` + `rubric_total` +
# `advance_threshold` on `_meta.json` (issue #346)
# ---------------------------------------------------------------------------


def test_paper_review_step3_stamps_rubric_id():
    body = _read(PUB_REVIEW_MD)
    assert "rubric_id" in body, (
        "paper-review.md step 3 MUST stamp `rubric_id` on `_meta.json` "
        "per issue #346 / `snippets/scorecard_kind.md` §'Rubric version "
        "stamping fields'."
    )
    # Pin the specific identifier — the paper skill ships /44 via
    # `anvil-pub-v2` per `anvil/skills/paper/rubric.md` line 3.
    assert '"anvil-pub-v2"' in body, (
        "paper-review.md step 3 MUST hardcode `rubric_id: \"anvil-pub-v2\"` "
        "(the paper skill's current /44 rubric identifier)."
    )


def test_paper_review_step3_stamps_rubric_total_and_threshold():
    body = _read(PUB_REVIEW_MD)
    assert "rubric_total" in body and "advance_threshold" in body, (
        "paper-review.md step 3 MUST stamp `rubric_total` AND "
...
def test_paper_review_emits_top_level_rubric_block():
    body = _read(PUB_REVIEW_MD)
    assert '"rubric": {' in body, (
        "paper-review.md MUST emit a top-level `rubric` block "
...
def test_paper_review_documents_prior_rubric_surfacing():
    body = _read(PUB_REVIEW_MD)
...
def test_paper_review_emits_rubric_transition_subsection():
    body = _read(PUB_REVIEW_MD)
    assert "## Rubric version transition" in body, (
        "paper-review.md MUST emit a `## Rubric version "
        "transition` subsection in findings.md when the prior rubric "
        "differs (issue #346 AC 5)."
    )


def test_paper_review_documents_score_history_rubric_id_append():
    body = _read(PUB_REVIEW_MD)
    ...
    assert (
        ...
        "paper-review.md MUST document the per-row `rubric_id` "
...
def test_fixture_dir_present():
    assert FIXTURES.is_dir(), (
        "tests/skills/paper/fixtures/rubric_version_transition/ "
...
# ---------------------------------------------------------------------------
# Rubric file pinning: paper /44 rubric line 3
# ---------------------------------------------------------------------------


def test_paper_rubric_md_declares_44_threshold_35():
    body = _read(PUB_RUBRIC_MD)
    assert "**44**" in body or "/44" in body
    assert "≥35/44" in body or "≥35**" in body


def test_paper_rubric_md_declares_dim_9_rhetorical_economy():
    """The paper rubric MUST carry a dim 9 row for *Rhetorical economy* at
