"""Doc-coverage guard for the paper skill's adoption of the per-phase
...
pins the paper-skill adoption (the #350-style phased rollout: snippet +
...
named ``test_paper_git_sync_doc.py``.
"""
...
SKILL_MD = REPO_ROOT / "anvil" / "skills" / "paper" / "SKILL.md"

# The 7 write-bearing paper commands per the issue #436 curation
# inventory. The portfolio orchestrator (paper.md) is read-only and
...
def test_command_references_git_sync_snippet(command: str):
    """Every write-bearing paper command MUST reference the snippet."""
    text = _read(COMMANDS / command)
...
def test_command_names_commit_per_phase_knob(command: str):
    """Every write-bearing paper command MUST name the conditional knob."""
    text = _read(COMMANDS / command)
...
def test_command_git_sync_uses_structured_message_shape(command: str):
    ...
    ``anvil(paper/<phase>):`` so the orchestrator-facing shape can't
    drift per-command."""
    text = _read(COMMANDS / command)
    assert "anvil(paper/" in text, (
        f"{command}'s git-sync step MUST use the anvil(paper/<phase>): "
