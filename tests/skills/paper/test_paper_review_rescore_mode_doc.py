"""Doc-coverage guard for the paper-review `--rescore-mode` wiring (issue #368).
...
Paper's review carries the `_review.json` canonical critic shape per
...
``test_paper_review_rescore_mode_doc.py`` to avoid collision.
"""
...
def test_paper_review_doc_describes_prior_rubric_lookup_at_n_not_n_minus_one():
    ...
    assert "N-1" in text or "N - 1" in text, (
        "paper-review.md must explicitly contrast the rescore-mode prior-"
    ...
    assert "<thread>.{N}.review/" in rescore_slice, (
        "paper-review.md's `--rescore-mode` block must explicitly name "
        "the `<thread>.{N}.review/_meta.json` target for the prior-rubric "
        "lookup under rescore mode (issue #368)."
    )


def test_paper_review_doc_references_issue_368():
    """Audit-trail: the issue number must appear in the doc for traceability."""
    text = _read(PUB_REVIEW_DOC)
    assert "#368" in text, (
        "paper-review.md must reference issue #368 in the `--rescore-mode` "
