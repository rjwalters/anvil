"""Doc-coverage guard for paper-audit staged-sidecar wiring (issue #350)."""
...
DOC = REPO_ROOT / "anvil" / "skills" / "paper" / "commands" / "paper-audit.md"
...
def test_paper_audit_doc_references_staged_sidecar_primitive():
    text = _read()
    assert "anvil/lib/sidecar.py" in text
    assert "staged_sidecar" in text


def test_paper_audit_doc_names_required_files_manifest():
    text = _read()
    for name in (
        "citation-audit.md",
        "numerical-audit.md",
...
def test_paper_audit_doc_step_1_invokes_cleanup_sweep():
    text = _read()
    assert "cleanup_one_staging" in text


def test_paper_audit_doc_describes_atomic_rename_contract():
    text = _read()
    assert "atomically" in text.lower() or "atomic" in text.lower()
    assert "leading-dot" in text or ".tmp" in text


def test_paper_audit_doc_references_issue_350():
    text = _read()
    assert "#350" in text


def test_paper_audit_doc_documents_non_python_driver_fallback():
