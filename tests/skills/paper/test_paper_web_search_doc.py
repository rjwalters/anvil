...
`paper-review` stays read-only: under the knob it may run 3–5 targeted
D4 searches whose findings land in `comments.md` as `related-work`-
tagged leads recommending a `paper-litsearch` re-run.

These tests pin the load-bearing prose tokens in the three markdown
carriers (`paper-litsearch.md`, `paper-review.md`, `SKILL.md`) so a future
...
``test_paper_web_search_doc.py`` to avoid collision.
"""
...
PUB_SKILL_DIR = REPO_ROOT / "anvil" / "skills" / "paper"
PUB_LITSEARCH_DOC = PUB_SKILL_DIR / "commands" / "paper-litsearch.md"
PUB_REVIEW_DOC = PUB_SKILL_DIR / "commands" / "paper-review.md"
...
def test_litsearch_doc_documents_web_search_knob():
    ...
    assert "web_search" in text, (
        "paper-litsearch.md must document the `web_search` BRIEF "
        "frontmatter knob (issue #424)."
    )
    assert "web_search: true" in text, (
        "paper-litsearch.md must show the `web_search: true` opt-in "
        "shape (issue #424)."
    )
...
def test_litsearch_doc_states_off_by_default_byte_identical():
    ...
    assert "byte-identical" in text, (
        "paper-litsearch.md must state that knob-absent / `false` "
...
def test_litsearch_doc_names_resolver_chokepoint():
    ...
    assert "anvil/lib/cite.py" in text, (
        "paper-litsearch.md must name `anvil/lib/cite.py` as the "
        "verification chokepoint (issue #424)."
    )
    assert "resolve()" in text and "BibRecord" in text, (
        "paper-litsearch.md must state that a web-discovered candidate "
...
def test_litsearch_doc_demotes_resolver_failures_to_leads():
    ...
    assert "UnsupportedIdentifierError" in text, (
        "paper-litsearch.md must route pmid/url hits "
        "(UnsupportedIdentifierError) to leads, never citations "
        "(issue #424)."
    )
    assert "CiteResolutionError" in text, (
        "paper-litsearch.md must route retry-exhausted resolution "
...
def test_litsearch_doc_specifies_web_leads_section():
    ...
    assert "## Web leads (unverified)" in text, (
        "paper-litsearch.md must specify the `## Web leads (unverified)` "
...
def test_litsearch_doc_specifies_provenance_table():
    ...
    assert "provenance" in text.lower(), (
        "paper-litsearch.md must specify the provenance table mapping "
        "web-verified bib keys to identifier + resolver (issue #424)."
    )
    assert "bib_key()" in text, (
        "paper-litsearch.md must route key generation through the lib's "
...
def test_litsearch_doc_references_issue_424():
    ...
    text = _read(PUB_LITSEARCH_DOC)
    assert "#424" in text, (
        "paper-litsearch.md must reference issue #424 in the web-search "
...
def test_litsearch_doc_keeps_no_invent_constraint_section():
    ...
    assert "## Critical constraint: do not invent citations" in text, (
        "paper-litsearch.md must keep the `Critical constraint: do not "
...
def test_review_doc_documents_web_search_knob():
    ...
    assert "web_search" in text, (
        "paper-review.md must document the `web_search` knob for the D4 "
...
def test_review_doc_keeps_reviewer_read_only_and_routes_leads():
    ...
    assert "related-work" in knob_slice, (
        "paper-review.md's web_search prose must tag web findings as "
        "`related-work` leads in comments.md (issue #424)."
    )
    assert "paper-litsearch" in knob_slice, (
        "paper-review.md's web_search prose must route verification to a "
        "`paper-litsearch` re-run — the resolver-verified-or-dropped "
...
def test_review_doc_states_byte_identical_when_knob_unset():
    ...
    assert "byte-identical" in knob_slice, (
        "paper-review.md's web_search Inputs bullet must state the "
...
def test_review_doc_references_issue_424():
    text = _read(PUB_REVIEW_DOC)
    assert "#424" in text, (
        "paper-review.md must reference issue #424 in the web-search "
        "prose for audit-trail traceability."
    )
...
def test_skill_doc_documents_knob_and_contract():
    ...
    assert "web_search" in text, (
        "paper SKILL.md must document the `web_search` knob (issue #424)."
    )
    assert "resolver-verified-or-dropped" in text, (
        "paper SKILL.md must name the resolver-verified-or-dropped "
        "contract (issue #424)."
    )
    assert "## Web leads (unverified)" in text or "Web leads (unverified)" in text, (
        "paper SKILL.md must mention the `Web leads (unverified)` leads "
        "section (issue #424)."
    )
