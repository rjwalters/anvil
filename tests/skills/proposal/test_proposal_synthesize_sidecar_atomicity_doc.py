"""Doc-coverage guard for proposal-synthesize staged-sidecar wiring
(issue #350).
"""
...
DOC = (
    REPO_ROOT / "anvil" / "skills" / "proposal" / "commands" / "proposal-synthesize.md"
)

_read = functools.partial(read_text, DOC)


def test_proposal_synthesize_doc_references_staged_sidecar_primitive():
    text = _read()
    assert "anvil/lib/sidecar.py" in text
    assert "staged_sidecar" in text


def test_proposal_synthesize_doc_names_required_files_manifest():
    text = _read()
    for name in (
        "verdict.md",
        "synthesis.md",
...
def test_proposal_synthesize_doc_step_1_invokes_cleanup_sweep():
    text = _read()
    assert "cleanup_one_staging" in text


def test_proposal_synthesize_doc_describes_atomic_rename_contract():
    text = _read()
    assert "atomically" in text.lower() or "atomic" in text.lower()
    assert "leading-dot" in text or ".tmp" in text


def test_proposal_synthesize_doc_references_issue_350():
    text = _read()
    assert "#350" in text


def test_proposal_synthesize_doc_documents_non_python_driver_fallback():
