"""Doc-coverage guard for the proposal skill's adoption of the per-phase
...
pins the proposal-skill adoption (the #350-style phased rollout: snippet +
...
named ``test_proposal_git_sync_doc.py``.
"""
...
COMMANDS = REPO_ROOT / "anvil" / "skills" / "proposal" / "commands"
SKILL_MD = REPO_ROOT / "anvil" / "skills" / "proposal" / "SKILL.md"

# The 7 write-bearing proposal commands per the issue #436 curation
# inventory. The portfolio orchestrator (proposal.md) is read-only and
...
def test_command_references_git_sync_snippet(command: str):
    """Every write-bearing proposal command MUST reference the snippet."""
    text = _read(COMMANDS / command)
...
def test_command_names_commit_per_phase_knob(command: str):
    """Every write-bearing proposal command MUST name the conditional knob."""
    text = _read(COMMANDS / command)
...
def test_command_git_sync_uses_structured_message_shape(command: str):
    ...
    ``anvil(proposal/<phase>):`` so the orchestrator-facing shape can't
    drift per-command."""
    text = _read(COMMANDS / command)
    assert "anvil(proposal/" in text, (
        f"{command}'s git-sync step MUST use the anvil(proposal/<phase>): "
