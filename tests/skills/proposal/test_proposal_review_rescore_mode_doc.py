"""Doc-coverage guard for the proposal-review `--rescore-mode` wiring (issue #368).
...
``test_proposal_review_rescore_mode_doc.py`` to avoid collision.
"""
...
def test_proposal_review_doc_references_rescore_mode_flag():
    """The literal `--rescore-mode` token must appear in the command file body."""
    text = _read(PROPOSAL_REVIEW_DOC)
    assert "--rescore-mode" in text, (
        "proposal-review.md must document the `--rescore-mode` flag "
        "(issue #368) — the marker `anvil:rubric-rebackport`'s "
        "`check_rescore_hook('proposal')` scans for the literal token."
    )


def test_proposal_review_doc_describes_rescore_sidecar_routing():
    """The prose must explicitly name the rescore sidecar path shape."""
    text = _read(PROPOSAL_REVIEW_DOC)
    assert "review.rescore-" in text, (
        "proposal-review.md must document re-routing the staged_sidecar "
        "output to the `<thread>.{N}.review.rescore-<id>/` path shape "
        "(issue #368)."
    )


def test_proposal_review_doc_describes_rescore_state_completed_stamp():
    """The prose must explicitly state `rescore_state: \"completed\"` is written."""
    text = _read(PROPOSAL_REVIEW_DOC)
    assert "rescore_state" in text and "completed" in text, (
        "proposal-review.md must document stamping `rescore_state: "
        "\"completed\"` on `_meta.json` (overwriting the rebackport "
        "tool's `\"scheduled\"` placeholder; issue #368)."
    )


def test_proposal_review_doc_describes_prior_rubric_lookup_at_n_not_n_minus_one():
    ...
    assert "N-1" in text or "N - 1" in text, (
        "proposal-review.md must explicitly contrast the rescore-mode "
    ...
    assert "<thread>.{N}.review/" in rescore_slice, (
        "proposal-review.md's `--rescore-mode` block must explicitly name "
        "the `<thread>.{N}.review/_meta.json` target for the prior-rubric "
        "lookup under rescore mode (issue #368)."
    )


def test_proposal_review_doc_references_issue_368():
    """Audit-trail: the issue number must appear in the doc for traceability."""
    text = _read(PROPOSAL_REVIEW_DOC)
    assert "#368" in text, (
        "proposal-review.md must reference issue #368 in the `--rescore-mode` "
