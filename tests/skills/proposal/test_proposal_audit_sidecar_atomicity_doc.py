"""Doc-coverage guard for proposal-audit staged-sidecar wiring (issue #350)."""
...
DOC = REPO_ROOT / "anvil" / "skills" / "proposal" / "commands" / "proposal-audit.md"
...
def test_proposal_audit_doc_names_required_files_manifest():
    text = _read()
    for name in (
        "verdict.md",
        "findings.md",
...
def test_proposal_audit_doc_step_1_invokes_cleanup_sweep():
    text = _read()
    assert "cleanup_one_staging" in text


def test_proposal_audit_doc_describes_atomic_rename_contract():
    text = _read()
    assert "atomically" in text.lower() or "atomic" in text.lower()
    assert "leading-dot" in text or ".tmp" in text


def test_proposal_audit_doc_references_issue_350():
    text = _read()
    assert "#350" in text


def test_proposal_audit_doc_documents_non_python_driver_fallback():
