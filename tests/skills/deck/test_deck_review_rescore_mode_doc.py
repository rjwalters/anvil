"""Doc-coverage guard for the deck-review `--rescore-mode` wiring (issue #368).
...
Deck is the only skill in the suite with a SPLIT init step (step 3 =
staged_sidecar + `_progress.json`; step 4 = `_meta.json` init). The
rescore-mode prose block hangs off step 4 (where `_meta.json` is
stamped) and references the path target from step 3 by back-reference.

Deck-review also aggregates specialist critic siblings (`deck-narrative`,
`deck-market`, `deck-design`, `deck-vision`). The rescore-mode prose
explicitly notes specialists are NOT rescored in v0 — only the
aggregator (`deck-review`) rescores.

Per the per-skill test filename convention (#58), this file is named
``test_deck_review_rescore_mode_doc.py`` to avoid collision.
"""
...
def test_deck_review_doc_describes_prior_rubric_lookup_at_n_not_n_minus_one():
    ...
    assert "N-1" in text or "N - 1" in text, (
        "deck-review.md must explicitly contrast the rescore-mode prior-"
