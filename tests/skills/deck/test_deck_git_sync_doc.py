"""Doc-coverage guard for the deck skill's adoption of the per-phase
...
pins the deck-skill adoption (the #350-style phased rollout: snippet +
...
named ``test_deck_git_sync_doc.py``.

Issue #537 (Group C of the cross-skill rollout, mirroring the #528 memo
...
bracket, and the ``anvil(deck/<phase>):`` commit token. The two pilot
...
COMMANDS = REPO_ROOT / "anvil" / "skills" / "deck" / "commands"
SKILL_MD = REPO_ROOT / "anvil" / "skills" / "deck" / "SKILL.md"

# The 12 write-bearing deck commands per the issue #436 curation
# inventory. The portfolio orchestrator (deck.md) is read-only and
# exempt by definition, as are the non-executable
# contract/walkthrough documents (deck-imagegen-adapter.md, deck-imagegen-onboarding.md).
...
def test_command_references_git_sync_snippet(command: str):
    """Every write-bearing deck command MUST reference the snippet."""
    text = _read(COMMANDS / command)
...
def test_command_names_commit_per_phase_knob(command: str):
    """Every write-bearing deck command MUST name the conditional knob."""
    text = _read(COMMANDS / command)
...
def test_command_git_sync_uses_structured_message_shape(command: str):
    ...
    ``anvil(deck/<phase>):`` so the orchestrator-facing shape can't
    drift per-command."""
    text = _read(COMMANDS / command)
    assert "anvil(deck/" in text, (
        f"{command}'s git-sync step MUST use the anvil(deck/<phase>): "
