"""Doc-coverage smoke tests for the pub-draft BRIEF bootstrap interview.
...
documented in the two files it touches (commands/pub-draft.md and
...
``test_pub_`` prefix so it never collides with doc tests of other
skills.
"""
...
DRAFT_MD = SKILL_ROOT / "commands" / "pub-draft.md"
...
# ---------------------------------------------------------------------------
# pub-draft.md — interview path (new in #425)
# ---------------------------------------------------------------------------


def test_pub_draft_documents_bootstrap_interview_section():
    body = _read(DRAFT_MD)
    assert "BRIEF bootstrap interview" in body, (
        "pub-draft.md MUST have a 'BRIEF bootstrap interview' section per "
        "issue #425 AC"
    )


def test_pub_draft_step3_branches_on_interactivity():
    body = _read(DRAFT_MD)
    lowered = body.lower()
    assert "interactive" in lowered, (
        "pub-draft.md step 3 MUST branch on interactivity (interview when "
        "interactive, fail-fast when not) per issue #425 AC"
    )
    assert "non-interactive" in lowered, (
        "pub-draft.md MUST document the non-interactive fail-fast branch"
    )
    assert "fail fast" in lowered or "fail-fast" in lowered, (
        "pub-draft.md MUST preserve fail-fast behavior for non-interactive "
        "runs (issue #425 AC)"
    )


def test_pub_draft_documents_no_interview_flag():
    body = _read(DRAFT_MD)
    assert "--no-interview" in body, (
        "pub-draft.md MUST document the deterministic --no-interview opt-out "
        "(issue #425 AC; precedent: report-promote's --ack-file split)"
    )
    assert "report-promote" in body, (
        "pub-draft.md SHOULD cite the report-promote interactive-vs-flag "
        "precedent the --no-interview opt-out mirrors"
    )


def test_pub_draft_documents_four_mandatory_question_topics():
    body = _read(DRAFT_MD)
    lowered = body.lower()
    # The four mandatory interview topics per the curator question set.
    assert "venue" in lowered, (
        "pub-draft.md interview MUST cover the target venue (mandatory topic)"
    )
    assert "thesis" in lowered or "claim" in lowered, (
        "pub-draft.md interview MUST cover the thesis / claim (mandatory topic)"
    )
    assert "evidence inventory" in lowered, (
        "pub-draft.md interview MUST cover the evidence inventory "
        "(mandatory topic)"
    )
    assert "scope" in lowered, (
        "pub-draft.md interview MUST cover scope (audience / length / "
        "double-blind / keywords) (mandatory topic)"
    )


def test_pub_draft_documents_optional_title_authors_question():
    body = _read(DRAFT_MD)
    lowered = body.lower()
    assert "title" in lowered and "author" in lowered, (
        "pub-draft.md interview MUST list title / authors / affiliation as "
        "an optional question (issue #425 curator question set)"
    )


def test_pub_draft_synthesized_brief_requires_venue_and_claim_frontmatter():
    body = _read(DRAFT_MD)
    ...
    assert "`venue` + `claim`" in body or "venue + claim" in body, (
        "pub-draft.md MUST require frontmatter venue + claim at minimum in "
        "the synthesized BRIEF (issue #425 AC)"
    )


def test_pub_draft_synthesized_brief_mirrors_example_brief_shape():
    body = _read(DRAFT_MD)
    assert "example-brief.md" in body, (
        "pub-draft.md MUST point at assets/example-brief.md as the "
    ...
    assert "Motivation" in body, (
        "pub-draft.md MUST name the Motivation prose section of the "
        "synthesized BRIEF"
    )
    assert "Related work" in body or "Related-work" in body, (
        "pub-draft.md MUST name the related-work hooks section of the "
        "synthesized BRIEF"
    )


def test_pub_draft_documents_todo_operator_markers():
    body = _read(DRAFT_MD)
    assert "# TODO(operator)" in body, (
        "pub-draft.md MUST document the # TODO(operator) marker discipline "
        "for skipped answers (issue #425 AC; #408 project-migrate precedent)"
    )
    assert "project-migrate" in body or "#408" in body, (
        "pub-draft.md SHOULD cite the #408 project-migrate starter-synthesis "
        "precedent for the TODO-marker discipline"
    )


def test_pub_draft_documents_no_fabrication_rule():
    body = _read(DRAFT_MD)
    lowered = body.lower()
    assert "never fabricated" in lowered or "no-fabrication" in lowered or (
        "no fabrication" in lowered
    ), (
        "pub-draft.md MUST document the no-fabrication rule for the interview "
        "(issue #425 AC)"
    )
    assert "MUST NOT invent" in body or "never invent" in body.lower(), (
        "pub-draft.md MUST use normative language: the drafter never invents "
        "evidence, results, or citations to fill interview gaps"
    )


def test_pub_draft_web_search_is_explicit_opt_in_only():
    body = _read(DRAFT_MD)
    assert "web_search" in body, (
        "pub-draft.md interview MUST cover the web-search appetite question "
        "(issue #425 curator question set)"
    )
    lowered = body.lower()
    assert "opt-in" in lowered, (
        "pub-draft.md MUST state web_search is emitted on explicit opt-in "
    ...
    assert "omitted otherwise" in lowered or "omit otherwise" in lowered or (
        ...
        "pub-draft.md MUST state the web_search key is omitted when the "
        "author does not opt in (no key, not web_search: false ambiguity)"
    )


def test_pub_draft_fail_fast_message_names_both_remedies():
    body = _read(DRAFT_MD)
    lowered = body.lower()
    # AC: the fail-fast error message names both remedies — write by hand,
    # or re-run interactively.
    assert "by hand" in lowered, (
        "pub-draft.md MUST document remedy (a): write <thread>/BRIEF.md by "
        "hand (issue #425 AC)"
    )
    assert "re-run" in lowered and "interactively" in lowered, (
        "pub-draft.md MUST document remedy (b): re-run pub-draft "
        "interactively for the interview path (issue #425 AC)"
    )


def test_pub_draft_lifecycle_unchanged_after_bootstrap():
    body = _read(DRAFT_MD)
    lowered = body.lower()
    assert "no new state" in lowered, (
        "pub-draft.md MUST state the lifecycle is unchanged post-bootstrap: "
        "no new states (issue #425 AC)"
    )
    assert "_progress.json" in body, (
        "pub-draft.md MUST state there is no _progress.json schema change "
        "(the interview happens before step 4 initializes progress)"
    )


def test_pub_draft_existing_brief_never_interviewed():
    body = _read(DRAFT_MD)
    lowered = body.lower()
    assert "never fires" in lowered or "unchanged" in lowered, (
        "pub-draft.md MUST state the interview never fires when BRIEF.md "
        "exists (issue #425 edge case: existing-brief path is unchanged)"
    )


def test_pub_draft_documents_project_brief_scope_exclusion():
    body = _read(DRAFT_MD)
    ...
    assert "per-thread" in body.lower(), (
        "pub-draft.md MUST scope the interview to the per-thread BRIEF only"
    )
    assert "--enroll" in body or "project-migrate" in body, (
        "pub-draft.md MUST note project-layout enrollment stays "
        "anvil:project-migrate --enroll territory (issue #425 scope exclusion)"
    )
    assert "documents:" in body, (
        "pub-draft.md MUST explicitly exclude the project-BRIEF documents: "
        "interplay (issue #425 curator decision 4)"
    )


def test_pub_draft_notes_per_thread_brief_has_no_strict_parser():
    body = _read(DRAFT_MD)
    lowered = body.lower()
    assert "no strict parser" in lowered, (
        "pub-draft.md MUST note the per-thread BRIEF has no strict parser "
...
def test_skill_md_artifact_contract_mentions_bootstrap():
    ...
    assert "interview" in block.lower(), (
        "pub SKILL.md artifact-contract BRIEF.md line MUST mention the "
        "pub-draft interview bootstrap (issue #425 AC)"
    )
...
def test_skill_md_command_table_row_mentions_bootstrap():
    ...
    row = next(
        (
            ...
            if line.startswith("| `pub-draft <thread>`")
        ),
        None,
    )
    assert row is not None, (
        "pub SKILL.md command-dispatch table MUST still have a pub-draft row"
    )
    assert "interview" in row.lower(), (
        "pub SKILL.md pub-draft command-table row MUST mention the BRIEF "
        "bootstrap interview (issue #425 AC)"
    )
    assert "--no-interview" in row, (
        "pub SKILL.md pub-draft command-table row MUST mention the "
...
def test_skill_md_preserves_web_search_default_off():
    ...
    assert "web_search: true" in body, (
        "pub SKILL.md MUST preserve the opt-in web_search section (#424)"
    )
    assert "default `false`" in body, (
        "pub SKILL.md MUST preserve the web_search default-off posture"
    )
...
def test_skill_md_preserves_lifecycle_line():
    ...
    assert "draft → review → revise → audit → figures" in body, (
        "pub SKILL.md MUST preserve the unchanged pub lifecycle — the "
...
def test_example_brief_reference_still_exists():
    ...
    assert EXAMPLE_BRIEF.exists(), (
        "anvil/skills/pub/assets/example-brief.md MUST exist as the "
        "synthesized-BRIEF shape reference (issue #425 AC)"
    )
...
def test_project_migrate_todo_marker_precedent_still_exists():
    """Guard: the #408 starter-synthesis TODO-marker precedent pub-draft.md
    ...
    assert "# TODO(operator)" in _read(PROJECT_MIGRATE_MD), (
        ...
        "discipline pub-draft's interview inherits (issue #425 / #408)"
    )
