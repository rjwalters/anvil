"""Doc-coverage guard for the pub-review `--rescore-mode` wiring (issue #368).
...
Pub's review carries the `_review.json` canonical critic shape per
`anvil/lib/review_schema.py`; the rescore_id surfaces alongside the
existing `rubric: "anvil-pub-v2"` field.

Per the per-skill test filename convention (#58), this file is named
``test_pub_review_rescore_mode_doc.py`` to avoid collision.
"""
...
def test_pub_review_doc_describes_prior_rubric_lookup_at_n_not_n_minus_one():
    ...
    assert "N-1" in text or "N - 1" in text, (
        "pub-review.md must explicitly contrast the rescore-mode prior-"
