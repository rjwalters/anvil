"""Doc-coverage guard for the pub skill's adoption of the per-phase
...
pins the pub-skill adoption (the #350-style phased rollout: snippet +
...
named ``test_pub_git_sync_doc.py``.
"""
...
SKILL_MD = REPO_ROOT / "anvil" / "skills" / "pub" / "SKILL.md"

# The 7 write-bearing pub commands per the issue #436 curation
# inventory. The portfolio orchestrator (pub.md) is read-only and
...
def test_command_references_git_sync_snippet(command: str):
    """Every write-bearing pub command MUST reference the snippet."""
    text = _read(COMMANDS / command)
...
def test_command_names_commit_per_phase_knob(command: str):
    """Every write-bearing pub command MUST name the conditional knob."""
    text = _read(COMMANDS / command)
...
def test_command_git_sync_uses_structured_message_shape(command: str):
    ...
    ``anvil(pub/<phase>):`` so the orchestrator-facing shape can't
    drift per-command."""
    text = _read(COMMANDS / command)
    assert "anvil(pub/" in text, (
        f"{command}'s git-sync step MUST use the anvil(pub/<phase>): "
