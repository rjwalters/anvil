"""Doc-coverage guard for the pub skill's adoption of the local-corpus
...
``git_sync.md`` adoption in ``test_pub_git_sync_doc.py``). This file pins
the pub-skill adoption so the conditional corpus-provenance steps can't
silently drift out of a command file in a later edit:

- ``pub-draft.md`` step 3b — drafter writes ``provenance.md`` before the
  LaTeX body, records ``metadata.corpus_dirs_resolved``.
- ``pub-review.md`` step 4d — reviewer back-checks 5–10 rows/pass and
  emits the ``provenance_back_check`` block; step 6 gains the
  fabrication-class critical flags.
- ``pub-audit.md`` step 5b — the exhaustive five-way ``kind:
  tool_evidence`` audit over a SEPARATE ``<thread>.{N}.corpus-audit/``
  sidecar with the ``provenance_summary`` six-counter roll-up.
- ``pub-revise.md`` step 8b — reviser copies ``provenance.md`` forward
...
named ``test_pub_provenance_doc.py``.
"""
...
COMMANDS = REPO_ROOT / "anvil" / "skills" / "pub" / "commands"
SNIPPET = REPO_ROOT / "anvil" / "lib" / "snippets" / "provenance.md"

# The 4 write-bearing pub commands adopting the provenance contract (the
# curation widened the issue-body's three to four — pub-revise carries
...
def test_snippet_exists():
    ...
    assert SNIPPET.is_file(), (
        ...
        "missing — the pub commands reference it, not a re-spec"
    )


@pytest.mark.parametrize("command", WRITE_BEARING_COMMANDS)
def test_command_references_provenance_snippet(command: str):
    """Every write-bearing pub command MUST reference the snippet — the
...
def test_draft_writes_map_before_prose_and_records_progress():
    """pub-draft step 3b MUST write provenance.md before the LaTeX body
    and record the resolved dirs in _progress.json so the reviewer and
    auditor can verify the run."""
    text = _read(COMMANDS / "pub-draft.md")
    assert "before the LaTeX body" in text or "before prose" in text, (
        "pub-draft MUST write provenance.md before the LaTeX body "
        "(§Section 2)"
    )
    assert "corpus_dirs_resolved" in text, (
        "pub-draft MUST record metadata.corpus_dirs_resolved when the "
        "tier is active"
    )


def test_review_documents_back_check_sample():
    """pub-review step 4d MUST document the 5–10-row spot-sample
    back-check as kind: judgment findings with quoted evidence."""
    text = _read(COMMANDS / "pub-review.md")
    assert "5–10 rows" in text or "5-10 rows" in text, (
        "pub-review MUST document the 5–10-row per-pass back-check "
        "sample (§Section 3)"
    )
    assert "kind: judgment" in text, (
        "pub-review back-check findings MUST be kind: judgment"
    )


def test_review_documents_provenance_back_check_block():
    """pub-review MUST emit the provenance_back_check _summary.md block
...
def test_audit_documents_corpus_audit_sibling():
    ...
    for fname in ("_review.json", "_meta.json", "_progress.json", "corpus-audit.md"):
        assert fname in text, (
            f"pub-audit MUST name {fname} in the .corpus-audit/ manifest"
        )


def test_audit_documents_tool_evidence_and_tool_calls():
    """pub-audit's .corpus-audit/_review.json MUST be kind: tool_evidence
    with non-empty tool_calls on every MISMATCH / NOT_FOUND / FABRICATED
    finding (§Section 4)."""
    text = _read(COMMANDS / "pub-audit.md")
    assert "kind: tool_evidence" in text, (
        "pub-audit .corpus-audit/_review.json MUST declare kind: "
        "tool_evidence"
    )
    assert "tool_calls" in text, (
        "pub-audit MUST document non-empty tool_calls on every "
        "MISMATCH / NOT_FOUND / FABRICATED finding"
    )


def test_audit_documents_provenance_summary_counters():
    """pub-audit MUST document metadata.provenance_summary with the six
    counters (§Section 7) in the .corpus-audit/_progress.json."""
    text = _read(COMMANDS / "pub-audit.md")
    assert "provenance_summary" in text, (
        "pub-audit MUST record metadata.provenance_summary (§Section 7)"
    )
    for counter in (
        "total_claims",
        "verified",
...
def test_read_only_files_are_exempt(exempt: str):
    ...
    contract — pub.md is exempt by definition."""
...
def test_no_lib_changes_scope_guard():
    ...
    file only — no lib file mentions the pub-specific progress key or the
