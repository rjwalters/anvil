r"""Doc-coverage guard for the pub-review multi-file ``\input``/``\include``
resolution wiring (issue #643).

``pub`` is the one anvil skill explicitly designed for multi-file LaTeX
papers — a master ``main.tex`` that ``\input{sections/...}``s many section
files is a normal whitepaper shape. Historically ``pub-review.md`` step 4
...
``test_pub_review_tex_includes_doc.py`` to avoid collision with the other
``test_pub_review_*_doc.py`` guards.
"""
...
PUB_REVIEW_DOC = (
    REPO_ROOT / "anvil" / "skills" / "pub" / "commands" / "pub-review.md"
)
PUB_AUDIT_DOC = (
    REPO_ROOT / "anvil" / "skills" / "pub" / "commands" / "pub-audit.md"
)
...
# ---------------------------------------------------------------------------
# pub-review.md step 4 — content read resolves the tree
# ---------------------------------------------------------------------------
...
def test_step4_names_the_resolver():
    ...
    assert "resolve_tex_inputs" in body, (
        "pub-review.md MUST name the resolver function resolve_tex_inputs "
        "(issue #643 AC — step 4 names the code that resolves the tree)"
    )
    assert "tex_includes" in body, (
        "pub-review.md MUST reference anvil/lib/tex_includes.py (issue #643)"
    )
...
def test_step4_does_not_regress_to_main_tex_only():
    ...
    assert idx > -1, "pub-review.md MUST retain a 'Read inputs' step"
    ...
    assert "recursively resolve" in lowered or "recursively-resolved" in lowered, (
        "pub-review.md step 4 MUST document recursively resolving "
        "\\input/\\include children (issue #643 AC)"
    )
    assert "\\input" in nearby and "\\include" in nearby, (
        "pub-review.md step 4 MUST name both \\input and \\include "
        "(issue #643 AC)"
    )
...
def test_step4_documents_reviewable_document_is_the_tree():
    ...
    assert "plus its resolved children" in lowered or "plus its children" in lowered or (
        ...
        "pub-review.md step 4 MUST frame the reviewable document as "
...
def test_step4_documents_single_file_degradation():
    ...
    assert "single-file thread" in lowered or "byte-identical" in lowered, (
        "pub-review.md step 4 MUST document the single-file degradation "
...
def test_step4_documents_missing_include_as_signal():
    ...
    assert "missing" in lowered and "signal" in lowered, (
        "pub-review.md step 4 MUST document dangling \\input as reviewer "
        "signal (a broken document) — issue #643"
    )


# ---------------------------------------------------------------------------
# pub-review.md step 4b — source_paths reuses the resolved list
# ---------------------------------------------------------------------------
...
def test_step4b_source_paths_uses_resolver():
    ...
    assert idx > -1, "pub-review.md MUST retain the render-gate source_paths input"
    nearby = body[idx : idx + 900]
    assert "ResolvedTex.files" in nearby or "resolve_tex_inputs" in nearby, (
        "pub-review.md step 4b source_paths MUST reuse the resolver's "
...
# ---------------------------------------------------------------------------
# pub-review.md step 5 / 5b — quoted-evidence covers children
# ---------------------------------------------------------------------------
...
def test_quoted_evidence_covers_input_children():
    ...
    assert "resolved body" in nearby.lower(), (
        "pub-review.md step 5 quoted-evidence rule MUST reference the "
...
def test_evidence_check_documented_to_expand_tex_body():
    ...
    assert "resolved body" in lowered, (
        "pub-review.md step 5b MUST document that evidence_check verifies "
        "against the resolved body (issue #643 AC)"
    )
    assert "resolve_tex_inputs" in nearby, (
        "pub-review.md step 5b MUST reference resolve_tex_inputs — the "
    ...
    assert "fabricated_evidence" in nearby or "fabricated evidence" in lowered, (
        "pub-review.md step 5b MUST explain the false-fabricated_evidence "
...
def test_frontmatter_reads_line_reflects_children():
    ...
    assert "\\input" in reads_line and "\\include" in reads_line, (
        "pub-review.md frontmatter Reads line MUST reflect that the "
...
def test_outputs_comments_line_reflects_children():
    ...
    assert idx > -1, "pub-review.md MUST retain the comments.md output line"
    nearby = body[idx : idx + 250]
    assert "\\input" in nearby or "children" in nearby, (
        "pub-review.md Outputs comments.md line MUST reflect that comments "
        "can key to \\input/\\include children (issue #643 AC)"
    )


# ---------------------------------------------------------------------------
# pub-audit.md sibling doc fix (guidance point 7)
# ---------------------------------------------------------------------------


def test_pub_audit_cite_enumeration_covers_children():
    """Issue #643 guidance 7: pub-audit's \\cite enumeration + claim
    ...
    assert "resolve_tex_inputs" in body, (
        "pub-audit.md MUST reference resolve_tex_inputs so its cite "
