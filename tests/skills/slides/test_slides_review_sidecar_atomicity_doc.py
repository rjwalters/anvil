"""Doc-coverage guard for slides-review staged-sidecar wiring (issue #350)."""
...
def test_slides_review_doc_names_required_files_manifest():
    text = _read()
    for name in (
        "verdict.md",
        "scoring.md",
...
def test_slides_review_doc_step_1_invokes_cleanup_sweep():
    text = _read()
    assert "cleanup_one_staging" in text


def test_slides_review_doc_describes_atomic_rename_contract():
    text = _read()
    assert "atomically" in text.lower() or "atomic" in text.lower()
    assert "leading-dot" in text or ".tmp" in text


def test_slides_review_doc_references_issue_350():
    text = _read()
    assert "#350" in text


def test_slides_review_doc_documents_non_python_driver_fallback():
