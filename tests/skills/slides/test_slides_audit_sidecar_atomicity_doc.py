"""Doc-coverage guard for slides-audit staged-sidecar wiring (issue #350)."""
...
DOC = REPO_ROOT / "anvil" / "skills" / "slides" / "commands" / "slides-audit.md"

_read = functools.partial(read_text, DOC)


def test_slides_audit_doc_references_staged_sidecar_primitive():
    text = _read()
    assert "anvil/lib/sidecar.py" in text
    assert "staged_sidecar" in text


def test_slides_audit_doc_names_required_files_manifest():
    text = _read()
    for name in ("verdict.md", "claims.md", "_meta.json", "_progress.json"):
        assert name in text, f"required file {name!r} missing from prose"


def test_slides_audit_doc_step_1_invokes_cleanup_sweep():
    text = _read()
    assert "cleanup_one_staging" in text


def test_slides_audit_doc_describes_atomic_rename_contract():
    text = _read()
    assert "atomically" in text.lower() or "atomic" in text.lower()
    assert "leading-dot" in text or ".tmp" in text


def test_slides_audit_doc_references_issue_350():
    text = _read()
    assert "#350" in text


def test_slides_audit_doc_documents_non_python_driver_fallback():
