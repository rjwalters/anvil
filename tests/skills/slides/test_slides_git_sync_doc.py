"""Doc-coverage guard for the slides skill's adoption of the per-phase
...
pins the slides-skill adoption (the #350-style phased rollout: snippet +
...
named ``test_slides_git_sync_doc.py``.
...
bracket, and the ``anvil(slides/<phase>):`` commit token. The two pilot
...
COMMANDS = REPO_ROOT / "anvil" / "skills" / "slides" / "commands"
SKILL_MD = REPO_ROOT / "anvil" / "skills" / "slides" / "SKILL.md"

# The 9 write-bearing slides commands per the issue #436 curation
# inventory. The portfolio orchestrator (slides.md) is read-only and
...
def test_command_references_git_sync_snippet(command: str):
    """Every write-bearing slides command MUST reference the snippet."""
    text = _read(COMMANDS / command)
...
def test_command_names_commit_per_phase_knob(command: str):
    """Every write-bearing slides command MUST name the conditional knob."""
    text = _read(COMMANDS / command)
...
def test_command_git_sync_uses_structured_message_shape(command: str):
    ...
    ``anvil(slides/<phase>):`` so the orchestrator-facing shape can't
    drift per-command."""
    text = _read(COMMANDS / command)
    assert "anvil(slides/" in text, (
        f"{command}'s git-sync step MUST use the anvil(slides/<phase>): "
