"""Doc-coverage guard for memo-image-accessibility staged-sidecar wiring
(issue #350).

Mirrors `test_memo_review_sidecar_atomicity_doc.py` from the memo-review
pilot (PR #354). memo-image-accessibility is opt-in via `--write-review`
and writes a 2-file sidecar (`_review.json` + `_findings.json`).
"""
...
DOC = (
    REPO_ROOT / "anvil" / "skills" / "memo" / "commands" / "memo-image-accessibility.md"
)
...
def test_memo_image_accessibility_doc_names_required_files_manifest():
    text = _read()
    for name in ("_review.json", "_findings.json"):
        assert name in text, f"required file {name!r} missing from prose"


def test_memo_image_accessibility_doc_invokes_cleanup_sweep():
    text = _read()
    assert "cleanup_one_staging" in text


def test_memo_image_accessibility_doc_describes_atomic_rename_contract():
    text = _read()
    assert "atomically" in text.lower() or "atomic" in text.lower()
    assert "leading-dot" in text or ".tmp" in text


def test_memo_image_accessibility_doc_references_issue_350():
    text = _read()
    assert "#350" in text


def test_memo_image_accessibility_doc_documents_non_python_driver_fallback():
