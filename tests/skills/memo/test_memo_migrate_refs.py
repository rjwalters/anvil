...
Covers the 10 acceptance criteria documented on the issue:
  ...
  5. §Sources parser handles all three observed shapes
     (beacon bulleted-with-markdown-link, geode numbered-prose,
     the-atelier numbered-bold-prefix). Parametrized fixtures derived from
...
# Beacon (bulleted, markdown-link). Subset of the on-disk BRIEF.md.
BEACON_SOURCES = """\
# Beacon — Memo Brief
...
class TestSourcesParserShapes(unittest.TestCase):
    ...
    def test_parse_beacon_bulleted_markdown_link_shape(self) -> None:
        """Beacon shape: ``- [Title](URL) — claim``."""
        entries = _parse_brief_sources(BEACON_SOURCES)
...
class TestSeedRefsFromBriefIntegration(unittest.TestCase):
    ...
    def test_idempotent_re_run_skips_existing_stubs(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td:
            thread_dir = Path(td) / "beacon"
            _write_brief(thread_dir, BEACON_SOURCES)
...
class TestStudioCanarySmoke(unittest.TestCase):
    ...
    def test_beacon_shape_produces_nonempty_refs_dir(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            thread_dir = Path(td) / "beacon"
            _write_brief(thread_dir, BEACON_SOURCES)
            result = seed_refs_from_brief(thread_dir)
