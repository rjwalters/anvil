"""Doc-coverage guard for memo-redteam.md staged-sidecar wiring (issues #350, #645).
...
DOC = REPO_ROOT / "anvil" / "skills" / "memo" / "commands" / "memo-redteam.md"
...
def test_memo_redteam_doc_references_staged_sidecar_primitive():
    text = _read()
    assert "anvil/lib/sidecar.py" in text
    assert "staged_sidecar" in text


def test_memo_redteam_doc_names_required_files_manifest():
    text = _read()
    for name in ("_review.json", "objections.md", "_meta.json", "_progress.json"):
        assert name in text, f"required file {name!r} missing from prose"


def test_memo_redteam_doc_invokes_cleanup_sweep():
    text = _read()
    assert "cleanup_one_staging" in text or "sidecar cleanup" in text


def test_memo_redteam_doc_describes_atomic_rename_contract():
    text = _read()
    assert "atomically" in text.lower() or "atomic" in text.lower()
    assert "leading-dot" in text or ".tmp" in text


def test_memo_redteam_doc_references_issue_350():
    text = _read()
    assert "#350" in text

def test_memo_redteam_doc_documents_non_python_driver_fallback():
