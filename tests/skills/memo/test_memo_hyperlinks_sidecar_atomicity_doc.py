"""Doc-coverage guard for memo-hyperlinks staged-sidecar wiring (issue #350).

Mirrors `test_memo_review_sidecar_atomicity_doc.py` from the memo-review
pilot (PR #354). The hyperlinks critic is a flat-shape sidecar
(`_review.json` only) opt-in via `--write-review`.
"""
...
DOC = REPO_ROOT / "anvil" / "skills" / "memo" / "commands" / "memo-hyperlinks.md"

_read = functools.partial(read_text, DOC)


def test_memo_hyperlinks_doc_references_staged_sidecar_primitive():
    text = _read()
    assert "anvil/lib/sidecar.py" in text
    assert "staged_sidecar" in text


def test_memo_hyperlinks_doc_names_required_files_manifest():
    text = _read()
    assert "_review.json" in text


def test_memo_hyperlinks_doc_step_1_invokes_cleanup_sweep():
    text = _read()
    assert "cleanup_one_staging" in text


def test_memo_hyperlinks_doc_describes_atomic_rename_contract():
    text = _read()
    assert "atomically" in text.lower() or "atomic" in text.lower()
    assert "leading-dot" in text or ".tmp" in text


def test_memo_hyperlinks_doc_references_issue_350():
    text = _read()
    assert "#350" in text


def test_memo_hyperlinks_doc_documents_non_python_driver_fallback():
