"""Doc-coverage guard for memo-figure-content staged-sidecar wiring (issue #350).

Mirrors `test_memo_review_sidecar_atomicity_doc.py` from the memo-review
pilot (PR #354). The figure-content critic is a flat-shape sidecar
(`_review.json` only) opt-in via `--write-review`; the staged_sidecar
wiring is conditional on that flag but the prose still pins the contract.
"""
...
def test_memo_figure_content_doc_names_required_files_manifest():
    text = _read()
    assert "_review.json" in text


def test_memo_figure_content_doc_step_1_invokes_cleanup_sweep():
    text = _read()
    assert "cleanup_one_staging" in text


def test_memo_figure_content_doc_describes_atomic_rename_contract():
    text = _read()
    assert "atomically" in text.lower() or "atomic" in text.lower()
    assert "leading-dot" in text or ".tmp" in text


def test_memo_figure_content_doc_references_issue_350():
    text = _read()
    assert "#350" in text


def test_memo_figure_content_doc_documents_non_python_driver_fallback():
