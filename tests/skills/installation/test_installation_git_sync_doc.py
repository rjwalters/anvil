"""Doc-coverage guard for the installation skill's adoption of the per-phase
...
pins the installation-skill adoption (the #350-style phased rollout: snippet +
...
named ``test_installation_git_sync_doc.py``.
"""
...
COMMANDS = REPO_ROOT / "anvil" / "skills" / "installation" / "commands"
SKILL_MD = REPO_ROOT / "anvil" / "skills" / "installation" / "SKILL.md"

# The 4 write-bearing installation commands per the issue #436 curation
# inventory. The portfolio orchestrator (installation.md) is read-only and
# exempt by definition.
WRITE_BEARING_COMMANDS = [
    "installation-draft.md",
    "installation-review.md",
    "installation-revise.md",
    "installation-figures.md",
]
...
def test_command_references_git_sync_snippet(command: str):
    """Every write-bearing installation command MUST reference the snippet."""
    text = _read(COMMANDS / command)
...
def test_command_names_commit_per_phase_knob(command: str):
    """Every write-bearing installation command MUST name the conditional knob."""
    text = _read(COMMANDS / command)
...
def test_command_git_sync_uses_structured_message_shape(command: str):
    ...
    ``anvil(installation/<phase>):`` so the orchestrator-facing shape can't
    drift per-command."""
    text = _read(COMMANDS / command)
    assert "anvil(installation/" in text, (
        f"{command}'s git-sync step MUST use the anvil(installation/<phase>): "
