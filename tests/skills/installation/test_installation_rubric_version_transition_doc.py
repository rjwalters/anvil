...
stamping landed by issue #346 and applied to the `installation` skill
by issue #357 (the /40 → /44 migration with dim 9 *Rhetorical economy*).
...
``test_installation_rubric_version_transition_doc.py`` so pytest does
not collide with future test files for adjacent issues.
"""
...
SKILL_ROOT = REPO_ROOT / "anvil" / "skills" / "installation"

INSTALLATION_REVIEW_MD = SKILL_ROOT / "commands" / "installation-review.md"
INSTALLATION_RUBRIC_MD = SKILL_ROOT / "rubric.md"
...
# ---------------------------------------------------------------------------
# Doc-coverage: installation-review step 3 stamps `rubric_id` etc.
# ---------------------------------------------------------------------------


def test_installation_review_step3_stamps_rubric_id():
    body = _read(INSTALLATION_REVIEW_MD)
    assert "rubric_id" in body, (
        "installation-review.md step 3 MUST stamp `rubric_id` on `_meta.json` "
        "per issue #346 / `snippets/scorecard_kind.md`."
    )
    assert '"anvil-installation-v2"' in body, (
        "installation-review.md step 3 MUST hardcode "
        "`rubric_id: \"anvil-installation-v2\"`."
    )


def test_installation_review_step3_stamps_rubric_total_and_threshold():
    body = _read(INSTALLATION_REVIEW_MD)
...
def test_installation_review_emits_top_level_rubric_block():
    body = _read(INSTALLATION_REVIEW_MD)
    assert '"rubric": {' in body, (
        "installation-review.md MUST emit a top-level `rubric` block "
...
def test_installation_review_documents_prior_rubric_surfacing():
    body = _read(INSTALLATION_REVIEW_MD)
...
def test_installation_review_emits_rubric_transition_subsection():
    body = _read(INSTALLATION_REVIEW_MD)
    assert "## Rubric version transition" in body


def test_installation_review_documents_score_history_rubric_id_append():
    body = _read(INSTALLATION_REVIEW_MD)
...
def test_fixture_progress_iter2_stamped_rubric_transition():
    ...
    rows = data["metadata"]["score_history"]
    assert len(rows) == 2
    assert rows[0]["rubric_id"] == "anvil-installation-v1-legacy-40"
    assert rows[1]["rubric_id"] == "anvil-installation-v2"
...
def test_fixture_meta_stamped_v1_carries_legacy_40_stamp():
    ...
    assert data["rubric_id"] == "anvil-installation-v1-legacy-40"
...
def test_fixture_meta_stamped_v2_carries_44_stamp():
    ...
    assert data["rubric_id"] == "anvil-installation-v2"
...
def test_fixture_summary_carries_rubric_block_with_prior_rubric_inferred():
    ...
    assert rubric_block["id"] == "anvil-installation-v2"
...
# ---------------------------------------------------------------------------
# Rubric file pinning: installation /44 rubric line 3
# ---------------------------------------------------------------------------


def test_installation_rubric_md_declares_44_threshold_35():
    body = _read(INSTALLATION_RUBRIC_MD)
    assert "**44**" in body or "/44" in body
    assert "≥35/44" in body or "≥35**" in body


def test_installation_rubric_md_declares_dim_9_rhetorical_economy():
    body = _read(INSTALLATION_RUBRIC_MD)
