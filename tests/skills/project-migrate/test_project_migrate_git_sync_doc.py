"""Doc-coverage guard for the project-migrate bridge tool's adoption of
the per-phase git commit/sync hook (issue #426; rolled out to the
bridge/utility tools in issue #436).

project-migrate is not a ``<thread>.{N}`` lifecycle phase, so it uses the
non-thread commit shape (``anvil(project-migrate/apply): <project>
[MIGRATED]``) canonicalized in ``anvil/lib/snippets/git_sync.md``
§Commit-message shape → "Non-thread commit shapes". The hook applies on
the ``--apply`` path only — dry-run / ``--report`` modes write nothing.
...
named ``test_project_migrate_git_sync_doc.py``.
"""
...
COMMAND = (
    REPO_ROOT / "anvil" / "skills" / "project-migrate" / "commands"
    / "project-migrate.md"
)
SKILL_MD = REPO_ROOT / "anvil" / "skills" / "project-migrate" / "SKILL.md"
...
def _git_sync_section() -> str:
    ...
    assert idx >= 0, "project-migrate.md missing the Git sync section"
    return text[idx:]


def test_command_references_git_sync_snippet():
    assert "snippets/git_sync.md" in _read(COMMAND), (
        "project-migrate.md MUST reference anvil/lib/snippets/git_sync.md "
        "(issue #436)"
    )
...
def test_command_names_commit_per_phase_knob():
    assert "commit_per_phase" in _read(COMMAND), (
        "project-migrate.md MUST gate its git-sync step on "
...
def test_command_git_sync_step_is_default_off():
    assert "default off" in _read(COMMAND).lower(), (
        "project-migrate.md's git-sync step MUST state it is default off"
    )


def test_command_git_sync_uses_structured_message_shape():
    """The non-thread adaptation keeps the ``anvil(project-migrate/``
    prefix fixed while the version token becomes the project slug."""
    text = _read(COMMAND)
    assert "anvil(project-migrate/" in text
    assert "<project> [MIGRATED]" in text


def test_command_git_sync_is_apply_path_only():
    """Dry-run / --report modes write nothing — the hook must scope
    itself to the --apply path."""
    text = _read(COMMAND)
    idx = text.find("## Git sync")
    assert idx >= 0, "project-migrate.md missing the Git sync section"
...
def test_command_uses_short_pointer_shared_contract():
    ...
    assert "stage only the dirs this phase wrote" in section, (
        "project-migrate.md's git-sync step MUST carry the canonical "
        "short-pointer shared-contract sentence"
    )
    assert "Git failures warn and continue" in section, (
        "project-migrate.md's git-sync step MUST keep the warn-and-continue "
        "clause"
    )
    assert "emit a one-line warning and continue" not in section, (
        "project-migrate.md still carries the verbose pre-#528 failure prose"
    )
    assert "byte-identical to a pre-#426 install" not in section, (
        "project-migrate.md still carries the verbose pre-#528 default-off prose"
    )
...
def test_command_keeps_per_command_mechanics():
    ...
    assert "Staging target" in section, (
        "project-migrate.md's git-sync step MUST keep an inline staging target"
    )
    assert "Ordering" in section, (
        "project-migrate.md's git-sync step MUST keep an inline ordering anchor"
    )
