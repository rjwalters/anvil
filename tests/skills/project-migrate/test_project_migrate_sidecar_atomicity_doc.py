"""Doc-coverage guard for project-migrate.md staged-sidecar wiring (issues #350, #645).
...
def test_project_migrate_doc_names_required_files_manifest():
    text = _read()
    for name in ("review.md", "_review.json", "_meta.json"):
        assert name in text, f"required file {name!r} missing from prose"


def test_project_migrate_doc_invokes_cleanup_sweep():
    text = _read()
    assert "cleanup_one_staging" in text or "sidecar cleanup" in text


def test_project_migrate_doc_describes_atomic_rename_contract():
    text = _read()
    assert "atomically" in text.lower() or "atomic" in text.lower()
    # project-migrate uses the distinctive "moved aside" staged/backup/swap
    # model (not the `.tmp` leading-dot staging shape of the critic docs), so
    # the atomicity vocabulary here is the moved-aside backup + swap.
    assert "moved aside" in text or ".bak" in text or ".staging" in text


def test_project_migrate_doc_references_adopt_review_issues():
    """This doc's staged_sidecar sites are the Phase 3a (#454) / Phase 3b
    (#507) adopt-review apply steps, not the generic critic-review path, so it
    references those issues rather than #350."""
    text = _read()
    assert "#454" in text
    assert "#645" in text
