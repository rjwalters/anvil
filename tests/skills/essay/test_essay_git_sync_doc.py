"""Doc-coverage guard for the essay skill's adoption of the per-phase
git commit/sync hook (issue #426; rolled out skill-wide in issue #436;
essay ships with it from birth per the #460 curation).

The hook contract lives in ``anvil/lib/snippets/git_sync.md``; this file
pins the essay-skill adoption so the conditional git-sync final step
can't silently drift out of a command file in a later edit.
...
named ``test_essay_git_sync_doc.py``.
"""
...
SKILL_MD = REPO_ROOT / "anvil" / "skills" / "essay" / "SKILL.md"

# The 3 write-bearing essay commands per the #460 curation (the v1
# command set is deliberately draft/review/revise/status only). The
# portfolio orchestrator (essay.md) is read-only and exempt by
# definition.
...
def test_command_references_git_sync_snippet(command: str):
    """Every write-bearing essay command MUST reference the snippet."""
    text = _read(COMMANDS / command)
...
def test_command_names_commit_per_phase_knob(command: str):
    """Every write-bearing essay command MUST name the conditional knob."""
    text = _read(COMMANDS / command)
...
def test_command_git_sync_uses_structured_message_shape(command: str):
    ...
    ``anvil(essay/<phase>):`` so the orchestrator-facing shape can't
    drift per-command."""
    text = _read(COMMANDS / command)
    assert "anvil(essay/" in text, (
        f"{command}'s git-sync step MUST use the anvil(essay/<phase>): "
...
def test_command_uses_short_pointer_shared_contract(command: str):
    ...
    contract sentence ("stage only the dirs this phase wrote") and the
    warn-and-continue clause are present, while the old verbose
    boilerplate ("emit a one-line warning", "byte-identical") is gone."""
