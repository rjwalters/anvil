"""Doc-coverage guard for essay-review.md staged-sidecar wiring (issues #350, #645).
...
def test_essay_review_doc_invokes_cleanup_sweep():
    text = _read()
    assert "cleanup_one_staging" in text or "sidecar cleanup" in text


def test_essay_review_doc_describes_atomic_rename_contract():
    text = _read()
