"""Doc-coverage guard for ip-uspto-pre-flight staged-sidecar wiring
(issue #350).
"""
...
def test_ip_uspto_pre_flight_doc_names_required_files_manifest():
    text = _read()
...
def test_ip_uspto_pre_flight_doc_invokes_cleanup_sweep():
    text = _read()
    assert "cleanup_one_staging" in text


def test_ip_uspto_pre_flight_doc_describes_atomic_rename_contract():
    text = _read()
    assert "atomically" in text.lower() or "atomic" in text.lower()
    assert "leading-dot" in text or ".tmp" in text


def test_ip_uspto_pre_flight_doc_references_issue_350():
    text = _read()
    assert "#350" in text


def test_ip_uspto_pre_flight_doc_documents_non_python_driver_fallback():
