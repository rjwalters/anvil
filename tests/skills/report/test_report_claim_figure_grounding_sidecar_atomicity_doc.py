"""Doc-coverage guard for report-claim-figure-grounding staged-sidecar wiring
(issue #350).
"""
...
def test_report_cfg_doc_references_staged_sidecar_primitive():
    text = _read()
    assert "anvil/lib/sidecar.py" in text
    assert "staged_sidecar" in text


def test_report_cfg_doc_names_required_files_manifest():
    text = _read()
    for name in ("_review.json", "_findings.json"):
        assert name in text, f"required file {name!r} missing from prose"


def test_report_cfg_doc_invokes_cleanup_sweep():
    text = _read()
    assert "cleanup_one_staging" in text


def test_report_cfg_doc_describes_atomic_rename_contract():
    text = _read()
    assert "atomically" in text.lower() or "atomic" in text.lower()
    assert "leading-dot" in text or ".tmp" in text


def test_report_cfg_doc_references_issue_350():
    text = _read()
    assert "#350" in text


def test_report_cfg_doc_documents_non_python_driver_fallback():
