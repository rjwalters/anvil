"""Doc-coverage guard for the report-review `--rescore-mode` wiring (issue #368).
...
Report is the customer-facing skill with the highest advance threshold
(≥39/44); its transition subsection lands in `verdict.md` (NOT a separate
`findings.md` — same shape as `installation-review.md`).

Per the per-skill test filename convention (#58), this file is named
``test_report_review_rescore_mode_doc.py`` to avoid collision.
"""
...
def test_report_review_doc_references_rescore_mode_flag():
    """The literal `--rescore-mode` token must appear in the command file body."""
    text = _read(REPORT_REVIEW_DOC)
    assert "--rescore-mode" in text, (
        "report-review.md must document the `--rescore-mode` flag "
        "(issue #368) — the marker `anvil:rubric-rebackport`'s "
        "`check_rescore_hook('report')` scans for the literal token."
    )


def test_report_review_doc_describes_rescore_sidecar_routing():
    """The prose must explicitly name the rescore sidecar path shape."""
    text = _read(REPORT_REVIEW_DOC)
    assert "review.rescore-" in text, (
        "report-review.md must document re-routing the staged_sidecar "
        "output to the `<thread>.{N}.review.rescore-<id>/` path shape "
        "(issue #368)."
    )


def test_report_review_doc_describes_rescore_state_completed_stamp():
    """The prose must explicitly state `rescore_state: \"completed\"` is written."""
    text = _read(REPORT_REVIEW_DOC)
    assert "rescore_state" in text and "completed" in text, (
        "report-review.md must document stamping `rescore_state: "
        "\"completed\"` on `_meta.json` (overwriting the rebackport "
        "tool's `\"scheduled\"` placeholder; issue #368)."
    )


def test_report_review_doc_describes_prior_rubric_lookup_at_n_not_n_minus_one():
    ...
    assert "N-1" in text or "N - 1" in text, (
        "report-review.md must explicitly contrast the rescore-mode prior-"
    ...
    assert "<thread>.{N}.review/" in rescore_slice, (
        "report-review.md's `--rescore-mode` block must explicitly name "
        "the `<thread>.{N}.review/_meta.json` target for the prior-rubric "
        "lookup under rescore mode (issue #368)."
    )


def test_report_review_doc_references_issue_368():
    """Audit-trail: the issue number must appear in the doc for traceability."""
    text = _read(REPORT_REVIEW_DOC)
    assert "#368" in text, (
        "report-review.md must reference issue #368 in the `--rescore-mode` "
