"""Doc-coverage guard for ip-uspto-provisional-112.md staged-sidecar wiring (issues #350, #645).
...
def test_ip_uspto_provisional_112_doc_references_staged_sidecar_primitive():
    text = _read()
    assert "anvil/lib/sidecar.py" in text
    assert "staged_sidecar" in text


def test_ip_uspto_provisional_112_doc_names_required_files_manifest():
    text = _read()
...
def test_ip_uspto_provisional_112_doc_invokes_cleanup_sweep():
    text = _read()
    assert "cleanup_one_staging" in text or "sidecar cleanup" in text


def test_ip_uspto_provisional_112_doc_describes_atomic_rename_contract():
    text = _read()
