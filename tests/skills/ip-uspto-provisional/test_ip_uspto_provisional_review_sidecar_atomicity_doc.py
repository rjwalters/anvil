"""Doc-coverage guard for ip-uspto-provisional-review.md staged-sidecar wiring (issues #350, #645).
...
DOC = REPO_ROOT / "anvil" / "skills" / "ip-uspto-provisional" / "commands" / "ip-uspto-provisional-review.md"
...
def test_ip_uspto_provisional_review_doc_references_staged_sidecar_primitive():
    text = _read()
    assert "anvil/lib/sidecar.py" in text
    assert "staged_sidecar" in text


def test_ip_uspto_provisional_review_doc_names_required_files_manifest():
    text = _read()
...
def test_ip_uspto_provisional_review_doc_invokes_cleanup_sweep():
    text = _read()
    assert "cleanup_one_staging" in text or "sidecar cleanup" in text


def test_ip_uspto_provisional_review_doc_describes_atomic_rename_contract():
    text = _read()
    assert "atomically" in text.lower() or "atomic" in text.lower()
    assert "leading-dot" in text or ".tmp" in text


def test_ip_uspto_provisional_review_doc_references_issue_350():
    text = _read()
    assert "#350" in text

def test_ip_uspto_provisional_review_doc_documents_non_python_driver_fallback():
