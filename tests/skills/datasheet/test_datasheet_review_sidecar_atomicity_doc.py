"""Doc-coverage guard for datasheet-review.md staged-sidecar wiring (issues #350, #645).
...
DOC = REPO_ROOT / "anvil" / "skills" / "datasheet" / "commands" / "datasheet-review.md"
...
def test_datasheet_review_doc_references_staged_sidecar_primitive():
    text = _read()
...
def test_datasheet_review_doc_invokes_cleanup_sweep():
    text = _read()
    assert "cleanup_one_staging" in text or "sidecar cleanup" in text


def test_datasheet_review_doc_describes_atomic_rename_contract():
    text = _read()
