"""Doc-coverage guard for the datasheet skill's adoption of the per-phase
...
pins the datasheet-skill adoption (the #350-style phased rollout: snippet +
...
named ``test_datasheet_git_sync_doc.py``.
"""
...
COMMANDS = REPO_ROOT / "anvil" / "skills" / "datasheet" / "commands"
SKILL_MD = REPO_ROOT / "anvil" / "skills" / "datasheet" / "SKILL.md"

# The 5 write-bearing datasheet commands per the issue #436 curation
# inventory. The portfolio orchestrator (datasheet.md) is read-only and
...
def test_command_references_git_sync_snippet(command: str):
    """Every write-bearing datasheet command MUST reference the snippet."""
...
def test_command_names_commit_per_phase_knob(command: str):
    """Every write-bearing datasheet command MUST name the conditional knob."""
    text = _read(COMMANDS / command)
...
def test_command_git_sync_uses_structured_message_shape(command: str):
    ...
    ``anvil(datasheet/<phase>):`` so the orchestrator-facing shape can't
    drift per-command."""
    text = _read(COMMANDS / command)
    assert "anvil(datasheet/" in text, (
        f"{command}'s git-sync step MUST use the anvil(datasheet/<phase>): "
