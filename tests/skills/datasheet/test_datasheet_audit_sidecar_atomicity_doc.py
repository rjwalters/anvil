"""Doc-coverage guard for datasheet-audit.md staged-sidecar wiring (issues #350, #645).
...
def test_datasheet_audit_doc_names_required_files_manifest():
    text = _read()
    for name in ("verdict.md", "findings.md", "evidence.md", "_meta.json", "_progress.json"):
        assert name in text, f"required file {name!r} missing from prose"


def test_datasheet_audit_doc_invokes_cleanup_sweep():
    text = _read()
    assert "cleanup_one_staging" in text or "sidecar cleanup" in text


def test_datasheet_audit_doc_describes_atomic_rename_contract():
    text = _read()
    assert "atomically" in text.lower() or "atomic" in text.lower()
    assert "leading-dot" in text or ".tmp" in text


def test_datasheet_audit_doc_references_issue_350():
    text = _read()
    assert "#350" in text

def test_datasheet_audit_doc_documents_non_python_driver_fallback():
