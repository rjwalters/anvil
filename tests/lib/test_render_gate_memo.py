...
def memo_version_dir(tmp_path):
    ...
    Body filename echoes the thread slug per #295 — for a ``foundry``
    thread the body is ``foundry.md`` inside ``foundry/foundry.1/``.
    """
    thread_root = tmp_path / "foundry"
    thread_root.mkdir()
    vd = thread_root / "foundry.1"
    vd.mkdir()
    (vd / "foundry.md").write_text(
        "# Investment memo\n\n"
        "## Recommendation\n\n"
        "Pass.\n\n"
        "## Thesis\n\n"
...
def test_select_memo_engine_honors_requested_xelatex_over_weasyprint(monkeypatch):
    ...
    canary's foundry / smart-actuators case where the per-document knob
...
def test_render_memo_source_threads_requested_engine(monkeypatch, memo_version_dir):
    ...
    out_pdf = memo_version_dir / "foundry.pdf"
...
def test_render_memo_source_consumer_template_xelatex(
    ...
    # tmp_path/foundry/foundry.1/ per the fixture).
...
def test_render_memo_source_metadata_flags_and_version_expansion(
    ...
    from the ``<slug>.{N}`` version-dir name (``foundry.1`` → 1)."""
    _all_engines_available(monkeypatch)
    captured = _capture_pandoc_cmd(monkeypatch)
    ...
    # {N} expanded to the fixture's version number (foundry.1).
...
def test_render_memo_source_absent_knobs_byte_identical(
    ...
    expected = [
        "pandoc",
        str(memo_version_dir / "foundry.md"),
        "-o",
        str(out_pdf),
...
def test_resolve_target_length_override_changes_derived_range():
    ...
    (the issue's meridian-synthesis reproducer numbers)."""
...
def test_gate_memo_image_refs_broken(
    ...
    memo_md = memo_version_dir / "foundry.md"
    memo_md.write_text(
...
def test_gate_memo_to_review_emits_critical_flags(
    ...
    rev = r.to_review(version_dir="foundry.1", critic_id="memo-gate")
...
def test_gate_memo_internal_smoke(monkeypatch, memo_version_dir, fake_pdfinfo_path):
    ...
    assert r.passed is True
    assert r.pages == 3
    # Default out_pdf is <version_dir>/<slug>.pdf — for the ``foundry``
    # thread fixture the basename is ``foundry.pdf`` per #295.
    assert r.pdf_path.endswith("foundry.pdf")
