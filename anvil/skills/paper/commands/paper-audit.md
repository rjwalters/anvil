...
6b. **Pending-marker terminal gate (deterministic, unconditional) — issue #841**: invoke `uv run --project .anvil python -m anvil.lib.pending_marker <thread>.{N}/ --write-review --blocking` (same tool and same invocation shape as `paper-review` step 4g; auto-detects `main.tex`, `--body <path>` override for adopted-in-place legacy threads). This re-runs the deterministic `[PENDING <source>]` marker gate as the **terminal check** before `AUDITED` — genuinely useful independent of step 4g when the auditor runs on a draft that has not yet been reviewed (see this file's Inputs section: "acceptable ... to run the auditor earlier"), and as defense-in-depth when it has. `--write-review` writes (overwrites, per the deterministic-regeneration carve-out documented in the module) `<thread>.{N}.pending/_review.json`; `--blocking` emits the `critical_pending_marker:unresolved_pending_marker` flag whenever any well-formed marker remains, forcing `Verdict.BLOCK` through the standard `discover_critics` → `aggregate` → `compute_verdict` path — no lib change needed (same wiring shape as the `.corpus-audit/` fabrication-class flags in step 5b). A well-formed marker is an honest disclosure, not a defect (see `rubric.md`) — do NOT add a corresponding entry to `numerical-audit.md`'s mismatch table; note any outstanding marker in `flags.md`'s critical-flags list instead (see the example below), naming the source label(s) from the tool's `outstanding_sources` output. **Clean result** (no markers in the body): sidecar still writes with empty findings/flags, byte-identical in effect to a pre-#841 audit. **Tooling-absent fail-open**: mirrors step 4 toolchain-absent handling — when `uv`/the module is unavailable, emit a stdout notice and manually scan for the literal `[PENDING <source>]` / `[PENDING: <source>]` shape (a source-less `[PENDING]` is malformed, not a marker), fold any found marker into `flags.md` exactly as the automated flag would, and record the fallback with a one-line note.
...
8. **Write `flags.md`**: a markdown list of critical flags, each with one-paragraph justification and the specific evidence (line numbers, table references, log excerpts) needed for the reviser to act.
   ...
   - **Unresolved pending marker** (`[PENDING benchmark-run-2024-11]`, Sec. 5): a well-formed pending marker remains in the body — an honest disclosure, not a defect, but the artifact cannot reach AUDITED until it is replaced with its real value. See `<thread>.{N}.pending/_review.json`.
