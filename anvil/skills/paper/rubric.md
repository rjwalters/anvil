...
| 9 | **Rhetorical economy** | 4 | Is every paragraph load-bearing? Could the same argument land in fewer words? Are the most important claims surfaced early? Is hedging proportional to genuine uncertainty, not used as a cushion? Could a busy reviewer extract the contribution in 90 seconds? The **self-flattering virtue-signaling adjective** class (see dim 7's default AI-tell check — *honest/candid/frank/uncomfortable X* that labels a move instead of making it) is also a rhetorical-economy failure: the label is words the argument does not need. Preserve the same semantic-work exception (a contrastive adjective that does real work is load-bearing, not a tell). |
...
- **Unresolved pending marker** (issue #841) — A well-formed `[PENDING <source>]` placeholder (see `anvil/lib/snippets/pending_marker.md`) remains in the body. Unlike the other flags in this list, this one is **deterministic, not a reviewer judgment call**: `paper-review` step 4g runs `anvil/lib/pending_marker.py` unconditionally in blocking mode, which raises a `critical_pending_marker:unresolved_pending_marker`-typed flag whenever any marker is still present — the reviewer does not need to notice the marker text and decide to flag it by hand. A well-formed marker is an **honest disclosure of a genuinely outstanding value, not a defect** — see the "Pending-measurement markers do not incur a dimension penalty" note above; do NOT also deduct dimension score for it. The flag exists so the paper cannot reach `READY`/`AUDITED` with a known gap still open; it clears automatically once the marker is replaced with its real value and the next review pass finds nothing.
