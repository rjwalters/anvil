# Northwind Pitch Strategy — Engagement Plan

## §1 — Recommendation

Engage Northwind as the lead Gen 1 customer for the Pericles platform.
...
The strategic frame for the Northwind conversation is that the
high-performance compute market is bifurcating, and Northwind and the
...
`meridian-synthesis/memo.2 §3.1`, which lays out the substrate
divergence and the three load-bearing failure modes (thermal,
radiation, integrity). The Northwind pitch lands the same framing
without re-deriving it — the meridian-synthesis memo IS the substrate
argument.
...
substrate that tells Northwind which compute blocks merit the Gen 2
...
Northwind's procurement cycle is the dominant calendar risk. The data-
center disagreement framing in `meridian-synthesis/memo.2 §3.1` is
load-bearing for the strategic frame — if Northwind does NOT share the
substrate-divergence view (which the meridian-synthesis analysis
