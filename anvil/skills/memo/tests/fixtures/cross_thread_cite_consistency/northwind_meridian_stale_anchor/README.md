# Fixture: northwind-pitch-strategy memo.1 stale meridian-synthesis §3.1 cite

**Studio canary date**: 2026-06-02
**Source threads**: `northwind-pitch-strategy memo.1` (citing memo) + `meridian-synthesis memo.1 → memo.2` (cited memo, where the section moved)
**Propagation arc**: cite landed in northwind-pitch-strategy memo.1; the meridian-synthesis revision (memo.1 → memo.2) moved the data-center disagreement framing from §5.4 → §5.2 and the §3.1 reference in northwind-pitch became stale; reviewer caught it manually; corrected to §5.2 in northwind-pitch memo.2.
...
2. **Phase B detector regression anchor**: when a future Phase B issue lands a `anvil/skills/memo/lib/cross_thread_cite.py` detector, this fixture is the regression-test anchor — "did the detector still catch the stale §3.1 anchor in meridian_synthesis.2?" The fixture is intentionally minimal (two cite occurrences in one citing memo, one cited memo with the matching framing at §5.2 instead of §3.1) so the regression target is unambiguous.
...
- `citing_memo.md` — minimal synthesized memo with the canary cite (`meridian-synthesis/memo.2 §3.1` cited twice, once in §2 paragraph 2 as the framing source and once in §4 paragraph 1 as the risk anchor). The cite is verbatim from the issue body's canary report.
- `cited_thread/meridian_synthesis.2/memo.md` — minimal synthesized cited memo where §3.1 does NOT exist (the cited memo's §3 is "Programming-model commonality" with no subsections) and §5.2 carries the matching data-center disagreement framing. The §1 / §2 reorganization context names the memo.1 → memo.2 reorganization that produced the stale-anchor failure.
...
|---|---|---|---|---|
| `meridian-synthesis/memo.2 §3.1` | §2 paragraph 2 | `cited_thread/meridian_synthesis.2/memo.md` | `§3.1` | **ANCHOR-MISSING-BUT-THREAD-PRESENT** (important) — §3.1 is not present in the cited memo; the disagreement framing now lives at §5.2 |
| `meridian-synthesis/memo.2 §3.1` | §4 paragraph 1 | `cited_thread/meridian_synthesis.2/memo.md` | `§3.1` | **ANCHOR-MISSING-BUT-THREAD-PRESENT** (important) — same stale anchor; per-instance counting (deduction is -1 per occurrence) |
...
Why missing-anchor and not contradicted: the meridian-synthesis §5.2 framing is consistent with the framing the northwind-pitch memo attributes to it — the anchor moved (memo.1 → memo.2 reorganization) but the substance did not. If the substance had also changed (e.g., the meridian-synthesis revision had reversed the disagreement framing or dropped the substrate-divergence argument entirely), the verdict would have escalated to `ANCHOR-CONTRADICTED` and the critical flag would have fired.
...
- `anvil/skills/memo/tests/fixtures/summary_detail_consistency/northwind_gen_attribution/` — the structural template fixture this fixture mirrors.
