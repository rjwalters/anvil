...
**Phase A ships as reviewer-prose discipline** (this sub-rule) — no Python module, following the summary↔detail (#245) and cross-thread (#236) precedents. A Phase B detector at `anvil/skills/memo/lib/skeleton_derivation.py` is a deferred follow-on gated on canary signal; the fixture under `tests/fixtures/skeleton_derivation/praetor_word_soup/` carries the expected-block shape as the regression anchor on landing.
