...
The dim 3 justification MUST cite the specific verdict and the refs-document path (e.g., "Back-checked 'Robb Walters: 15+ year Sphere Staff Scientist tenure' against `refs/cv.pdf`: CONTRADICTED ('Acme Semi, Palo Alto CA, 2026-current') — -2 + critical flag"). For portfolio-level evidence pool hits (issue #280), the path surfaces the layer — e.g., "Back-checked 'industry comp ranges 18-25% gross margin' against `research/comps/silicon-comp-matrix.md`: VERIFIED — no deduction". Vague "needs refs back-check" deductions without named instances are not actionable for the reviser and SHOULD be avoided — same standard as §"Citation hooks (dim 3)".
...
**Calibration crosscheck (ALWAYS emitted; two modes — issue #913).** The red-team ALWAYS writes a `calibration.md` block in its sibling dir comparing its independently-generated objection set against an anticipated set. Which anticipated set depends on whether an author-supplied strongman exists:

- **`strongman-crosscheck` mode** — when `refs/strongman-against.md` is present in the resolved refs-dir list, the author's strongman becomes a check on the red-team:
...
- **`self-derived` mode** — when no `strongman-against.md` resolves anywhere in the resolved refs-dir list (the common case: a reviser cannot honestly author a strongman against its own memo, so most threads never accumulate one), the memo's own risk register (dim 4 *Risk honesty*), named concessions/caveats, and self-stated kill-criteria substitute for the missing author-supplied anticipated set:
  - **Anticipated** — red-team objections that overlap with something the memo's own self-critique already names.
  - **Novel** — red-team objections with no counterpart in the memo's own self-critique. This is the STRONGER blind-spot signal of the two modes — a novel objection here evaded not just the author's imagination but the author's own generous self-accounting.

  `calibration.md` MUST state its mode and its bias explicitly at the top of the file: a self-derived anticipated set is generous to the author by construction, since it can only ever contain objections the author already wrote down; it cannot measure the blind spot that `strongman-crosscheck` mode measures. `_meta.json` records `calibration_mode: "strongman-crosscheck" | "self-derived"` alongside the existing `strongman_crosscheck_present` boolean so downstream consumers can weight the two modes differently.

The calibration block is **operator-facing audit-trail only** in both modes — it does NOT contribute findings or critical flags to `_review.json`. The existing §"Strongman back-check (dim 3)" sub-rule above continues to function as designed for `strongman-crosscheck` mode; `self-derived` mode has no analog in `memo-review` and is a red-team-only signal. The self-authored strongman (when present) becomes a **calibration signal on the author's adversarial imagination**, not the sole source of adversarial input; when absent, the memo's own self-critique plays that role instead, with the documented weaker-signal caveat.
...
### Comprehension back-check (findings-only, no dimension)

The `memo-comprehension` critic (`commands/memo-comprehension.md`, issue #753) is a **cold-reader comprehension critic**: a blind reader that reads ONLY the memo body + `exhibits/` (no BRIEF, no research, no rubric, no skeleton, no prior review), answers a fixed 7-question questionnaire in its own words, and has each reconstruction diffed against the intended message. It exists because every other phase in the memo lifecycle is an atom-level or compliance-level check the drafter can co-optimize — the reviewer verifies each claim and scores each dimension, all surface features a fluent drafter can satisfy while the document as a whole fails to *transmit*. The canary failure that motivated it: a memo scoring 43/44 (`advance: true`, 0 critical, dims 8/9 at 4/4, 22/22 refs verified) that the operator judged "almost word-soup bad ... mimicking the prosody of a report but not having a clear understanding of what we are trying to communicate." Comprehension is a property of the whole; blindness is the instrument that measures it, because a reviewer who has read the BRIEF cannot un-know what the memo means (curse of knowledge, mechanized).

**This sub-rule adds NO rubric dimension, NO score, and NO gate.** It is documented here for its **verdict vocabulary** only. The comprehension critic is **findings-only**, the same posture as the optional `.audit/` / `.critic/` siblings: it owns no dimension, its `_review.json` carries all-`null` scores (contributing nothing to any dimension's mean-of-non-null merged score per `anvil/lib/critics.py::aggregate`), and its `critical_flags` list is **always empty**. It can never move the /44 total or the verdict — it is provably non-perturbing. Its findings surface in `<thread>.{N}.comprehension/comments.md` at `major` severity and are consumed by `memo-revise` through the existing generic critic-sibling enumeration with zero code change.

The comprehension critic classifies each of the five reconstruction questions (what does this company sell / who buys it / why does this team win / what is the ask / what kills it) with ONE of four verdicts:

- **`CLEAR`** — the blind reader recovered what was intended. The message transmitted. **No finding.**
- **`GARBLED`** — the answer exists in the body but the blind reader got it **wrong** (present but not transmitting — buried, hedged into vapor, fogged by mechanism language). **Finding at `major`.**
- **`MISSING`** — the question is unanswerable from the body, AND the intent says it should be answerable. The document failed to say the thing it meant to say. **Finding at `major`.**
- **`HONEST-GAP`** — the question is unanswerable because the answer is **genuinely unresolved**, AND the body **says so plainly**. **This is NOT a defect. No finding.** This verdict is load-bearing for early-stage artifacts: a concept-stage memo legitimately cannot answer "what do we sell yet" — the critic verifies the memo states that gap in plain words rather than fogging it with mechanism language. A memo that plainly says "the product does not yet exist; this raise funds the prototype" earns `HONEST-GAP` on "what does this company sell", not `MISSING`. The whole reason the vocabulary carries a fourth verdict is that without it a blind critic would penalize an honest early-stage artifact for stating what it does not yet know — pushing drafters back toward the exact mechanism-fog this critic exists to catch. The `MISSING` vs. `HONEST-GAP` boundary is precisely whether the document is **honestly silent** (gap acknowledged in plain words → `HONEST-GAP`) or **failing to communicate** (gap unacknowledged, or fogged behind mechanism language the reader mistook for an answer → `MISSING`).

The questionnaire's remaining two questions feed non-scored channels: undefined load-bearing terms become a `major` jargon list (define at first use or delete), and missing-context items become advisory `scope: expand` enrichment candidates (not must-fix).

See `commands/memo-comprehension.md` for the full critic spec (blind-read invocation contract, the seven-question questionnaire, the three-phase procedure, and the `_review.json` / `answers.md` / `verdicts.md` / `comments.md` output shapes).
...
This sub-rule formalizes the 2026-06-05 studio canary's `.enrich/` smart-actuators hack as standing rubric guidance — the canary demonstrated that the standard judgment-class reviewer, given explicit enrichment scope, produces `comments.md` enrichment output indistinguishable in shape from a dedicated detector. The hack ran three parallel subagents per document over 8 smart-actuators threads in noncanonical `<thread>.{N}.enrich/` critic siblings; this subsection brings the same output into the canonical `<thread>.{N}.review/comments.md` stream the reviser already consumes, with **zero schema delta** and **no new critic-sibling type**. See Epic #328 (reframed 2026-06-05) Track A for the framing rationale; #333 ships this sub-rule.
...
**Validation step.** The canary author offered `.enrich/` smart-actuators fixtures in the Epic #328 comment thread. When those fixtures land in `tests/skills/memo/fixtures/enrichment_canary/` (or a comparable location), they serve as the Phase B regression anchor in the same way `tests/fixtures/summary_detail_consistency/northwind_gen_attribution/` anchors the summary-detail Phase B detector. Fixture validation is a follow-on to this sub-rule's landing — the rubric guidance ships first; the fixtures land when available.
...
- **`CONTRADICTED`** — detail section contradicts the summary on a load-bearing noun / number / quarter / actor. Severity **always `critical`** — this is the canary failure mode the contract exists to catch. The Northwind-pitch Gen-1/Gen-2/Gen-3 attribution swap is the worked-example anchor (see `tests/fixtures/summary_detail_consistency/northwind_gen_attribution/`).
...
**Phase B (deferred, optional).** An automated detector at `anvil/skills/memo/lib/summary_detail.py` is a Phase B follow-on, gated on canary signal. The canary-anchor fixture under `tests/fixtures/summary_detail_consistency/northwind_gen_attribution/` carries the expected-block shape so Phase B's detector has a regression anchor on landing. Promotion from skill-local to `anvil/lib/` is a Phase C decision gated on a deck-side or paper-side second consumer per CLAUDE.md §"Skill-local first, lib promotion later".

### Skeleton↔section derivation leg (issue #752)

**A third leg of the intra-memo consistency check.** Anvil documents are drafted body-first — prose is written, then summarized, then scored claim-by-claim — so nothing owns the document's *communicative intent* as a first-class artifact, and a memo can pass every atom-level check while failing to transmit its plan. `memo-draft` step 5f closes that gap on the producer side: the drafter authors `<thread>.{N}/skeleton.md` (a recursive claim tree — one root claim + one claim per section — see `commands/memo-draft.md` §"Author the skeleton before the body") BEFORE the body, and the body is an expansion of it. This sub-rule closes the loop on the reviewer side: it back-checks whether the body actually delivers the skeleton it was supposed to expand.

**Why the existing legs can't catch this.** The summary↔detail leg above checks whether the memo's *own summary blocks* match its detail. But a memo whose summary and detail agree can still fail to transmit a coherent plan — the whole reads as "word-soup" (true, sourced, hedged atoms that do not add up to a stated intent) precisely because there was no stated intent to derive from. The canary anchor is the sentinel-thread memo that scored 43/44 (advance:true, 0 critical) yet the operator rejected as "not having a clear understanding of what we are trying to communicate." The skeleton is the missing referent; the derivation leg is the check that the body is answerable to it.

**Same verdict tags, same severity ladder as the summary↔detail leg.** For each `(skeleton claim, body section)` pair the reviewer classifies the relationship as one of:

- **`MATCH`** — the body section lands the section's skeleton claim (and, for the root claim, the body as a whole delivers it). No finding emitted.
- **`ABSENT`** — a skeleton claim that no body section elaborates. Severity typically `important`; `critical` when the ABSENT claim is the skeleton's **root claim** (the body never delivers the one thing the reader was supposed to take away — the exact word-soup failure mode; analogous to ABSENT-thesis on the summary↔detail leg).
- **`CONTRADICTED`** — a body section contradicts its skeleton claim on a load-bearing noun / number / actor, OR the body's actual thesis contradicts the skeleton's root claim. Severity **always `critical`** (same as the summary↔detail leg).
- **`DIVERGENT`** — the body section and its skeleton claim are technically compatible but the body has drifted in framing a sophisticated reader would notice. Severity typically `suggestion`; `important` when the drift shifts the recommendation.

The critical-flag integration is identical to the summary↔detail leg: a `CONTRADICTED` finding at `critical` severity (or an `ABSENT` root claim at `critical`) is a **critical-flag candidate** under the open-ended "any deal-breaker a sophisticated reader would catch" slot, surfaces in `verdict.md` as `Skeleton derivation: CONTRADICTED` (or `Skeleton derivation: ABSENT (root claim)`), and forces `advance: false` via the existing "no critical flags" clause — NOT a new gate. `ABSENT` (non-root) and `DIVERGENT` findings at `important` / `suggestion` severity are observational only.

**The plain-language / jargon check.** The skeleton contract (`commands/memo-draft.md` step 5f) forbids any term in the skeleton that a domain-literate outsider could not parse unless the skeleton defines it. The reviewer additionally checks the inverse on the body: an **undefined coinage that load-bears a body section but never appears (defined) in the skeleton** ("harness-native efficiency", "the substrate under the primitive", "the escalation floor is invariant", a variable defined once in a subordinate clause that then load-bears an economics section) is a **`DIVERGENT` finding at `important` severity** — the body introduced load-bearing jargon the intent artifact never sanctioned. This is the reviewer-side complement to the drafter's step-5f jargon discipline; it does NOT raise a critical flag on its own (it is a coherence-drift signal, surfaced for the reviser to fold back into the skeleton on the next pass).

**Backwards-compat — absence is observational, never a deduction (load-bearing).** When the version dir has **no `skeleton.md`** (every memo drafted before issue #752, and any thread whose operator opts out), this leg records `ran: false` with `reason: "no skeleton.md in version dir"` and emits **no findings and no deduction**. The reviewer notes the absence observationally so the operator sees the skeleton-first discipline is available; it never blocks advance and never costs score. This matches the `ran: false` explicit-skip convention of the summary↔detail and cross-thread legs and preserves byte-identical behavior for pre-#752 threads. The skeleton is opt-in producer discipline; the reviewer never penalizes its absence.

**Phase A ships as reviewer-prose discipline** (this sub-rule) — no Python module, following the summary↔detail (#245) and cross-thread (#236) precedents. A Phase B detector at `anvil/skills/memo/lib/skeleton_derivation.py` is a deferred follow-on gated on canary signal; the fixture under `tests/fixtures/skeleton_derivation/sentinel_word_soup/` carries the expected-block shape as the regression anchor on landing.

### Related (composition with other back-checks)

This is the **intra-memo** back-check; the related cross-thread back-check (#236) is the **cross-thread** analog. Both share the verdict-tag pattern (4-valued for #236's refs analog; 3-valued for this issue's summary-detail variant). When both ship, the framework covers the **back-check triangle** — and the issue #752 skeleton↔section derivation leg adds an intent-derivation dimension to the intra-memo leg:

1. **memo A claim ↔ memo A `refs/`** — existing §"Refs back-check (dim 3)" above (PR #144).
2. **memo A callout ↔ memo A §N** — this sub-rule (intra-memo summary-detail consistency), now with the §"Skeleton↔section derivation leg (issue #752)" companion (memo A `skeleton.md` claim ↔ memo A §N).
...
**Why this matters.** As consumers accumulate portfolios of interlinked anvil threads, each revision of any thread can invalidate cites *from other threads* that point at section headers that get renamed, moved, or dropped. The Studio canary (2026-06-02) caught this manually: `northwind-pitch-strategy memo.1 §2` cited `meridian-synthesis/memo.2 §3.1` as the location of the data-center disagreement framing, but when `meridian-synthesis` revised from memo.1 to memo.2, the framing moved to §5.2 and §3.1 became stale. A less-thorough reviewer would have scored the dim 3 evidence claim cleanly and the stale cite would have propagated to v3+. See `tests/fixtures/cross_thread_cite_consistency/northwind_meridian_stale_anchor/` for the canary anchor.
...
- **Literal-path cites**: `<thread-slug>/<thread-slug>.<N>/<thread-slug>.md` (e.g., `meridian-synthesis/meridian-synthesis.2/meridian-synthesis.md` — body filename echoes the slug per #295).
- **Short-form cites**: `<thread-slug> §X` (e.g., `meridian-synthesis §3.1`).
- **Relative-path cites** (the studio convention): `output/<thread-slug>/...` (e.g., `output/meridian-synthesis/meridian-synthesis.2/meridian-synthesis.md`).
- **Backtick-wrapped cites**: `` `<thread-slug>/<thread-slug>.<N>/<thread-slug>.md` §<X> `` (e.g., `` `meridian-synthesis/meridian-synthesis.2/meridian-synthesis.md` §5.2 ``).
...
2. **Cited version** → the highest-`N` version under the cited thread's portfolio entry. **Pinning to a specific cited version** (e.g., `meridian-synthesis.2`) is a **stronger contract** and the reviewer gives it a **positive note** in the dim 3 justification, NOT a deduction. The version pin says "this cite is intentional against a specific historical state and should not be re-resolved."
...
### Plain-language framing (issue #1042/#1044)

Dim 9's anti-pattern list above targets padding, not patient explanation. Per `anvil/lib/snippets/plain_language.md` (the plain-language-first framework — issue #1042/#1043) and `anvil/lib/snippets/rubric.md` §"Dim 9 — teaching-oriented length is not padding", a passage that states an idea in ordinary language before naming it, or that keeps a plain restatement alongside a newly-introduced technical term, is doing the memo's job — it is NOT one of the anti-pattern instances listed above and MUST NOT be cited as a dim 9 deduction. The test: **would deleting this passage remove information the reader needs, or only remove words?** Deleting it removes only words → padding, deduct. Deleting it removes the reader's on-ramp to a term the memo later relies on → teaching, do not deduct.

This composes with — but is a distinct check from — the memo's step 5f **skeleton-level** plain-language constraint (`commands/memo-draft.md` step 5f; no term may appear in `skeleton.md` that a domain-literate outsider could not parse unless defined inline): the skeleton constraint is a drafting-time gate on the claim tree itself, while this dim 9 calibration governs how the reviewer scores the *expanded body prose* that walks a reader from plain language to a defined term. The framework's complementary cold-reader review test (`plain_language.md` point 7) is already implemented as the memo-only `memo-comprehension` critic sibling (`commands/memo-comprehension.md`, issue #753) — see `commands/memo-review.md` step 5's dim 9 sub-step for the reviewer-side cross-reference.
