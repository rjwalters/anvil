...
<project>/                 Project root (carries the project BRIEF; see §"Project root")
  ...
  <thread>/                Thread directory (named for the slug)
    ...
    <thread>.1.redteam/      Optional independent adversarial critic sibling (issue #560)
      ...
      calibration.md           ALWAYS emitted (issue #913): strongman-crosscheck mode (author-supplied refs/strongman-against.md; anticipated / novel / over-weighted objections) or self-derived mode (memo's own risk register / concessions / kill list) when no strongman resolves
      _meta.json               { critic: redteam, scorecard_kind: human-verdict, rubric_id: anvil-memo-v2, ... }
      _progress.json           Phase state (phase: redteam)
    <thread>.1.comprehension/ Optional cold-reader comprehension critic sibling (issue #753) — blind-read, findings-only, non-gating
      _review.json             Canonical typed review per anvil/lib/review_schema.py (all scores null; critical_flags always empty)
      answers.md               Phase 1: blind reader's answers to the 7-question questionnaire, frozen before any intent is read
      verdicts.md              Phase 2: per-question verdict (CLEAR / GARBLED / MISSING / HONEST-GAP) + diff-against-intent justification
      comments.md              Phase 3: GARBLED/MISSING findings (major) + undefined-term jargon list + question-7 enrichment candidates
      _meta.json               { critic: comprehension, scorecard_kind: human-verdict, rubric_id: anvil-memo-v2, ... }
      _progress.json           Phase state (phase: comprehension)
...
- **`--skeleton-gate`** (opt-in) stops the drafter after the skeleton so the operator can approve or correct the seven-sentence claim tree in one minute — the checkpoint at which every expensive sentinel-thread failure would have been caught (see `commands/memo-draft.md` §"CLI flags").
...
**Independence layer (issue #560)**: an optional `memo-redteam` critic (`commands/memo-redteam.md`) provides an **independent adversarial critic** alongside the author-supplied strongman. The red-team critic is **adversarial-by-default**: it generates objections independently — explicitly without reading `refs/strongman-against.md` during objection generation — and renders `DEFEATED` / `SURVIVES` / `UNENGAGED` verdicts on whether the memo's response defeats each objection. The author's strongman is preserved as drafter substrate AND becomes a **calibration crosscheck** in the red-team's output, ALWAYS written to `<thread>.{N}.redteam/calibration.md` (issue #913): `strongman-crosscheck` mode (anticipated / novel / over-weighted objections against `strongman-against.md`) when the author-supplied file is present, `self-derived` mode (anticipated / novel objections against the memo's own risk register / concessions / kill list, explicitly labeled as generous-to-the-author) otherwise — most threads never accumulate a hand-authored strongman, so `self-derived` is the common case. The red-team's `redteam_survives` / `redteam_unengaged` critical flags on load-bearing objections flow through the standard critic aggregation pathway (`anvil/lib/critics.py::aggregate`) to force `advance: false` — same plumbing as every other load-bearing back-check critical flag, no new state-machine transition. See `rubric.md` §"Red-team back-check (dim 2 + dim 3)" for the verdict vocabulary and `commands/memo-redteam.md` for the full critic spec.

**Comprehension layer (issue #753)**: an optional `memo-comprehension` critic (`commands/memo-comprehension.md`) is a **cold-reader comprehension critic** — the only critic whose failure mode is structurally decorrelated from the drafter's, because it must *produce* understanding rather than *assess* compliance. It is dispatched as a **fresh agent that reads ONLY the memo body + `exhibits/`** (no BRIEF, no `research/`, no `refs/`, no rubric, no skeleton, no prior reviews — the **blind-read invocation contract** is the load-bearing mechanism: a critic that has read the intended message cannot un-know it, and its reconstruction is worthless). The blind reader answers a fixed 7-question questionnaire (what does this company sell / who buys it / why does this team win / what is the ask / what kills it / undefined terms / what was missing) in its own words; each reconstruction answer is then diffed against intent and classified `CLEAR` / `GARBLED` / `MISSING` / `HONEST-GAP`. **`HONEST-GAP` is an explicit non-defect** — a memo that plainly states an unresolved gap (e.g. "the product does not yet exist; this raise funds the prototype") earns credit for stating it honestly rather than fogging it with mechanism language, so no finding is emitted. `GARBLED` and `MISSING` become `comments.md` findings at `major`; undefined terms become a jargon list; missing-context items become advisory enrichment candidates. The comprehension critic is **findings-only and provably non-gating**: it owns no rubric dimension (its `_review.json` carries all-`null` scores, so it contributes nothing to any dimension's mean-of-non-null merged score), and its `critical_flags` list is always empty. Its findings are consumed by `memo-revise` through the existing generic `<thread>.{N}.<critic>/` sibling enumeration with **zero code change**. Same opt-in, absence-tolerant posture as the `.audit/` / `.critic/` siblings. See `rubric.md` §"Comprehension back-check (findings-only, no dimension)" for the verdict vocabulary and `commands/memo-comprehension.md` for the full critic spec.
...
1. **`<thread>/REVISION_DIRECTIVE.md`** — single active directive at the thread root. Each new revision pass reads this file (if present) and the operator either edits in place between revisions or deletes the file when the directive no longer applies. This is the simpler shape — one file, always names the *next* revision pass. Matches the foundry-style operator workflow documented in `BRIEF.md`-citation form in the studio canary.
...
**Phase A / second-consumer discipline.** The convention is documented here at zero layout cost — it formalizes the foundry-style operator workaround surfaced by the studio canary (1 of 21 studio threads, per the curation in issue #237) without committing to a layout change. A future Phase B (a documented `directives/` slot in the canonical layout, drafter-side ingestion, or `_progress.json` integration) is gated on a second consumer signaling the same need, per the repo's `wait for the second consumer` lib-promotion discipline (CLAUDE.md, §"Working on this repo"). Operators who do not write directives are unaffected; the reviser's contract is "read if present, ignore if absent."
...
A `READY` thread (`advance:true` + 0 critical) is the normal terminus, but an operator MAY reject it for a reason the rubric cannot see — comprehension, a thesis reframe, new evidence that flips a decision — by invoking `memo-revise <thread> --reject "<reason>"` (issue #754). This is a distinct speech act from `--polish`: `--polish` sweeps the nits on a passing memo (line-level signal only, never restructures), while `--reject` is the operator **overruling the scoring system** and reopening the thread for a substantive rewrite. The entry point exists because the studio canary produced a memo that scored 43/44, `advance:true`, 0 critical at iteration 5 — READY by the rubric — that the operator nonetheless rejected ("almost word-soup; I am not learning anything interesting"), needing a new skeleton, a restated thesis, and a Gate-4-flipping break-even re-run. The pre-#754 workaround was to smuggle that mandate through `--polish` with the real intent carried in a directive file — which recorded `revision_mode = "polish"`, an audit-trail lie about what the revision was.

What `--reject` does:

1. **Bypasses the `advance:true` + 0-critical refusal** at `memo-revise` step 4, exactly as `--polish` does.
2. **Grants the FULL revision-plan contract**, NOT the polish subset — it consumes the review, ALL critic siblings, `_directives/v{N+1}.md`, and the BRIEF, and MAY restructure substantively.
3. **Treats the operator's reject reason as the highest-priority finding**, ranked above every critic note — it is definitionally the thing the rubric missed.

The reopened output is a normal `<thread>.{N+1}/` version dir carrying two `metadata` extensions as the on-disk audit trail:

- `metadata.revision_mode = "operator_reject"` (default is `"normal"` or absent).
- `metadata.reject_reason = "<verbatim operator-supplied reason>"` (default is `null` or absent).

The reason argument is **required**: empty, whitespace-only, or missing values are rejected with a clear error and the thread is left untouched — the same shape as `--polish` / `--override-no-go`. `--reject` is **mutually exclusive** with `--polish` and `--override-no-go` (either combination is rejected as contradictory); `--scope` / `--plan` / `--apply` compose normally.

**Signal for the rubric maintainer.** An `operator_reject` on an `advance:true` version is the strongest available evidence of a rubric blind spot, so the divergence is put on the record: the next `memo-review` pass emits a `## Operator rejection` subsection in `findings.md` quoting the reject reason verbatim (the #346 rubric-version-transition precedent — operator-visibility prose, no score impact). The reviewer does NOT otherwise special-case the reopened version — it scores it on its own rubric merits. `revision_mode` and `reject_reason` are audit-trail-only: not scored, not gating, no state-machine impact.

See `commands/memo-revise.md` §"CLI flags" §"`--reject`" for the full reviser-side contract and `commands/memo-review.md` step 9c for the findings-header note.
...
**Phase 1 — `--plan`.** `memo-revise <thread> --plan` writes a change-set preview at `<thread>.{N+1}.plan/plan.md` and exits WITHOUT producing `<thread>.{N+1}/<thread>.md`. The plan describes each planned edit (source critic, priority, insertion site, one-line summary, expected words delta, expected dim delta, and a proposed resolution mode — `added` / `restructured` / `removed` / `moved-to-exhibit`, issue #748) plus an aggregate footer with the projected new word count and a target-length flag (`within_target` / `exceeds_max` / `under_min` / `no_target`). The canonical shape is documented in `templates/plan.md.template`.
...
**Resolution modes + net-delta reporting (issue #748).** Every planned (non-declined) row also carries a `Mode` cell — `added` / `restructured` / `removed` / `moved-to-exhibit` — naming how the reviser proposes to resolve the finding. This is independent of the accept/decline mechanism above: an operator may edit the `Mode` cell in place (e.g. swap `added` to `removed`) without declining the item. The confirmed mode flows into that item's `Resolution: <mode> — <change>` row in `changelog.md`, which also opens with a `words: <before> → <after> (<Δ>)` / `bold spans: <before> → <after> (<Δ>)` net-delta header. Both are audit-trail-only — no score, gate, or state-machine impact, same constraints as `revision_mode` below. See `commands/memo-revise.md` §"Resolution modes" and §"Write `changelog.md`" for the full contract.
...
- **AI-authorship byline (opt-in, off by default — issue #941).** A project MAY declare a top-level `ai_byline:` block on its `BRIEF.md` (`anvil/lib/project_brief.py::AiByline`, resolved by `resolve_ai_byline(project_dir)`) to have `memo-render` append a short, configurable AI-authorship disclosure line to the rendered PDF. **Strictly opt-in**: absent block, or `enabled: false` (the default), is byte-identical to a pre-#941 install. When active, the resolved line is merged into the pandoc `render_metadata` passthrough (the #391 mechanism) as `ai_byline`, and `anvil/lib/memo/template.html` / `template.tex` render it as an italicized paragraph after the body. This is an editorial provenance choice, not a substitute for the intrinsic token-level model-output watermark — see `anvil/lib/snippets/provenance.md` for the boundary discussion and `commands/memo-render.md` step 4h for the full contract.
...
| `memo-revise <thread> [--polish "<reason>"] [--reject "<reason>"] [--plan|--apply]` | reviser | latest `<thread>.{N}/` + all `<thread>.{N}.*/` critic siblings (and `<thread>.{N+1}.plan/` on `--apply`) | `<thread>.{N+1}/` with `changelog.md` (default path; `--apply` path); OR `<thread>.{N+1}.plan/plan.md` only (on `--plan`); with `--polish`, also `metadata.revision_mode = "polish"` + `metadata.revise_force_reason` audit trail; with `--reject`, also `metadata.revision_mode = "operator_reject"` + `metadata.reject_reason` audit trail (reject reason ranked as the top revision-plan finding); with `--plan`/`--apply`, also `metadata.revision_mode = "plan_then_apply"` (or `"polish_plan_then_apply"` / `"operator_reject_plan_then_apply"` when composed with `--polish` / `--reject`). `--plan` and `--apply` are mutually exclusive; `--reject` is mutually exclusive with `--polish` and `--override-no-go`. See §"Operator-confirmable change-set preview" + §"Operator-initiated polish passes" + §"Operator rejection of an advancing version" for the full contracts. |
...
### Worked example: `synthesis-brief` (meridian-synthesis canary)
...
See the `meridian-synthesis` entry in `templates/BRIEF.rubric-overrides.md.example` for a copy-and-edit reference. The entry calibrates dims 1, 5, 6, 7 (the canary's load-bearing recalibrations) and sets `target_length` to `[9000, 13000]` words.

### Worked example: `feedback-memo` (northwind-pitch-strategy canary)
...
See the `northwind-pitch-strategy` entry in `templates/BRIEF.rubric-overrides.md.example` for a copy-and-edit reference. The entry calibrates dims 1, 4, 5, 6, 7 with prose framed around the positional-recommendation shape, and sets `target_length` to `[4000, 6000]` words.
