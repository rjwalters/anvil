...
- **`--scope critical-only`** — addresses ONLY review-critical-flag (and any optional `.audit/` / `.critic/` sibling critical-flag, EXCLUDING the specially-resolved `pending_dependency` flag — issues #842/#845) findings. All `blocker`, `major`, `minor`, and `nit` `comments.md` entries are deferred. Use case: a hot-fix iteration that lands the must-fix critical-flag failures (e.g., a `Summary-detail consistency: CONTRADICTED` flag from `memo-review` step 7) while explicitly punting the rest to the next pass.
...
Operator-rejection entry point for an advancing version (issue #754). When the prior review's `<thread>.{N}.review/verdict.md` records `advance: true` with 0 critical flags, the default verdict pre-check at step 4 refuses to proceed (the thread is `READY` — correct for the autonomous loop). `--polish "<reason>"` also bypasses that refusal, but its declared scope is line-level signal only (sub-threshold dimension lifts, `nit`-tagged / untagged comments) — a thesis-reframing rewrite is out of its scope. `--reject "<reason>"` is the lifecycle verb for the case the rubric cannot see: the operator overrules the scoring system and reopens an advancing thread for a **substantive** revision (comprehension, thesis reframe, new evidence that flips a decision). It:

1. **Bypasses the `advance:true` + 0-critical refusal at step 4** exactly as `--polish` does (proceeds to step 5 regardless of the passing verdict).
2. **Grants the full revision-plan contract, NOT the polish subset.** Unlike `--polish` (scoped to sub-threshold dimensions + `nit`/untagged comments), a `--reject` pass consumes the review, ALL critic siblings, `_directives/v{N+1}.md`, and the BRIEF, and MAY restructure substantively — new skeleton, reordered argument, updated deciding numbers. It is a normal default-path revision that happens to run against a passing version.
3. **Records the operator-supplied reason verbatim** in `<thread>.{N+1}/_progress.json.metadata.reject_reason`, and sets `metadata.revision_mode = "operator_reject"`.
4. **Treats the reject reason as the highest-priority finding** — the revision plan opens with it, ranked ABOVE every critic finding (see step 7). It is definitionally the thing the rubric missed.
5. **Preserves the passing `<thread>.{N}.review/verdict.md` unmodified** — the rubric's advancing verdict remains a permanent record of the divergence between the scoring system and operator judgment at that iteration, alongside the reopened version's audit trail.

**The reason argument is required.** `--reject` without a value, `--reject ""`, and `--reject "   "` (whitespace-only) are all rejected with a clear error pointing at this rule; the thread is left untouched (no version dir written, no `_progress.json` mutation). The reason exists as on-disk audit trail in `_progress.json.metadata.reject_reason` and is quoted verbatim in the `changelog.md` reject-pass header note — operators MUST supply substantive intent (e.g., *"Almost word-soup; the reader learns nothing interesting. Restate the thesis root as one corrected sentence and rebuild the skeleton around the Gate-4-flipping break-even re-run."*). This mirrors the `--polish "<reason>"` / `--override-no-go "<reason>"` rejection pattern: an unjustified override is treated as malformed.

**Signal for the rubric maintainer.** An `operator_reject` on an `advance:true` version is the strongest possible evidence of a rubric blind spot. The next `memo-review` on the thread SHOULD note the rejection in its `findings.md` header as operator-visibility prose with no score impact — the same #346 rubric-version-transition precedent used for surfacing an off-rubric divergence (see `commands/memo-review.md` step 9c). The divergence lands where scoring-calibration discussions can find it.

**What `--reject` bypasses.** Step 4's `advance:true` + 0-critical refusal **only** — the same precondition `--polish` bypasses. Step 3 (iteration-cap check) still applies — `--reject` against a thread at `max_iterations` still hits the BLOCKED notice. Step 1 (review-exists check) still applies — running `--reject` requires a fresh review to reject against. The flag is single-pass: it produces exactly one `<thread>.{N+1}/`, never loops, never consults a target score, never re-invokes itself.

**Composition with `--polish` and `--override-no-go`.** Mutually exclusive with BOTH. `--reject` + `--polish` is rejected as contradictory: both bypass the same `advance:true` + 0-critical precondition, but `--polish` scopes to line-level nits while `--reject` grants the full substantive contract — passing both is ambiguous about which scope the operator intends. `--reject` + `--override-no-go` is rejected as contradictory: they address opposite verdicts (`--reject` requires `advance:true` + 0-critical; `--override-no-go` requires `Verdict: NO-GO`). The reviser rejects either combination with a clear error naming the mutual exclusion.

**Composition with `--scope` / `--plan` / `--apply`.** These compose normally, exactly as they do with the existing flags. `--scope` defaults to `important` and filters comment-severity tiers as usual (the reject reason and sub-threshold dimension lifts are always in scope regardless of `--scope` — the reject reason is the highest-priority finding per step 7, and dimension lifts are independent of comment severity per §"CLI flags" §"`--scope <level>`"). `memo-revise <thread> --reject "<reason>" --plan` writes a reject-mode plan whose header records `Revision mode: operator_reject`, carrying the verbatim reject reason forward; the `--apply` invocation reads the reason from the plan header so the operator does NOT re-pass `--reject "<reason>"` on `--apply` — the plan IS the audit trail (same shape as the `--polish` composition). The composed flow produces `metadata.revision_mode = "operator_reject_plan_then_apply"`.

See SKILL.md §"Operator rejection of an advancing version" for the user-facing shape.
...
- **Header table** with the thread slug, source/target version paths, source review verdict + total /44, ISO-8601 timestamp, and the `Revision mode` (one of `normal` / `polish` / `operator_reject`).
- **Planned edits table** with one row per planned change: `ID`, `Source` (critic sibling + tag), `Priority` (one of `critical` / `major` / `nit` / `declined`), `Insertion site` (§N.M ¶X anchor in source memo), `Summary` (one-line description), `Words Δ` (signed integer), `Dim Δ` (e.g., `+1 dim 3`), `Mode` (one of `added` / `restructured` / `removed` / `moved-to-exhibit` — issue #748; see §"Resolution modes" below). The `Mode` cell is the writer-side reviser's proposed resolution mode for that item, chosen per the step 7 "consider `removed` before `added`" instruction; it is an ordinary table cell and MAY be edited by the operator in place between `--plan` and `--apply` (independent of the accept/decline mechanisms in §"Per-item rejection" — an operator may keep an item accepted but swap its proposed mode, e.g. from `added` to `removed`, simply by overwriting the cell). The `Mode` value flows verbatim into the item's `changelog.md` resolution-mode tag at `--apply` time (step 0b-9 / step 9 below).
...
1. **Pre-flight argument validation.** Reject `--plan` + `--apply` as mutually exclusive. Reject `--reject` + `--polish` and `--reject` + `--override-no-go` as mutually exclusive (per §"CLI flags" §"`--reject`"). When `--polish "<reason>"` or `--reject "<reason>"` is also passed, validate the reason argument per the existing required-reason contract (no empty / whitespace-only; same rejection shape as the default path).
...
4. **Verdict pre-check.** Same as step 4 of the default path. When neither `--polish` nor `--reject` is passed, an `advance:true` + 0-critical thread is rejected with the standard READY notice (no plan written). When `--polish "<reason>"` OR `--reject "<reason>"` IS passed, the verdict pre-check is bypassed per the existing polish-pass / reject-pass contract.
...
7. **Build the revision plan.** Same logic as step 7 of the default path: for each rubric dimension below threshold or with a critical flag, enumerate the specific changes; for each `comments.md` entry tagged `blocker` / `major` / `nit` (the `nit` tier ONLY when `--polish` is also passed), plan a concrete change. Resolve conflicting feedback between siblings explicitly per the existing convention. **For each planned item, also choose a proposed resolution mode** (`added` / `restructured` / `removed` / `moved-to-exhibit`) per the "consider `removed` before `added`" instruction documented at step 7 of the default path (issue #748) — the mode is written into the item's `Mode` cell in `plan.md` and remains freely editable by the operator before `--apply` (see §"Plan sibling shape" §"Planned edits table").
8. **Write the plan artifact via the staged sidecar.** **Open the staged sidecar** for the plan dir by invoking `anvil/lib/sidecar.py::staged_sidecar(final_dir=<thread>.{N+1}.plan, required_files=["plan.md", "_meta.json", "_progress.json"])`. Every file write below MUST land **inside the yielded staging directory** (the path of the shape `.<thread>.{N+1}.plan.tmp/`), NOT inside the final `<thread>.{N+1}.plan/` path. Write:
   - `plan.md` per the shape in `templates/plan.md.template`. The `Revision mode` header field equals `normal` (default), `polish` (when `--polish` was passed), or `operator_reject` (when `--reject` was passed). When `--polish` OR `--reject` was passed, write the verbatim operator reason into a header subsection or as a trailing line so `--apply` can read it back. Under `operator_reject` mode, the reject reason is the plan's highest-priority planned item per step 7 (ranked above every critic row). The aggregate footer's `Target-length flag` is computed from the projected new word count vs. the resolved target window. Each row of the `Planned edits` table carries a `Mode` cell per item 7 above.
...
1. **Pre-flight argument validation.** Reject `--plan` + `--apply` as mutually exclusive. Reject `--polish "<reason>"` when the loaded plan declares `Revision mode: normal` (contradiction — the plan is not a polish-pass plan). Reject `--reject "<reason>"` on `--apply` when the loaded plan does NOT declare `Revision mode: operator_reject` (the operator does not re-pass the reject reason on `--apply` — the plan IS the audit trail; passing it against a non-reject plan is a contradiction). Reject `--apply` when the plan declares `Revision mode: polish` or `Revision mode: operator_reject` and the plan's recorded operator reason is empty — this should never happen in practice but the defensive guard prevents a malformed plan from silently dropping the audit-trail reason.
...
7. **Parse the plan.** Read `plan.md`:
   - Extract the `Revision mode` (one of `normal` / `polish` / `operator_reject`).
   ...
   - For every non-declined row, also read the `Mode` cell (`added` / `restructured` / `removed` / `moved-to-exhibit`, issue #748) — this is the value the operator confirmed (by leaving the writer-side proposal unedited) or corrected (by overwriting the cell) between `--plan` and `--apply`. The mode carries through verbatim to that item's `changelog.md` resolution-mode tag at step 9 below.
...
8. **Initialize `_progress.json`.** Same as step 5 of the default path, with two differences:
   - `metadata.revision_mode = "plan_then_apply"` when the plan declared `normal` mode; `"polish_plan_then_apply"` when the plan declared `polish` mode; `"operator_reject_plan_then_apply"` when the plan declared `operator_reject` mode (issue #754). These are additive audit-trail-only values (NOT scored, NOT gating, NO state-machine impact). Readers that pre-date these changes tolerate the new values per the `revision_mode` shallow-merge tolerance contract.
   - `metadata.reject_reason = <plan's verbatim operator reason>` when the plan declared `operator_reject` mode (carried through from plan-time, NOT re-read from CLI); `null` (or omitted) otherwise.
...
9. **Resume at step 6 of the default path.** Read inputs, build the revision (now scoped to the non-declined items from the plan), produce `<thread>.md` + `changelog.md`. The reviser MUST address every non-declined planned item **using the mode recorded in that item's `Mode` cell** (parsed at step 7 above); declined items become `Resolution: declined — <reason>` rows in `changelog.md` (declined-by-deletion items get `Resolution: declined — removed from plan` with no specific reason). See step 9 of the default path below for the `changelog.md` mode-tag row shape.
10. **Changelog header note.** When the plan declared `polish` mode, prepend the polish-pass blockquote header note per step 9 of the default path (verbatim operator reason quoted from the plan, NOT the CLI). When the plan declared `operator_reject` mode, prepend the reject-pass blockquote header note per step 9 of the default path (verbatim reject reason quoted from the plan, NOT the CLI) and lead the resolutions table with the reject reason's top-priority row. When the plan declared `normal` mode, prepend a `(via plan)` annotation to the changelog's first line so downstream readers see the two-phase path was taken without inspecting `_progress.json`.
11. **Report.** Print the status line per step 11 of the default path, with `(via plan)` annotation: `Revised <thread>.{N} → <thread>.{N+1}/ (addressed M notes, declined P; via plan)`. When the plan declared `polish` mode, the annotation is `(polish pass; addressed M notes, declined P; via plan)`; when the plan declared `operator_reject` mode, the annotation is `(operator reject; addressed M notes, declined P; via plan)`.
...
4. **Verdict pre-check**: parse `<thread>.{N}.review/verdict.md`. If `advance == true` and there are no critical flags AND neither `--polish` nor `--reject` was passed, exit with a notice: the thread is `READY`, no revision needed. (Default behavior is to refuse to revise an already-passing version.)
   ...
   **`--reject` bypass (issue #754).** When `memo-revise <thread> --reject "<reason>"` is invoked, this step is skipped entirely; proceed to step 5 regardless of `advance:true` + 0-critical — the same bypass as `--polish`, but the pass that follows uses the FULL default-path revision-plan contract (step 7) rather than the polish subset, and the operator's reject reason enters the plan as its highest-priority item. Pre-check the flag's reason argument before bypassing: an absent / empty / whitespace-only reason is rejected with a clear error (see §"CLI flags" §"`--reject`"); the thread is left untouched. `--reject` is mutually exclusive with `--polish` and `--override-no-go` — reject either combination before the bypass (see §"CLI flags" §"`--reject`"). The override path additionally records `metadata.revision_mode = "operator_reject"` and `metadata.reject_reason = "<verbatim>"` at step 5.
5. **Initialize `_progress.json`**: write `phases.revise.state = in_progress`, `phases.revise.started = <ISO>`, `metadata.iteration = N+1`, `metadata.max_iterations` (the effective cap from step 3), and `metadata.iteration_cap_rationale` (the rationale from step 3 when the per-document BRIEF override is in effect; `null` otherwise). The drafter / reviser carry both fields forward on every pass so every version dir's `_progress.json` records the cap + rationale in effect when the version was produced. Also resolve `target_length` for v{N+1} per step 6 and record `metadata.target_length_resolved` with provenance — the resolution must happen before the revision-plan prompt is built so the resolved range is in scope for both the prompt injection and the `_progress.json` provenance write.
   ...
   **Operator-reject audit trail (issue #754).** When the reviser was invoked with `--reject "<reason>"` (per §"CLI flags" §"`--reject`"), write instead:
   - `metadata.revision_mode = "operator_reject"`.
   - `metadata.reject_reason = "<verbatim operator-supplied reason>"` — stored verbatim, no trimming / normalization / truncation beyond what JSON encoding requires. Same shape as `revise_force_reason` and `no_go_override_reason`.

   Both fields participate in the standard shallow-merge rule and are absent on every non-reject path (every non-reject version dir is byte-identical to the pre-#754 shape). The fields are audit-trail-only — NOT scored, NOT gating, NO state-machine impact at the reviser. The next `memo-review` pass does NOT special-case `metadata.revision_mode` to alter scoring (a reopened version dir is scored on its own rubric merits); it DOES read the field to emit the operator-visibility findings-header note per `commands/memo-review.md` step 9c, which carries no score impact. `--reject` is mutually exclusive with `--polish` and `--override-no-go`, so `revise_force_reason` and the NO-GO override fields are absent on a reject pass.
...
6. **Read inputs**:
   ...
   - Every other `<thread>.{N}.<critic>/` sibling discovered on disk (auditor, secondary critic, etc.), **including `<thread>.{N}.pending/` when present** (the pending-marker gate — issues #842/#845): its `_review.json` (`kind: tool_evidence`) carries a `pending_dependency`-typed `critical_flags` entry plus `nit` findings marking any honestly-declared `[PENDING <source>]` value still outstanding. Re-check this sibling on every revision pass — a marker resolved in v{N} but re-introduced (or newly added) in v{N+1} must be caught by the *next* `memo-review` re-run, not silently missed here. **This is the ONE flag class the reviser must NOT "resolve" by editing prose** — see the pending-marker carve-out under §"Notes for the reviser agent" below.
...
7. **Build a revision plan** — apply the `--scope` filter from step 5:
   - **Highest priority — no filter, ranked above every critic finding (issue #754)**: when the reviser was invoked with `--reject "<reason>"` (`metadata.revision_mode == "operator_reject"`), the operator's verbatim reject reason is the plan's top-priority item, ranked ABOVE every critic finding below. It is definitionally the thing the rubric missed — the operator overruled an `advance:true` verdict for a reason the scoring system could not see, so it leads the plan and the revision restructures to address it (new skeleton, reordered argument, updated deciding numbers, as the reason directs). The reject reason is NOT subject to the `--scope` filter. This item is absent on every non-reject pass.
   - **Always include (no filter)**: critical-flag findings (review-critical-flag from `memo-review` step 7, plus any optional `.audit/` / `.critic/` sibling critical-flag) — **EXCLUDING** the specially-resolved `pending_dependency`-typed flag from an optional `<thread>.{N}.pending/` sibling (issues #842/#845). These are addressed regardless of `--scope` per the §"CLI flags" critical invariants; the pending-marker flag is handled separately per §"Notes for the reviser agent" §"NEVER fabricate a value to clear a `pending_dependency` flag" below — it is carried forward verbatim, never "addressed" by editing prose.
   ...
   - **Findings are leads, not evidence (issue #749).** A citation, anchor, or verbatim quote supplied by a `comments.md` finding, a `verdict.md` critical-flag justification, an `.audit/`/`.critic/` sibling note, or an operator directive is a LEAD, not a verified fact. Before writing or modifying any citation, anchor, or quotation in the body in response to such a finding, the reviser MUST resolve it against the actual source file at write time — the same verification discipline `rubric.md` §"Refs back-check (dim 3)" already requires the reviser to apply when originating a citation itself. Do NOT transcribe a finding's own citation into the body unverified, even under iteration pressure or when the finding arrived pre-packaged with a specific location. When the verified source location or text differs from what the finding supplied, write the VERIFIED value into the body — not the finding's value — and log the correction in `changelog.md` per step 9's "Review-supplied citation correction note." This closes the two-generation error chain documented in issue #749: a citation mis-anchored in review prose that a trusting reviser writes into the body unverified, and that a subsequent reviewer's remediation guess then corrects to a second wrong location instead of the right one.
   ...
   - **Choose a resolution mode for each planned change before drafting it** (issue #748). The reviser's control surface historically selected *which* findings to address but not *how* — and the cheapest "how" is always to add a clause, which is why bold density and word count climb monotonically across revise cycles even when every finding is legitimately addressed (canary evidence: 19.9% → 25.5% of words bolded across four clean revise passes, portfolio median 10.9%). For every item surviving the filters above, decide which of four modes will resolve it:
     - **`added`** — new prose/exhibit content.
     - **`restructured`** — same content, reordered/reframed (net word-neutral).
     - **`removed`** — resolved by deletion: cut the overclaim rather than hedging it, cut the redundant restatement, cut the section that provoked the finding.
...
9. **Write `changelog.md`**: a markdown table mapping each critic note to the change made.
   ...
   **Skeleton-diff note (issue #752).** When the revision changed `skeleton.md` (case 1 of §"Skeleton-first revision" — structure or load-bearing claims changed), append a `> Skeleton diff:` blockquote note to the changelog header (after the net-delta lines, before the polish-pass / directive header notes) summarizing the claim-tree change in one or two lines — e.g. `> Skeleton diff: root claim changed from "owning the agent beats owning the gateway" to "owning the harness beats owning the models" (value-migration thesis); §3 (gateway argument) demoted from a section claim to a sub-claim under §2.` This makes the structural intent-change visible as a small tree diff at the top of the changelog, the load-bearing benefit of skeleton-first revision (a seven-sentence diff instead of a 3,400-word prose diff). When the skeleton was carried over verbatim (case 2 — line-level revision only), OR reconstructed from a prior body that had none (case 3), state that in one line — e.g. `> Skeleton diff: none (line-level revision; skeleton carried over).` or `> Skeleton diff: reconstructed from prior body (legacy thread had no skeleton.md).` The note is audit-trail only — not scored, not gating.
   ...
   |------------------------------|-----------------------------------------------|-------------------------------------|
   | acme-seed.1.review (blocker) | TAM figure $40B unsourced                     | added — cited Gartner 2025 report; verified figure is $38B (corrected) |
   | acme-seed.1.review (major)   | Risk #2 lacks mitigation                      | added — 1-paragraph mitigation referencing escrow structure        |
   | acme-seed.1.audit            | Cash burn rate disagrees with deck            | restructured — recomputed from primary deck; updated body and exhibit (no new prose, corrected the number in place) |
   | acme-seed.1.review (major)   | §4 overclaims "category-defining" without evidence | removed — cut the unsupported superlative rather than hedging it; the paragraph reads as a plain capability statement without it |
   | acme-seed.1.review (major)   | $38B figure not clearly contextualized        | addressed (judgment-held) — restructured — reframed as context anchor, not TAM claim; Gartner 2025 cited for validation range. Framing is structural: $38B is the ocean our customers swim in, not a claimed share. |
   ...
   **Resolution modes (issue #748).** Every `Resolution:` row where the body actually changed carries a leading mode tag naming *how* the finding was resolved — one of:
   - **`added`** — new prose/exhibit content.
   - **`restructured`** — same content, reordered/reframed/recomputed in place; net word-neutral.
   - **`removed`** — resolved by deletion: the overclaim, redundant restatement, or scope-creeping section was cut rather than hedged, padded, or qualified.
   - **`moved-to-exhibit`** — body prose demoted to apparatus (an exhibit, footnote, or appendix) rather than removed outright.

   The mode is a prefix on the existing free-text change description (`Resolution: <mode> — <change>`); it does NOT replace or narrow the description, it labels it. Under `--plan` / `--apply`, the mode is proposed by the writer-side `--plan` pass into that item's `Mode` cell in `plan.md` and flows verbatim into the `changelog.md` row at `--apply` time (see §"Plan sibling shape" §"Planned edits table") — the operator MAY overwrite the `Mode` cell before applying, independent of accepting or declining the item.

   Three further `Resolution:` dispositions (unchanged from the pre-#748 contract, extended to carry a mode tag where applicable):
   - **`Resolution: <mode> — <change>`** (default) — routine addressing; body changed; no carry-forward needed.
   - **`Resolution: declined — <one-line reason>`** — reviser disagrees; body did NOT change in response to this finding, so no mode applies. Carries forward to next pass (see §"Critical invariants" and step 7 above).
   - **`Resolution: addressed (judgment-held) — <mode> — <one-line judgment>`** — body DID change (carrying a mode tag same as the default row), but the addressing embodied a non-obvious structural framing that a future critic could re-raise. Use when: the addressing required a structural choice (reframe, scope limit, architectural trade-off) AND a fresh critic reading only the body — without the changelog — could legitimately flag the same concern from a different angle. Do NOT use for routine addressing. Carries forward to next pass alongside `declined` entries.

   **No new gating (issue #748).** Resolution modes and the net-delta header are audit-trail additions only — same class of change as `metadata.scope` and `metadata.revision_mode`. There is no minimum count of `removed` / `moved-to-exhibit` rows, no rubric dimension reads the mode tags, and a revision that resolves every finding via `added` still lands (the reviser is prompted at step 7 to reconsider such a plan before applying, but nothing on disk refuses it). If canary signal later shows the audit trail alone doesn't bend behavior, promotion to a reviewer-side finding is a separate, future decision.
   ...
   **Reject-pass header note (issue #754).** When `metadata.revision_mode == "operator_reject"` (i.e., the reviser was invoked with `--reject "<reason>"`), prepend a blockquote header note to `changelog.md` BEFORE the table, quoting the operator's reason verbatim:

   ```
   > Operator reject — `revision_mode: operator_reject`. Operator reason: <verbatim reason>.
   > The `advance:true` + 0-critical verdict was overruled by operator judgment
   > against the rubric — the reviewer scored this version READY, the operator
   > rejected it for a reason the rubric could not see. This revision uses the full
   > revision-plan contract; the reject reason is the highest-priority finding,
   > ranked above every critic note.

   | Source                       | Note                                | Resolution                          |
   ...
   ```

   The reject reason's top-priority resolution SHOULD appear as the FIRST row of the resolutions table, with a `Source` of `operator (--reject)` and the resolution describing the substantive change made in response (e.g., `restructured — rebuilt the skeleton around the corrected thesis; §2–§4 reordered so the break-even re-run leads`). The reason is quoted verbatim in the header note — do NOT paraphrase or shorten. This note is the on-record signal of the operator↔rubric divergence, complementing the operator-visibility findings-header note the next `memo-review` emits (see `commands/memo-review.md` step 9c). The reject-pass header note and the polish-pass header note are mutually exclusive (the two flags are mutually exclusive); a changelog carries at most one.
   ...
   **Review-supplied citation correction note (issue #749).** When step 7's "Findings are leads, not evidence" verification turns up a citation, anchor, or quote that a review finding, audit-sibling note, or directive supplied but that does NOT match what the source file actually says or where it actually lives, log the correction as its own row (or, if multiple corrections happen this pass, a short blockquote note before the resolutions table) naming the finding's claimed location, the verified location, and the source that settled it:

   ```
   > Corrected review-supplied anchor: `acme-seed.3.review`'s critical-flag prose cited the quote "..." to `research/15` §3B; verified against the source and found it at `research/11` §6 (line 857) instead. Body written against the verified location.
   ```

   This note is what makes a review-sidecar citation error visible on the pass that FIXES it, rather than only becoming visible after a second reviewer's own remediation guess also lands on the wrong location — the two-generation failure mode issue #749 exists to close. Absence of the note (every finding's supplied location verified clean) is the default and unremarkable; the note is written only when verification actually corrected something.
...
11. **Report**: print the path to the new version dir and a one-line status. The status line MUST include the scope level and the deferred count alongside the existing addressed / declined counts — e.g., `Revised acme-seed.1 → acme-seed.2/ (scope: important; addressed 4 notes, deferred 3 to next iteration, declined 1)`. The scope tag is the cheap operator signal that the run took a tiered filter; the deferred count is the cheap signal of how many findings were punted. Under `--scope all` the deferred count is zero and the line MAY omit the `deferred N to next iteration` clause (or print `deferred 0 to next iteration` — readers tolerate both shapes).
   ...
   When `metadata.revision_mode == "operator_reject"` (issue #754), include the `operator reject` annotation alongside the scope annotation; both stack in the status line. Example:
   - `Revised sentinel.5 → sentinel.6/ (operator reject; scope: important; addressed 5 notes, deferred 1 to next iteration, declined 0)` — an operator-rejected `advance:true` version reopened for a full substantive revision led by the reject reason.

   The polish-pass / operator-reject tag in the status line is the cheap operator signal that the run took the corresponding bypass; the scope tag is the cheap signal of which severity tiers were addressed. Both complement the on-disk `_progress.json.metadata.revision_mode` and `_progress.json.metadata.scope` audit trails.
...
- **Critical flags trump everything — with ONE explicit exception (issues #842/#845).** If any critic sibling raised a critical flag, the revision MUST address it — failing to do so is a worse outcome than declining a stylistic suggestion. **The exception is the specially-resolved `pending_dependency` flag** (from an optional `<thread>.{N}.pending/` sibling — see the carve-out immediately below): it is NOT a defect to fix in prose.
- **NEVER fabricate a value to clear a `pending_dependency` flag (issues #842/#845).** A `[PENDING <source>]` marker (surfaced by the `<thread>.{N}.pending/` sibling as a `pending_dependency`-typed `critical_flag` and `nit` findings, and as an "Outstanding dependencies" note in `verdict.md`) is an **honest disclosure that a load-bearing number does not exist yet** — a term sheet not yet signed, a customer reference not yet returned, a benchmark still running. It carries its own priority tier precisely so it does NOT force `advance: false` the way an ordinary critical flag does — do **not** treat it as a "critical flag that must be addressed" per the bullet above. The ONLY correct resolution is to replace the marker with the **real value once it genuinely exists**; until then, **leave the marker in place** (carry it forward verbatim into `<thread>.{N+1}/<thread>.md`) and note in `changelog.md` that the dependency is still outstanding (`Resolution: pending — <source> not yet resolved; marker carried forward`). **Inventing a plausible-looking number to make the marker "go away" is a severe error** — it is the exact fabrication failure this convention exists to prevent. The marker is not counted against any dimension score and does not block the revise loop; it only holds the `READY`/`AUDITED` terminal transition (`SKILL.md` §"Pending-marker terminal gate"), which is correct — the memo genuinely is not done until the value lands. **Orthogonal to `--override-no-go`**: a pending marker never blocks a normal revise pass and is not a condition the `--override-no-go` flag interacts with — the two overrides (NO-GO resurrection vs. pending-value resolution) are independent.
...
`metadata.revision_mode` is one of `"normal"` (default), `"polish"` (when invoked with `--polish "<reason>"`), `"operator_reject"` (when invoked with `--reject "<reason>"`, issue #754), `"plan_then_apply"` (when invoked via the `--plan` → `--apply` two-phase flow on a normal-mode plan), `"polish_plan_then_apply"` (when invoked via `--polish --plan` → `--apply` on a polish-mode plan), or `"operator_reject_plan_then_apply"` (when invoked via `--reject --plan` → `--apply` on a reject-mode plan). Absence of the field is tolerated by readers and treated as `"normal"` — every pre-this-change memo version dir omits this field, and downstream consumers MUST handle that case. The `"plan_then_apply"` and `"polish_plan_then_apply"` values are additive and were introduced by issue #243; the `"operator_reject"` and `"operator_reject_plan_then_apply"` values are additive and were introduced by issue #754; readers that pre-date either change tolerate the new values per the same shallow-merge rule. `metadata.revise_force_reason` is `null` (or absent) on the default path; the verbatim operator-supplied reason string when `--polish` was used (read either from the CLI on the default path or from the plan header on the `--apply` path — both paths produce byte-identical disk shapes). `metadata.reject_reason` (issue #754) is `null` (or absent) on every non-reject path; the verbatim operator-supplied reason string when `--reject` was used (read either from the CLI on the default path or from the plan header on the `--apply` path). All of these fields (`scope`, `revision_mode`, `revise_force_reason`, `reject_reason`, and the `plan_then_apply` / `polish_plan_then_apply` / `operator_reject` / `operator_reject_plan_then_apply` values on `revision_mode`) are skill-specific extensions to the `_progress.json` schema and are preserved by the shallow-merge rule per `anvil/lib/snippets/progress.md`. **These fields are audit-trail only — not scored, not gating, not state-machine inputs.** The reviewer does NOT read `revision_mode`, `revise_force_reason`, `reject_reason`, or `scope` to alter scoring and does NOT special-case the polish pass, the reject pass, the scope filter, or the plan-then-apply path; it scores the produced version on its own rubric merits. The one exception is the operator-visibility findings-header note the next `memo-review` emits when `revision_mode == "operator_reject"` (see `commands/memo-review.md` step 9c) — prose only, no score impact.
