...
**Role**: reviser.
**Reads**: latest `<thread>.{N}/` and ALL `<thread>.{N}.*/` critic siblings (`.review/`, `.audit/`, `.critic/`, ...). Also `<thread>/.anvil.json` (optional `body_filename` field — see §"Body filename resolution" below).
**Writes**: `<thread>.{N+1}/` containing the revised memo body (`<body_filename>`, default `memo.md`), exhibits, `_progress.json`, and a `changelog.md` mapping critic notes to the changes made.
...
- **Latest version**: highest `N` with `<thread>.{N}/<body_filename>` (default `memo.md`; see §"Body filename resolution").
...
```
<thread>.{N+1}/
  <body_filename>    Revised memo body. Default `memo.md`; per-thread override
                     via `body_filename` in `<thread>/.anvil.json` (see SKILL.md
                     §"Body filename customization"). Both default and override
                     paths produce a file at the same `<thread>.{N+1}/` location;
...
At command start (before step 1 of either the default or plan-then-apply path), invoke `anvil/skills/memo/lib/anvil_config.py::load_body_filename(<thread_dir>)` to resolve the body markdown filename for this thread. The returned string flows through every `<thread>.{N}/` and `<thread>.{N+1}/` path construction in the procedure below (every reference to `<body_filename>` resolves to this value; the default `"memo.md"` and an override like `"paper.md"` flow through identically). The loader is lenient — a missing `.anvil.json`, an absent `body_filename` key, or a malformed value all degrade to `"memo.md"` with a `UserWarning`. The body filename is consistent across versions of the same thread: a thread that drafted `<thread>.1/paper.md` revises into `<thread>.2/paper.md`.
...
Operator-confirmable change-set preview entry point. When passed, `memo-revise` writes a plan-only artifact at `<thread>.{N+1}.plan/plan.md` (a critic-sibling-shaped directory, NOT a version dir) describing the planned edits, and exits WITHOUT producing `<thread>.{N+1}/<body_filename>`. The plan is the in-band, durable, git-diffable alternative to an out-of-band AskUserQuestion prompt — operators see *what specifically would change* before any edit is committed, and edit the plan in place to reject items.
...
Plan-confirmation entry point. When passed, `memo-revise` reads an existing `<thread>.{N+1}.plan/plan.md` (written by a prior `--plan` invocation), validates the plan against the latest source review + critic siblings, and produces `<thread>.{N+1}/<body_filename>` (default `memo.md`) + `changelog.md` + `_progress.json` per the existing reviser contract — honoring per-item operator edits to the plan (declined items become `Resolution: declined — <reason>` rows in the changelog).
...
5. **Target version already exists.** `<thread>.{N+1}/` already contains a `<body_filename>` (default `memo.md`) — `--apply` already ran, or the operator hand-built a version dir. Remediation: delete or rename the existing version dir, then re-run `--apply`.
...
- A `<thread>.{N+1}.plan/` directory MUST NOT contain `<body_filename>` (default `memo.md`). The body markdown belongs in the version dir (`<thread>.{N+1}/`) that `--apply` writes.
...
When `--plan` is passed, the reviser executes a plan-only pass that produces `<thread>.{N+1}.plan/` (NOT `<thread>.{N+1}/<body_filename>`):
...
2. **Discover state.** Same as step 1 of the default path (find highest `N` with `<body_filename>` + `verdict.md`; require fresh review).
...
The reviser exits after step 9. **No `<thread>.{N+1}/<body_filename>` is written.** The next operator action is to read + (optionally edit) `plan.md` and then invoke `memo-revise <thread> --apply`.
...
2. **Locate the plan.** Find `<thread>.{N+1}.plan/plan.md` where `N` is the highest version with a `<body_filename>` + `verdict.md`. If absent, reject per plan-validity case 1 (no matching plan exists; remediation: run `--plan` first).
...
6. **Existing-target-version check.** If `<thread>.{N+1}/<body_filename>` already exists, reject per plan-validity case 5 (target version already exists; remediation: delete or rename the existing version dir).
...
1. **Discover state**: find the highest `N` with `<thread>.{N}/<body_filename>` (default `memo.md` — see §"Body filename resolution") AND at least `<thread>.{N}.review/verdict.md`. If no review exists, exit with an error ("no review to revise against; run `memo-review` first").
2. **Resume check**: if `<thread>.{N+1}/_progress.json.revise.state == done` and `<body_filename>` + `changelog.md` exist, the revision is complete — exit early with a notice.
...
6. **Read inputs**:
   - Prior version's `<body_filename>` (default `memo.md`; see §"Body filename resolution") and `exhibits/`.
...
8. **Produce `<body_filename>`** at `<thread>.{N+1}/<body_filename>` (default `memo.md`):
...
9.7. **Invoke `memo-render` (optional, non-blocking)**: after the revised `<body_filename>` (default `memo.md`) and `changelog.md` are written, invoke `memo-render <thread>` to render the revised body → PDF (`<body_basename>.pdf`; e.g. `memo.md` → `memo.pdf`, `paper.md` → `paper.pdf`) and write the render-gate findings into `<thread>.{N+1}/_progress.json.phases.render` + `_progress.json.render_gate`. This step is the lifecycle wiring shipped by Epic #158 Phase 3 (issue #190).
...
- A completed revision (`revise.state == done` AND `<body_filename>` + `changelog.md` exist) is never re-run.
