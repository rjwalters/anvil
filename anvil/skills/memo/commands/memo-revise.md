...
- **Critical flags trump everything — with ONE explicit exception (issue #841).** If any critic sibling raised a critical flag, the revision MUST address it — failing to do so is a worse outcome than declining a stylistic suggestion. **The exception is the specially-resolved `pending_dependency` flag** (from the `<thread>.{N}.pending/` sibling): it is NOT a defect to fix in prose. NEVER fabricate a value to clear a `[PENDING <source>]` marker — the ONLY correct resolution is replacing it with the real value once it genuinely exists. Until then, carry the marker forward verbatim into `<thread>.{N+1}/<thread>.md` and note in `changelog.md` that the dependency is still outstanding (`Resolution: pending — <source> not yet resolved; marker carried forward`). Inventing a plausible-looking number to make the marker "go away" is the exact fabrication failure mode this convention exists to prevent.
