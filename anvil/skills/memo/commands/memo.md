...
4. Recommend the next command per thread:
   ...
   | `REVIEWED` (advance=true, unresolved pending marker) | `(held — resolve the outstanding dependency named in verdict.md; do NOT run memo-revise to fabricate it)` |
   | `REVIEWED` (advance=true, no figures yet) | `memo-figures <thread>` (optional) |
   | `READY` | (terminal) |
   | `READY` + figures missing exhibits | `memo-figures <thread>` |

   **Pending-marker terminal gate (issue #841)**: a thread with `advance: true` and zero critical flags still reads as state `REVIEWED`, not `READY`, while `<slug>.{N}.review/verdict.md`'s "Outstanding dependencies" section is non-empty (equivalently, `<slug>.{N}.pending/_review.json` carries an unresolved `pending_dependency` flag). This is a distinct check from the critical-flag check below — a `pending_dependency` flag never appears in `verdict.md`'s `Critical flags` list, so it must be checked separately via the "Outstanding dependencies" section or the `.pending/` sidecar's `pass` field. See `commands/memo-review.md` step 7.
...
- **`memo-comprehension <thread>`** (issue #753) — cold-reader comprehension critic. Dispatched as a **fresh agent that reads ONLY the memo body + `exhibits/`** (no BRIEF, no `research/`, no rubric, no skeleton, no prior reviews — blindness is the load-bearing instrument); it answers a fixed 7-question questionnaire in its own words, then diffs each reconstruction against intent and classifies it `CLEAR` / `GARBLED` / `MISSING` / `HONEST-GAP` (`HONEST-GAP` is an explicit non-defect). **Findings-only and non-gating**: it owns no rubric dimension (its `_review.json` carries all-`null` scores, contributing nothing to any dimension's mean-of-non-null merged score) and never emits a critical flag (`critical_flags` is always `[]`). Its `GARBLED` / `MISSING` findings surface in `<slug>.{N}.comprehension/comments.md` at `major` and are consumed by `memo-revise` via the **existing generic critic-discovery / aggregation path with zero code change** — a new critic tag, not a new framework. **Non-gating**: absence of a comprehension sibling does NOT block the state machine; existing memo threads continue to advance unchanged. See `commands/memo-comprehension.md` for the full critic spec.
