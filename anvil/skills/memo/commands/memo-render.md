...
4g. **Resolve the consumer rhetoric rules** (issue #463, optional consumer rule file for the advisory `memo_rhetoric_lint` check): the intended carrier is the #461 voice contract's `voice.rhetoric_rules` BRIEF sub-key (a path to a consumer JSON rule file; wiring lands as a follow-up once both issues merge). Until that sub-key exists, omit the `rhetoric_rules_path=` kwarg and the framework default rule set (`anvil/lib/rhetoric_lint.py::DEFAULT_RHETORIC_RULES`) applies — defaults-only behavior is identical whether or not a `voice:` block is present. When a consumer file IS passed: valid rules merge over the defaults (id collision → consumer wins), `disable` ids switch off defaults, and malformed JSON graceful-degrades to a defaults-only run with one warning finding naming the parse error.
5. **Invoke the render gate**: call
   ...
   result = gate(
       ...
       rhetoric_rules_path=rhetoric_rules_path,  # omit until voice.rhetoric_rules is wired (issue #463; defaults apply)
   )
   ```
...
result = gate(
    kind="memo",
    version_dir=Path("meridian-synthesis.2"),
    out_pdf=Path("meridian-synthesis.2/meridian-synthesis.pdf"),
