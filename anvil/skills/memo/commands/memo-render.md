...
- **No anvil source repo on the consumer machine.** The importable `anvil/` package ships under `<consumer>/.anvil/anvil/` directly; the install-time `anvil_source` path (recorded in the gitignored `.anvil/.install-local.json` sidecar, not the tracked `install-metadata.json` — issue #894) is provenance metadata only and is not consulted at runtime (issue #230 canary).
