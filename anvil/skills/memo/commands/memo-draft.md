...
5. **Draft the memo**: produce `memo.md` with:
...
6. **Create exhibits** (inline only — full figure generation belongs to `memo-figures`): any tables or simple inline data structures referenced from the body should land in `exhibits/` as `.md` or `.csv` files. Image generation is deferred to `memo-figures`.
7. **Update `_progress.json`**: `phases.draft.state = done`, `phases.draft.completed = <ISO timestamp>`.
8. **Report**: print the path to the new version dir and a one-line status (e.g., `Drafted acme-seed.1/ (memo.md: 1240 words, 2 exhibits)`).
