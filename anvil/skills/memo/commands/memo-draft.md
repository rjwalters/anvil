...
6. **Draft the memo**: produce `<thread>.md` as an **expansion of the approved `skeleton.md`** (step 5f) — every body section must remain answerable to its skeleton section claim, and the body as a whole must deliver the skeleton's root claim. Do NOT introduce a load-bearing section or a load-bearing coinage that the skeleton does not already carry; if the act of drafting surfaces a genuinely new load-bearing claim, add it to `skeleton.md` first (still in this same version dir, before `phases.draft.state == done`), then expand it — the skeleton stays the source of truth for intent. Produce `<thread>.md` with:
   ...
   - **Evidence**: claims with sources. Inline citations are acceptable (footnote style or parenthetical); exhaustive reference list at the end is preferred for primary sources.
     ...
     **Label-once-then-ledger pattern (issue #747).** When **three or more** load-bearing claims share one provenance status (e.g., "these figures are all founder-reported and unreplicated," or "these estimates all derive from the same 2024 industry survey"), reach for a **claim-family ledger** instead of restating the caveat (c) inline at every point of use: (1) state the shared provenance status once at the body level, in the section (or memo) where the family first appears, pointing at the ledger, and (2) enumerate the individual claims in an `exhibits/` table or footnote list with a per-claim `Status` column. The body-level statement plus the ledger together satisfy the hook contract for every claim the ledger enumerates — you do NOT need to also append the hedge inline at each point of use, and doing so anyway earns no additional dim 3 credit while costing on dim 9 *Rhetorical economy* (see `rubric.md` §"Citation hooks (dim 3)" §"Label-once-then-ledger convention (issue #747)" and §"Dim 9 — rhetorical economy" §"Redundant provenance restatement (issue #747)"). Reserve the inline hedge (c) for claims that stand alone — fewer than three sharing a status, or a claim whose caveat differs from its neighbors — the ledger pattern is for **families**, not isolated estimates.
