...
4g. **Load `rubric_overrides` from `<thread>/.anvil.json`** — issue #233 / #265:
   - Invoke `anvil/skills/memo/lib/anvil_config.py::load_rubric_overrides(<thread_dir>)`. The thread dir is the **portfolio-root parent** of `<thread>.{N}/<thread>.md` (the directory that contains `BRIEF.md` and `.anvil.json`, NOT a version subdirectory). The loader returns a `RubricOverrides` instance per the schema documented in `anvil_config.py`'s module docstring.
