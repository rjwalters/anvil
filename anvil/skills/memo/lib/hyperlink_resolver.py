"""Hyperlink resolver critic for the memo skill (issue #335, Epic #328 Track B Phase 2).
...
external HTTP links found in a memo version directory.
...
- **Skill-local first.** Module lives at
  ``anvil/skills/memo/lib/hyperlink_resolver.py`` (this file). Promotion
  to ``anvil/lib/`` is deferred per CLAUDE.md "wait for the second
  consumer" rule.
...
writes ``<version_dir>.hyperlinks/_review.json``. The CLI entry-point
convention (``python -m anvil.skills.memo.lib.<module> <version_dir>``)
is documented as the **agreed coordination point** with the Phase 3
citation-coverage critic (#336) so both critics share an invocation
shape.
...
# ---------------------------------------------------------------------------
# Sibling-module imports (sys.path bootstrap for skill-local lib)
# ---------------------------------------------------------------------------
#
# The memo skill's lib modules use sys.path-based sibling imports per the
# precedent set by ``cross_thread_refs.py`` (which does
# ``from latest_resolution import ...``). Mirror that so this module can
# be imported as either ``anvil.skills.memo.lib.hyperlink_resolver``
# (the documented CLI form) OR via sys.path-injection in a test harness.
...
def _build_cli_parser() -> "object":
    ...
    p = argparse.ArgumentParser(
        prog="python -m anvil.skills.memo.lib.hyperlink_resolver",
        description=(
            "Hyperlink resolver critic for the memo skill. Walks every "
            "link expression in a memo version directory and validates "
            "each per its class (cross-thread refs, markdown internal, "
            "markdown external, wiki-links)."
        ),
    )
    p.add_argument(
        "version_dir",
