...
4. **Score the 9 dimensions** per `rubric.md` into `scoring.md` (`# | Dimension | Weight | Score | Justification`, integer scores, 1–3 sentence justifications quoting evidence):
   ...
   - **Dim 2 (Intuition before formalism)**: does every mechanism get a plain-language "why" before notation? Are analogies load-bearing and correct? Quote notation-without-prior-intuition, or an analogy that fights the mechanism, when deducting. Score against `anvil/lib/snippets/plain_language.md` §"The 7-point framework" (issue #1042) — a term introduced without its ordinary-language explanation, or a plain restatement dropped once the term is named, is this dimension's core failure mode.
