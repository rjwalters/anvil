...
5. **Draft the body** to `<thread>.1/<thread>.md` (the filename **echoes the slug** per #295 — never `primer.md`, never `post.md`), teaching from intuition:
   ...
   - **Intuition before formalism (dim 2)**: every mechanism gets a "why it works / why this choice" in plain language *before* (or instead of) notation. Analogies must be load-bearing and correct — an analogy that fights the underlying mechanism is worse than none. This is the primer skill's worked instance of the plain-language-first framework (issue #1042) — per `anvil/lib/snippets/plain_language.md` §"The 7-point framework", introduce a technical term only once its plain-language explanation has landed, and keep that explanation alongside the term afterward, not just at first mention.
