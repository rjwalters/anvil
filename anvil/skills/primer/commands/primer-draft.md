...
5. **Draft the body** to `<thread>.1/<thread>.md` (the filename **echoes the slug** per #295 — never `primer.md`, never `post.md`), teaching from intuition:
   ...
   - **Plain language before jargon (dim 9)**: per `anvil/lib/snippets/plain_language.md` (issue #1042), state each idea in ordinary language before naming it, introduce a technical term only when it earns its place, and keep the plain restatement alongside the label once it's introduced — the same discipline dim 2's "intuition before formalism" already applies at the mechanism level, generalized to every technical term in the piece. A passage that carries a reader from an ordinary-language statement to a precise one is not padding; see the snippet's point 6 and `anvil/lib/snippets/rubric.md` §"Dim 9 — teaching-oriented length is not padding".
