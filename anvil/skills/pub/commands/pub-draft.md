...
4. **Initialize `_progress.json`**: write `phases.draft.state = in_progress`, `phases.draft.started = <ISO timestamp>`, `metadata.iteration = N+1`, `metadata.max_iterations` (inherit from `<thread>/.anvil.json` if set, else 4). When the corpus tier is active (step 3b), also record `metadata.corpus_dirs_resolved`; omit it entirely when the tier is inactive.
