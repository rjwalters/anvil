...
| File | What it is |
|---|---|
| `SKILL.md` | Artifact contract, the `code_ref` optional-companion-input activation contract, the `## Adopting an existing spec` workflow, state machine (parallel review+audit, AUDITED-terminal), the audit-verdict scope boundary (three-way verdict is Phase 2), output format (LaTeX body), and the "Relationship to `anvil:primer`/`anvil:report`" table (this is a NEW skill, not a parameterization). Read this first. |
| `rubric.md` | 9-dimension /44 scorecard (`anvil-spec-v1`). **≥39 advances** (the audit-grade / legal band). Normative correctness at weight 7 (dominant); internal-consistency + claim-precision heavy; the review-side critical flags + the Phase-2-forward-referenced implementation-mismatch flag. |
...
| `commands/spec-audit.md` | Auditor (factual + spec↔implementation consistency). Resolves `code_ref` as its oracle; Phase 1 records a suspected mismatch as a `major` finding (the three-way verdict lands in Phase 2). Degrades gracefully when `code_ref` is absent/unresolvable. |
| `commands/spec-revise.md` | Reviser. Consumes BOTH critic siblings; never rewrites the spec to match a vestigial code path (ambiguous mismatches escalate to the operator). |
...
| `templates/spec.template.tex` | LaTeX body skeleton (scope/conformance, definitions, normative content, validity predicates, implementation-status placeholder, revision history). |
...
3. **Direction is never presumed** — the fix for a spec↔implementation mismatch is a human decision (spec-wrong / code-wrong / intentional-gap). Phase 1 records a suspected mismatch as a `major` finding and never auto-rewrites the spec; the full three-way verdict + implementation-status register land in Phase 2 (#707). This is the load-bearing safety property born from the motivating botho near-miss.
...
5. **Adoption-first** — most specs predate anvil adoption. `## Adopting an existing spec` (modeled on `pub`'s "Migrating an existing paper," not `project-migrate`) is a first-class placement workflow, and `spec-draft` is correspondingly thin.
