...
| `READY` | Latest `<thread>.{N}.review/verdict.md` records `advance: true` (≥35) AND latest `<thread>.{N}.audit/verdict.md` records `pass: true` AND no unresolved critical flag in either sibling AND no unresolved `[PENDING <source>]` marker (issue #842/#843 — see `anvil/lib/snippets/pending_marker.md`; the `<thread>.{N}.pending/` sibling's specially-resolved `pending_dependency` flag gates this terminal transition even when the score/critical-flag verdict otherwise clears) |
| `AUDITED` | Same as `READY` for this skill — `AUDITED` is the standard anvil terminal state; proposal reaches it once both critic siblings clear AND no unresolved `[PENDING <source>]` marker remains. There is no further `CUSTOMER-READY`/`promote` stage (that is report-specific). |

> **Pending-marker gate is a separate terminal condition, not a critical flag (issue #842/#843).** An unresolved `[PENDING <source>]` marker (an honest disclosure that a load-bearing figure is not yet available — a vendor quote not returned, a site survey still scheduled, a permit ruling still pending) holds the READY and AUDITED transitions *independently of* the "no unresolved critical flag" clause above. It is deliberately **not** implied by that clause: the deterministic `pending_marker` gate (`anvil/lib/pending_marker.py`, wired at `proposal-review` step 4l / step 7 and `proposal-audit` step 11b) emits the specially-resolved `pending_dependency` flag type, which `convergence.blocking_critical_flags` filters *out* of the ordinary blocking-critical set — so it never forces `advance: false` / `pass: false`, never lowers a dimension score (see `rubric.md`), and would be missed by a reader relying on the literal "no unresolved critical flag" wording. Resolving it means replacing the marker with the real value; **never fabricate a value to force the terminal transition.**
