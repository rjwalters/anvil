...
- **`cost_basis`** (issue #840): read from the thread-level `<thread>/BRIEF.md` frontmatter via `anvil/skills/proposal/lib/project_brief.py::load_cost_basis`; default `quoted`. Calibrates how dimension 6 is scored (see step 4k / step 5).
...
4k. **Load `cost_basis` from the thread-level `<thread>/BRIEF.md`** — issue #840:
   - Invoke `anvil/skills/proposal/lib/project_brief.py::load_cost_basis(<thread_dir>)`. Same thread dir as step 4j — the directory containing `BRIEF.md`, NOT a version subdirectory.
   - The loader returns one of:
     - `"quoted"` — the operator has declared (or left default) the standard vendor-sourced posture. Dim 6 scores against the standard calibration in the rubric table (byte-identical to pre-#840 behavior).
     - `"estimated"` — the operator has declared the proposal's priced figures are unsourced internal engineering estimates (no vendor quotes exist). Dim 6 scores at step 5 against the estimate-basis-consistency calibration documented in `rubric.md` §"Dim 6 — `cost_basis` calibration".
     - `"none"` — the operator has declared this proposal has no hardware BOM and no vendor quotes at all. Dim 6 scores against the `none`-mode calibration in the same rubric section (full weight by default, absent an un-deferred stray priced claim).
     - `None` — the field is absent, malformed, or set to an unrecognized value (a typo like `Quoted`, `vendor`); the calibration does not fire; dim 6 scores against the standard calibration verbatim (same as `"quoted"`).
   - **Lenient by design**: the loader never raises. Every absence / malformed path resolves to `None`, which reads identically to `"quoted"` for scoring purposes — this is the load-bearing backwards-compat: a thread with no BRIEF, no frontmatter, no `cost_basis` key, or a typo behaves byte-identically to a thread that pre-dates this helper.
   - **Orthogonal to `customer_kind`**: `cost_basis` answers "do priced claims have external sources at all?"; `customer_kind` answers "who is the audience for dim 7?" An `internal` build spec can be `quoted`, `estimated`, or `none` just as freely as an `external` pitch — the two knobs are independent and both may fire on the same thread.
   - **Cache the resolved value** as `cost_basis_resolved` for the dim 6 scoring at step 5 and the `_summary.md` write at step 9b. The cached value is one of `"quoted"` / `"estimated"` / `"none"` / `None`.
...
5. **Score each dimension** (1–9 per rubric):
   ...
   - **Dim 6 (Cost credibility) `cost_basis` calibration sub-step** (issue #840): the cached `cost_basis_resolved` from step 4k routes dim 6's scoring:
     - `"quoted"` or `None` — score dim 6 exactly as written in the rubric table (byte-identical to pre-#840 behavior): figures need a basis (planning range, vendor list price, quote); an unsourced price is a defect.
     - `"estimated"` — apply the estimate-basis-consistency ladder from `rubric.md` §"Dim 6 — `cost_basis` calibration" verbatim: full weight requires every figure captioned as an estimate with a named, internally-consistent basis and correct arithmetic; an unsourced figure is NOT a defect under this mode (it is the expected, correctly-labelled shape) — only a missing/thin/inconsistent estimate basis, or a figure that reads as if vendor-sourced despite the declared mode, costs points. Append the verbatim suffix `"cost_basis: estimated — scoring dim 6 on estimate-basis consistency, not vendor sourceability"` to the dim 6 `scoring.md` justification.
     - `"none"` — dim 6 scores full weight by default (there is no BOM to source); the only deduction path is a stray un-deferred priced claim outside §10 Open Decisions, which is scored as if unsourced under `quoted`. Append the verbatim suffix `"cost_basis: none — no hardware BOM / vendor quotes; dim 6 scores full weight absent a stray un-deferred priced claim"` to the dim 6 `scoring.md` justification.
     **Inert when `cost_basis_resolved` is `"quoted"` or `None`**: no suffix is appended, no calibration prose changes — dim 6 reads exactly as the pre-#840 rubric table.
...
9b. **Write `_summary.md` with the top-level `rubric` block (issue #346)**: emit a JSON-in-markdown `_summary.md` carrying at minimum the `rubric` block — the rubric the reviewer scored against, so a downstream consumer aggregating across versions does not need to walk back to `anvil/skills/proposal/rubric.md` (which may have changed between v3 and v5 of a long thread that spanned the `/40 → /44` migration). Shape:
    ...
    **Backwards-compat — `recommendation_target_resolved`**: a legacy review sibling produced before this block shipped MAY omit `recommendation_target_resolved` entirely. Downstream consumers MUST tolerate the absence by treating it as `{value: null, applied: false}`. New reviews produced after this contract ships SHOULD emit the block on every review run (with `value: null, applied: false` when the field was not declared or the BRIEF was missing).
    ...
    The `cost_basis_resolved` block (issue #840) is populated from the cached `cost_basis_resolved` value from step 4k, and lives at the **top level** of `_summary.md` (sibling to `rubric` and `recommendation_target_resolved`) for the same reason: it is operator-declared posture metadata that triggers a dim 6 calibration, not rubric-version metadata. Shape:
    - `value` (`str | null`): the verbatim `cost_basis` value from `<thread>/BRIEF.md`'s YAML frontmatter when present and in the closed set (one of `"quoted"` / `"estimated"` / `"none"`); `null` when the field was absent, malformed, or set to an unrecognized value.
    - `applied` (`bool`): `true` if and only if `value` is `"estimated"` or `"none"` and the reviewer applied the corresponding dim 6 calibration per step 5's `cost_basis` calibration sub-step (and `rubric.md` §"Dim 6 — `cost_basis` calibration"). `false` for every other path — including `value: "quoted"` (standard dim 6 calibration applies) and `value: null` (no posture declared; standard dim 6 calibration applies, byte-identical to pre-#840).

    **The `cost_basis_resolved` block does NOT participate in `critical_flag`**, mirroring `recommendation_target_resolved` — the load-bearing surfacing of the calibration is the `scoring.md` suffix; this block is the structured audit-trail shadow.

    **Backwards-compat — `cost_basis_resolved`**: a legacy review sibling produced before this block shipped MAY omit it entirely. Downstream consumers MUST tolerate the absence by treating it as `{value: null, applied: false}` (identical to the pre-#840 `"quoted"` reading).
