...
6. **Read inputs** — discover the synthesis sibling first; prefer `gaps.json` as the revision-plan source when present, fall back to per-sibling finding reading when absent.
   ...
   **6e. Perishable-claim freshness (both paths, advisory — issue #863)**: run `uv run --project .anvil python -m anvil.lib.probe_freshness <thread>/<thread>.{N}` (bare `python -m ...` in the anvil source repo). It reads the auditors' `probes.json` re-probe lists across every version of the thread and returns a bounded checklist of perishable claims — live URLs, HTTP statuses, version pins, served-artifact SHAs, quotes with expiries — that are either `STALE` (older than their freshness budget) or `NOT-REPROBED` (verified against an earlier version and carried forward since). Carry the `needs_reprobe` list into step 7 as a plan input. The report's `unknown_freshness` list names audit siblings that recorded no probes at all (every pre-#863 sibling) — informational, never a defect. The tool is advisory: it always exits `0`, never affects the score, the `--scope` filter, `advance`, or any critical flag, and a `NEEDS-REPROBE` status alone is not a reason to revise.
7. **Build a revision plan** — the walk depends on the source selected in step 6.

   **Findings are leads, not evidence (issue #749).** Applies to BOTH paths below. A `refs/<file>` anchor, quote, price, or spec value supplied by a gap's `recommended_response`, a singleton's finding note, a `comments.md` entry, or an audit `findings.md` row is a LEAD, not a verified fact. Before writing or modifying such a value in `proposal.tex` in response to a finding, the reviser MUST resolve it against the actual source file at write time — the same verification discipline `rubric.md` §"Refs back-check (dim 6 + dim 4)" already requires `proposal-audit` to apply when it originates a source-of-truth claim itself. Do NOT transcribe a finding's own citation into the body unverified. When the verified value differs from what the finding supplied, write the VERIFIED value — not the finding's value — and log the correction in `changelog.md` per step 9's "Review-supplied citation correction note."

   **An expired verification is also a lead (issue #863).** The same rule extends across *time*, not just across siblings: a prior pass's `VERIFIED` on a **perishable** claim — a live URL, an HTTP status, a version pin, a served-artifact SHA, a quote with an expiry — is a lead once step 6e reports it `STALE` or `NOT-REPROBED`. Before carrying such a claim into `<thread>.{N+1}/proposal.tex`, **re-run the probe** (the report supplies `target`, `method`, the previously `observed` value, and often a literal `recheck_command`, so this is a bounded re-check, not a re-audit). If the world moved, correct the claim and log it in `changelog.md` with the old and new observed values. If you cannot re-probe it, say so in the changelog rather than silently inheriting the older verdict — a perishable claim the recipient can falsify with a browser is the worst way for a customer-facing document to fail. The reviser does not write `probes.json` (audit siblings are read-only and the reviser is not a critic); the next `proposal-audit` pass records the fresh probes.
...
9. **Write `changelog.md`**: a markdown table mapping each gap or critic note to the change made. The `Source` column shape depends on whether the revision plan was built from synthesis (step 7a) or from per-sibling findings (step 7b).
   ...
   **Review-supplied citation correction note (issue #749).** When the "Findings are leads, not evidence" verification at step 7 turns up a `refs/<file>` anchor, quote, price, or spec value that a gap, singleton, `comments.md` entry, or audit `findings.md` row supplied but that does NOT match what the source file actually says, add a row here (or a short blockquote note before the resolutions table) naming the finding's claimed value, the verified value, and the source that settled it:

   ```
   > Corrected review-supplied anchor: `gossamer-lan.3.audit` findings.md row 7 cited `refs/quote-acme-sfp.pdf` for a $42/unit transceiver price; verified against the quote and found $52/unit ($42 was the prior-generation part). BOM written against the verified price.
   ```
...
- **Audit-critical flags trump everything — with ONE explicit exception (issue #841).** A failed BOM subtotal or a link budget that does not close is a worse outcome than declining a stylistic suggestion. Fix the math first. **The exception is the specially-resolved `pending_dependency` flag** (from the `<thread>.{N}.pending/` sibling): it is NOT a defect to fix in prose. NEVER fabricate a value (a vendor price, a bench measurement) to clear a `[PENDING <source>]` marker — the ONLY correct resolution is replacing it with the real value once it genuinely exists. Until then, carry the marker forward verbatim into `<thread>.{N+1}/proposal.tex` and note in `changelog.md` that the dependency is still outstanding (`Resolution: pending — <source> not yet resolved; marker carried forward`). Inventing a plausible-looking cost figure to make the marker "go away" is the single most severe error available in a customer-facing proposal.
