...
- **`cost_basis`** (issue #840): read from the thread-level `<thread>/BRIEF.md` frontmatter via `anvil/skills/proposal/lib/project_brief.py::load_cost_basis`; default `quoted`. Gates step 7's vendor-quote sourceability walk (see step 7).
...
3. **Open the staged sidecar** for the audit dir by invoking the context manager `anvil/lib/sidecar.py::staged_sidecar(final_dir=<thread>.{N}.audit, required_files=["verdict.md", "findings.md", "evidence.md", "probes.json", "_meta.json", "_progress.json"])`. Every file write below MUST land **inside the yielded staging directory** (the path of the shape `.<thread>.{N}.audit.tmp/`), NOT inside the final `<thread>.{N}.audit/` path. On clean context exit, the primitive verifies the manifest, then atomically renames the staging dir to its final name (issue #350). Note: when the writer uses an accepted alias for `findings.md` per the "Alias contract" above (e.g., `claim-log.md`), the required-files manifest passes the alias name in place of `findings.md`. Then, **inside the staging dir**, initialize `_progress.json` for the audit dir: `phases.audit.state = in_progress`, `phases.audit.started = <ISO>`, `for_version = N` (per `anvil/lib/snippets/progress.md`). Also initialize `_meta.json` with `scorecard_kind: human-verdict` (see `anvil/lib/snippets/scorecard_kind.md`); proposal-audit ships task-specific `findings.md` and `evidence.md` alongside the scorecard-kind declaration. (Per the migration note in `audit.md`, this command emits the legacy prose triple today; the legacy adapter bridges it to the `kind: tool_evidence` contract.)
   ...
   **Non-Python-driver ordering (fail-open, manual fallback)** — issue #645: `staged_sidecar` is a Python context manager. A manual/agent session with **no orchestrating Python driver** cannot hold its `with` block open across the file writes below (it writes files with its own editing tool between discrete steps), so it MUST use the equivalent CLI shim rather than writing straight into the final `<thread>.{N}.audit/` dir (which silently reopens the #350 partial-write defect this primitive exists to close). Two tiers, in preference order:
   ...
   The two tiers land a byte-identical on-disk result to the `staged_sidecar` context-manager path; they exist only to give a Python-less session a code-enforced (tier 1) or contract-faithful (tier 2) route to the same atomicity guarantee. When an orchestrating Python driver IS present, use `staged_sidecar` directly as documented above — the CLI shim is not needed. (If your agent harness pattern-matches and rejects filenames like `verdict.md` / `findings.md` on a `Write`, a Bash-heredoc write into the staging dir is an accepted fallback — see `anvil/lib/snippets/critics.md` §"Orchestrator output-file guard collisions".)

4. **Read inputs**: load `<thread>.{N}/proposal.tex`, parse the BOM / labor / project-total tables and the spec tables, enumerate `refs/`. **Optional perspective context**: enumerate `<thread>.*.perspective/` siblings under the thread root and, if any exist, load the latest one's `notes.md` and `candidates.md` for use in step 7's sourceability walk. Absence is fine — the audit's existing cost-only sourceability behavior is unchanged when no perspective sibling is present (graceful skip; backward-compat per `anvil/lib/snippets/perspective.md` §"State-machine non-gating"). **`cost_basis` (issue #840)**: read the thread-level `<thread>/BRIEF.md`'s `cost_basis` frontmatter key via `anvil/skills/proposal/lib/project_brief.py::load_cost_basis(<thread_dir>)` — lenient, never raises, resolves to `"quoted"` / `"estimated"` / `"none"` / `None`. Cache as `cost_basis_resolved` (treat `None` as `"quoted"` for gating purposes — byte-identical to pre-#840 behavior). This gates step 7's vendor-quote sourceability walk.
...
7. **Audit sourceability** — the cost-credibility check. **Gated on `cost_basis_resolved` from step 4 (issue #840)**:
   - **`cost_basis_resolved == "none"`** — SKIP the vendor-quote sourceability walk below (the priced-line basis bullets AND the perspective candidates sub-step) entirely. There is no hardware BOM and no vendor quotes to back-check; the priced-table requirement was dropped at draft time (`templates/proposal.tex.j2` §"7. Bill of Materials / Cost Basis" — the proposal ships a "Cost Basis" section instead). Record `Sourceability walk (vendor-quote): skipped — cost_basis: none` in `evidence.md` and proceed directly to the **refs back-check sub-step below**, which is UNCHANGED by `cost_basis` (it covers non-cost claims — scope, deliverability, comparable-project — and remains active regardless), then to step 8. If the proposal makes a stray priced claim anyway (a dollar figure outside §10 Open Decisions), that is a contract violation of the declared `cost_basis: none` — treat it as a finding and flag it under critical flag 2 as if unsourced.
   - **`cost_basis_resolved == "estimated"`** — do NOT treat a missing vendor-quote basis as a defect; every price is *expected* to be an internal engineering estimate, not a sourced quote. In place of the vendor-quote walk below, verify (a) each price is captioned as an estimate per the template's estimate-labelled caption contract (e.g. "Materials subtotal (estimated)", an "ESTIMATE BASIS" preface, an "ESTIMATE, NOT A QUOTE" cost-notes caption), and (b) the estimate basis is named and internally consistent (a stated basis — comparable prior work, engineering judgment, a named estimator — not silently absent) and the arithmetic holds (`Qty × Unit = Total` per step 5, unchanged). Record the estimate-basis check in `evidence.md` in place of the vendor-quote basis. **Critical flag 2 still applies** if the estimate basis is missing entirely, the captioning is absent (the proposal reads as if vendor-sourced despite the declared mode), or the arithmetic is wrong — the estimate/quote distinction changes WHAT counts as a valid basis, not whether some basis must be stated at all. The perspective candidates sub-step below still applies if a perspective sibling's comparable-project entries are cited as the estimate's stated basis.
   - **`cost_basis_resolved == "quoted"` or `None`** — unchanged pre-#840 behavior: run the full vendor-quote sourceability walk below.
   ...
   - **Refs back-check sub-step for non-cost claims** (issue #166): the cost-only sourceability walk above is the existing v0 audit behavior; the back-check **extends** the same per-claim discipline to **non-cost claims** whose evidentiary basis lives in `refs/`. Enumerate `<thread>/refs/` and identify the **source-of-truth materials** present per SKILL.md §"Source-of-truth materials" (files named for their content — `quote-<vendor>.{pdf,md}`, `datasheet-<part>.pdf`, `sow-*.md`, `comparables/<project>.md`, `cv-<lead>.{pdf,md}`, `site-plan-*.pdf`, `prior/<vN>.{pdf,md}`). The back-check applies to source-of-truth materials only; generic reference material (rough notes, draft sketches not named as a source-of-truth) is out of scope for this sub-step. For each source-of-truth refs-document **type** present that is on-topic for a non-cost claim — **scope-bearing** files (SOW templates, sequenced-install method references), **deliverability-bearing** files (CVs of named leads, comparables documenting prior delivery capability), **comparable-bearing** files (prior-project cost / scope / outcome references) — pick at least one load-bearing claim in `proposal.tex` whose evidentiary basis is the document's subject and write a `findings.md` entry of the form:
     ...
     **Finding hygiene — resolve locations before writing them (issue #749).** When a `findings.md` row names a specific `refs/<file>` location, passage, or price, the auditor MUST have actually looked it up before writing it — the back-check discipline above applies to the auditor's OWN row prose, not just to the proposal's claims. Do NOT name a `refs/<file>` anchor or price from memory or a plausible-sounding guess. A finding that mis-cites its own basis is a defective finding: the next reviser has every incentive to trust a finding's own citation rather than re-derive it under iteration pressure (see `commands/proposal-revise.md` step 7 §"Findings are leads, not evidence"). This closes the audit-sidecar leg of the two-generation citation-error chain documented in issue #749.
...
11b. **Pending-marker terminal gate (deterministic, unconditional) — issue #841**: invoke `uv run --project .anvil python -m anvil.lib.pending_marker <thread>.{N}/ --write-review --body proposal.tex` (same tool and invocation shape as `proposal-review` step 4l). This re-runs the deterministic `[PENDING <source>]` marker gate as defense-in-depth before `AUDITED`, and is genuinely useful independent of the review pass when the auditor runs first. `--write-review` re-writes `<thread>.{N}.pending/_review.json` (deterministic-regeneration carve-out — a later pass supersedes an earlier one), carrying the specially-resolved `pending_dependency` flag whenever any active marker remains. **The gate is the tool's exit code / `pass` field, NOT a `Verdict.BLOCK`**: the flag is visible-but-non-blocking, so it does not tank `pass` the way a fabricated-cost flag does — instead, per step 12 below, `AUDITED` additionally requires the gate to have passed. A well-formed marker is an honest disclosure, not a defect — do NOT add a corresponding row to the claim inventory as a mismatch, and do NOT fabricate a value to clear it. **Tooling-absent fail-open**: mirrors step 4's toolchain-absent handling — manually scan `proposal.tex` for the literal marker shape and hold the terminal gate on any found marker without fabricating, recording the fallback with a one-line note.
12. **Compute pass/fail**: `pass = (no critical flags) AND (all priced lines and quantitative claims verified, or partial-with-acceptable-rationale)`. Under `cost_basis_resolved == "none"`, there are (by design) no priced lines to verify — the clause is vacuously satisfied; pass turns entirely on critical flags and any other quantitative claims (spec/link-budget, internal consistency) audited at steps 6/8. Under `cost_basis_resolved == "estimated"`, "verified" reads as "estimate-basis stated, internally consistent, and arithmetically correct" per step 7, not "vendor-sourced".

    **Terminal-state gate for `AUDITED` (deterministic, separate from `pass` — issue #841)**: even when `pass == true`, the thread does NOT reach `AUDITED` while step 11b's pending gate has not passed (an active `[PENDING <source>]` marker remains anywhere in `proposal.tex`). Hold the thread at `REVIEWED+AUDITED` (not `AUDITED`) with an "Outstanding dependencies" note in `verdict.md` naming the still-pending source(s) — the audit genuinely is not done until the value lands; never fabricate a value to force `AUDITED`.
13. **Write `verdict.md`** in the format specified in `rubric.md`:
    ...
    - **Outstanding dependencies (conditional — issue #841)**: when step 11b's pending gate did not pass, list each still-pending source label and state explicitly this holds `AUDITED`, is NOT a scored defect, and NOT a critical flag. Omitted entirely when the gate passed cleanly.
