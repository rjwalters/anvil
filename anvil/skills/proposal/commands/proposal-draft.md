...
5. **Fill the template** to produce `proposal.tex` from `templates/proposal.tex.j2`. The template provides the 10-section skeleton; the drafter elaborates each section into prose, tables, and figure references. Thread the customer's **hard constraints** (from the Premise) through every section — the reviewer scores constraint satisfaction (dim 3) on exactly this:
   ...
   7. **Bill of Materials / Cost Basis** — the central proposal artifact for a hardware-BOM proposal: a multi-section priced `tabularx` BOM + a **labor estimate** subsection + a **project total** subsection (materials + labor → total). Anchors scope completeness (dim 4) and cost credibility (dim 6). Gated on `cost_basis` (see step 6b and "Priced tables" below) — a `cost_basis: none` proposal replaces this section with a lighter "Cost Basis" section instead.
...
6b. **`cost_basis` handling** (issue #840): the brief's `cost_basis` (default `quoted`) gates section 7's shape — UNLIKE `orientation`/`customer_kind`, this key changes section content, not just presentation:
    - **`quoted`** (default): fill section 7 exactly as documented in step 7 below — unchanged pre-#840 behavior. Every priced line needs a sourceable basis (planning range, vendor list price, a `refs/` quote).
    - **`estimated`**: fill the SAME three tables (BOM, labor estimate, project total), but caption every subtotal and the intro/notes prose as an estimate — e.g. "Materials subtotal (estimated)" and an explicit "ESTIMATE, NOT A QUOTE" note stating the estimate basis (comparable prior work, engineering judgment, a named estimator). Do NOT invent a vendor quote or list price to satisfy the `quoted` shape — an unsourced-but-labelled estimate is the correct output for this mode.
    - **`none`**: do NOT fill the BOM/labor/project-total tables at all. Instead write a short "Cost Basis" section (the template's else-branch) stating plainly that this proposal has no hardware BOM and no vendor quotes, and move any commercial/budget terms into Open Decisions (§10) as an unresolved question rather than a priced line item.
    See `templates/proposal.tex.j2` §"7. Bill of Materials / Cost Basis" for the exact gated template shape and `rubric.md` §"Dim 6 — `cost_basis` calibration" for how the reviewer scores each mode.
7. **Priced tables** (critical for this skill when `cost_basis` is `quoted` or `estimated`): Section 7 is where the proposal lives or dies on cost credibility. Pre-wire all three tables — the multi-section BOM (`\multicolumn{4}{@{}l}{\textbf{...}}` section headers, `\addlinespace` between groups, `Item | Qty | Unit | Total` columns, `\toprule`/`\midrule`/`\bottomrule`, a bold **Materials subtotal** row), the **Labor estimate** (`Task | Hours | Cost`, bold subtotal), and the **Project total** (Materials + Labor → bold Total). Under `cost_basis: quoted`, every priced line must have a sourceable basis (planning range, vendor list price, quote) — the auditor walks every line for arithmetic and sourceability. Under `cost_basis: estimated`, every priced line must instead be captioned as an estimate with a stated estimate basis (see step 6b) — the auditor still walks the arithmetic but does not require vendor sourceability. Under `cost_basis: none`, this step does not apply — see step 6b's `none` branch instead. See `assets/figure-conventions.md` for the priced-table conventions.
