...
- `anvil-deck-narrative` / `anvil-deck-market` / `anvil-deck-design` /
  `anvil-deck-economics` / `anvil-deck-vision` — deck-skill specialists
  owning specific rubric-dim groups (1+7, 3+4, 8, 10) or the six-dimension
  rendered-only vision-rubric subset (`anvil-deck-vision`; #952).
...
**Out of v0 scope** (re-verified 2026-08-13; see `anvil/agents/README.md`
"Deferred scope" for the full itemized list):
- Specialty / non-lifecycle agents other than the deck fan-out — `*-perspective`,
  `*-imagegen`, ip-uspto §101/§112/claims/etc., memo helpers,
  proposal-synthesizer remain unshipped. (`anvil-deck-vision` and
  `anvil-deck-economics` have since shipped — #952.)
- Bridge tool agents (`anvil-project-migrate`, `anvil-rubric-rebackport`) —
  still unshipped; one-shot human-invocable, low fan-out benefit.
- Daemon / tmux orchestration analog of `loom-daemon` — still unshipped;
  no tracked follow-up issue exists yet.
