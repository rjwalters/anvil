...
- **Explain before you name.** Per `anvil/lib/snippets/plain_language.md` (issue #1042), introduce a technical or industry term only when it earns its place and state the idea in ordinary language first — on the slide body and in `speaker-notes.md`. An investor hearing an unexplained acronym mid-pitch cannot pause to look it up; a slide that briefly frames a term in plain language before naming it is not bloat against dim 9 *Rhetorical economy* (owned by `deck-narrative`) — a term dropped without explanation is a comprehension gap, not economy.
