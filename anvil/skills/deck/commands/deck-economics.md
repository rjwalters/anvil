...
6. **Identify critical flags**:
   - **`Fabricated traction`**: the critic owns dim 10 numbers' BRIEF cross-check the same way `deck-review` did pre-#551. Any number on the model / pricing / unit-economics / financials slides that is not attested in `BRIEF.md` is a `Fabricated traction` critical flag. This boundary is load-bearing: it preserves the existing four-flag set's exact semantics. Examples that trip the flag: a "$120K ARPU" on Slide 9 that doesn't appear in the brief; a "60% rev-share" on Slide 10 that the brief doesn't attest; a "8% attach rate" on Slide 11 that the brief doesn't ground.
   - **`Incoherent or absent business model`** (post-#552 — the dedicated business-model critical flag introduced in sibling issue #552): the structural-coherence escalation for dim 10. Three trigger disjuncts per the #552 prose: (a) no revenue mechanic stated on the slide; (b) internally contradictory unit economics (e.g., contribution margin asserted at 70% but the price–cost trace yields 30%); (c) counterparty-rejecting terms (e.g., a rev-share split that, on the published comparable, the counterparty would reject — pricing-gravity violation). **If #552 has NOT landed at #551 build time**: the dedicated flag does not yet exist; this critic does NOT raise it. The `Fabricated traction` flag still covers fabricated-number failures via the BRIEF cross-check above. Once #552 lands, this critic becomes the **primary** raiser of the new flag, with `deck-review` as fallback.
