...
5. **Render matplotlib charts**:
   ...
   **5b. Figure legibility preflight** (issue #563). Once all figures have been rendered (mermaid + matplotlib), run the deterministic legibility gate to catch figures that are *present and well-formed yet illegible at the slide's rendered height*. This is the **cheap mechanical pre-flight before the expensive VLM `deck-design` critic** — same principle as the `slide-content-overflow` lint and the `auto_shrink_detector`.
   ...
   The intrinsic source font height per diagram type lives in `Geometry.intrinsic_text_h_px_by_diagram_type` (mermaid: 18 px after issue #563's theme update; matplotlib: 14 px, matplotlib's default 10 pt axis label at its own stock `savefig.dpi` of 100).

   **Resolution correction (issue #904).** Those constants are heights in *source PNG pixels*, so for a renderer that lays text out in points and rasterizes at a chosen resolution they are only meaningful relative to the DPI they were calibrated at. matplotlib is exactly that: `anvil/lib/figures/anvil.mplstyle` pins `savefig.dpi: 200`, so a default 10 pt label is ~28 px, not 14. Treating 14 px as resolution-independent understated every matplotlib figure's glyph height ~2x and fired `error` on demonstrably legible charts. The gate therefore also reads the PNG's declared resolution from its optional `pHYs` chunk (matplotlib writes it; stdlib `struct.unpack` again, still no Pillow) and rescales the constant by `declared_dpi / reference_dpi` per `Geometry.reference_dpi_by_diagram_type`. Because the figure's intrinsic pixel height grows with DPI by the same factor, the estimate is DPI-invariant — the same physical chart at 72, 100 or 200 DPI scores identically. Mermaid is deliberately **not** rescaled: `mmdc`'s `--scale 2` is a device-pixel ratio rather than a DPI, and Puppeteer emits no `pHYs` chunk at all, so the 18 px constant stays as calibrated in #563. A PNG that declares no resolution keeps its type's calibrated constant (the conservative direction — under-estimating glyph height over-flags rather than under-flags).

   A future image-measurement implementation (Option 2 from the curator's plan — numpy/Pillow connected-component analysis under a `[legibility_lint]` extra) would replace the type-based lookup with measured text bounding-box heights; the public API (`lint_figures(deck_md, figures_dir, geometry=...)`) is the documented escalation seam.
