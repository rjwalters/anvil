...
Total ownership: 19/49. Other dimensions are scored by specialist critics (`deck-narrative` for 1+7+9, `deck-market` for 3+4, `deck-design` for 8, `deck-economics` for 10) and are left `null` in `_summary.md`. Note: post-#357, `deck-narrative` owns dim 9 *Rhetorical economy* in addition to dims 1 and 7 — the arc/ask critic's natural turf includes "could a busy investor extract the ask in 90 seconds?". `deck-narrative` scores dim 9 against the plain-language-first distinction between padding and patient teaching in `anvil/lib/snippets/plain_language.md` and `anvil/lib/snippets/rubric.md` §"Dim 9 — teaching-oriented length is not padding" (issue #1042) — a slide that briefly explains a term in ordinary language before naming it is not a deduction target; this aggregator does not re-score dim 9, but reads `deck-narrative`'s `_summary.md` contribution for the aggregated verdict. Post-#551, `deck-economics` owns dim 10 *Business-model & unit-economics credibility* as primary, with `deck-review` retained as fallback (parallel to how dims 3 / 4 live in `deck-market`'s hot path with `deck-review` as the fallback).
...
5c. **Run silent-Marp-auto-shrink lint (post-render, optional)** — issue #102 / #100b / #562:
   - Invoke `anvil.skills.deck.lib.auto_shrink_detector`'s `detect_auto_shrink(<thread>.{N}/deck.pdf, <thread>.{N}/deck.md)` as a Python import — NOT as a filesystem path. The canonical consumer invocation is `uv run --project .anvil python -c "from anvil.skills.deck.lib.auto_shrink_detector import detect_auto_shrink; ..."`. The module resolves through the importable mirror at `.anvil/anvil/skills/deck/lib/auto_shrink_detector.py` (post-#230). The detector reads the rendered PNGs (reuses `<thread>.{N}.vision/slides/` if the vision critic already populated it; otherwise renders fresh via `anvil.lib.render.render_pdf_to_pngs`), computes a per-page content bbox by sampling the background from corner patches and thresholding pixel diffs, classifies each slide by `<!-- _class: ... -->` directive (default `content`), and applies a **two-of-three composite** flag rule over peer-relative signals (issue #562): (a) bottom margin `> 1.5 × class_median` AND `> 0.18`; (b) top margin `> 1.5 × class_median` AND `> 0.10`; (c) content-area `< 0.75 × class_median`. A page is flagged when at least 2 of the 3 signals fire — the two-of-three quorum catches Marp's fit-to-scale shrink mode (bbox shrunk on all sides simultaneously) even when the bottom-margin signal alone sits near the class median. Singleton-class slides (typically one `title`, one `ask`) are recorded as skipped with a reason — never flagged (the D4 tautology contract; see the singleton-class carve-out bullet below, issue #965).
   - **Why a post-render check is necessary — the unified-gate contract** (issue #562): the two pre-flight lints (5b source-side `marp_lint` and 5c post-render `auto_shrink_detector`) constitute **one trustworthy gate** whose `advance:false` matches what a human reviewer will flag in the rendered PDF. The source-side check is now sizing-aware (issue #562 / `_image_cost_units` parses Marp's `bg`, `h:NNNpx`, `h:N%`, and `w:N` image keywords so image-bearing slides aren't falsely flagged for overflow on `bg right:N%` panels or `h:NNNpx` clamped figures); the post-render check is now composite-signal-aware (issue #562 / two-of-three quorum over bottom margin, top margin, and content area so Marp fit-to-scale shrink — the mode that left the bottom margin near class median but visibly shrunk the rest of the bbox — fires correctly). Reviewers should NOT hand-confirm against the rendered PDF: an `advance:false` from this gate means a real overflow / fit-shrink that a human will see. `deck-vision` v1 `vertical_overflow` remains the qualitative VLM companion (one API call per slide) but is no longer the load-bearing disambiguator — the deterministic gate stands on its own. **One documented carve-out (issue #965)**: on a slide whose `_class:` is a singleton, the post-render half of the gate structurally cannot weigh in, so the pair does NOT stand on its own there and `deck-vision` v1 is promoted back to load-bearing disambiguator for that one slide. See the next two bullets.
   - **Singleton-class carve-out — the one hole in the unified gate, and its named resolution (issue #965)**:
     - **The hole.** The peer-relative rule needs `thresholds.min_peers_per_class` (default 3) pages in a class to compute a median. A class with fewer pages is recorded in `AutoShrinkResult.skipped_classes` and **never flagged** — the deliberate D4 contract ("auto-shrink against itself is a tautology"), pinned by `TestF3_SingletonClassNeverFlagged` in `anvil/skills/deck/tests/test_auto_shrink_detector.py`. That contract is correct and unchanged. Its consequence is that when the step-5b source-side `slide-content-overflow` lint raises an **error** on a slide whose class is a singleton — the `title` slide is the canary shape, since image + subtitle + kicker + footer is the densest single slide in most decks — step 5c can neither confirm nor refute it. Note this is NOT the #622 load-bearing-skip case: that path fires when the WHOLE detector skipped (missing deps / missing PDF). Here the detector ran fine and skipped one class.
     - **Name the intersection; do not let it disappear.** Compute `uncrosscheckable_singletons`: one entry `{ "slide": <int>, "class_name": "<cls>", "source_side_rule": "slide-content-overflow" }` for every slide S where (a) S's class — per the same `<!-- _class: ... -->` classification the detector uses, defaulting to `content` — is a key in `AutoShrinkResult.skipped_classes`, AND (b) step 5b reported at least one `severity="error"` finding on S. Cache the list, write it to `_summary.md.lint.auto_shrink.uncrosscheckable_singletons` (step 9), and emit the named `findings.md` subsection (step 11). The list is `[]` in the common case and the rest of the review is then byte-identical to pre-#965 behavior.
     - **This is NOT a new gate.** `lint_critical_flag` is computed exactly as before — the source-side error already forced `advance:false` and still does. `uncrosscheckable_singletons` adds no error, no warning, and no critical-flag category; it is a routing hint that tells `deck-revise` which errors have no deterministic post-render corroboration.
     - **The sanctioned resolution path is `deck-vision` v1 `vertical_overflow` — never a manual look at the PDF.** Per `commands/deck-vision.md`, v1 is "the deeper companion to `marp_lint`'s slide-content-overflow rule": it is scored per-slide from the rendered PNGs by a VLM, it is an automated critic pass rather than a human hand-confirm, and unlike the pixel-bbox detector it is not defeated by a background image (see the next bullet). A clean v1 verdict on the slide is therefore the justification that licenses the per-slide escape hatch `<!-- anvil-lint-disable: slide-content-overflow -->`; a v1 finding on the slide confirms the source-side error and the slide must be fixed. **The reviewer does not apply the escape hatch** — reviewers never edit `deck.md`. It names the path here; `commands/deck-revise.md` step 7c executes it and records the citation.
     - **Surface whatever the vision sibling already says.** If `<thread>.{N}.vision/_review.json` exists when this reviewer runs, read it and state in the `findings.md` entry whether v1 `vertical_overflow` scored a finding on the affected slide, so the reviser does not re-derive it. If the sibling does not exist, say so and recommend running `deck-vision` on `<thread>.{N}` before the next `deck-revise`. Do NOT invent a v1 verdict from the markdown source — an unread sibling is reported as unread.
   - **Why not an absolute-margin floor for singleton classes? (design decision, issue #965)**: adding a peerless absolute-floor signal to the detector — "flag when `bottom_margin_norm` exceeds a fixed conservative constant, regardless of class size" — was considered and **rejected**. Two reasons, both specific to the classes that are actually singletons in this skill:
     1. **The shipped singleton classes are vertically centred by design.** `anvil/skills/deck/assets/anvil-deck.css` declares both `section.title` and `section.ask` as `display: flex; flex-direction: column; justify-content: center`. A large bottom margin on those slides is the intended layout, not shrink evidence. A floor conservative enough not to flag every healthy title slide is too high to catch anything; a floor low enough to catch real shrink flags the healthy case. Peer comparison is what supplies the missing "what should this class look like?" baseline, and by construction a singleton has none.
     2. **On the exact slide shape this issue is about, the bbox is saturated.** The motivating idiom is a title slide carrying an image (`bg`, `bg right:N%`, `bg left:N%`). A background or panel image paints non-background pixels across the full slide height, so `_content_bbox`'s ink-extent bbox spans the whole page and every margin signal reads ~0. The check would return "clean" for structural reasons rather than evidentiary ones — a gate that answers "clean" while blind is worse than one that honestly says "I cannot see this class."
     The peer-relative composite rule remains the only sound use of this pixel primitive. The gap is closed by naming a cross-check that CAN see these slides (`deck-vision` v1 reads the rendered PNG with a VLM and is not fooled by a background image), not by adding a deterministic signal that cannot.
...
5d. **Run deck↔memo parity lint (Phase A, warning-only)** — issue #200:
   ...
   - **On `ImportError` / `ModuleNotFoundError`** (the `anvil.skills.deck.lib.parity_lint` module itself is not importable — distinct from the no-memo-sibling skip above, which is a runtime return value): the reviewer MUST NOT silently skip. Record a single info-level entry in `findings.md` § Parity-lint findings of the form `lint=unavailable (module not importable: <ImportError message>)` and set the `_summary.md.lint.deck_memo_parity` block to `{"ran": false, "memo_sibling": null, "reason": "module not importable: <ImportError message>", "warnings": 0, "infos": 0, "only_in_memo": [], "only_in_deck": [], "only_in_memo_economic": [], "only_in_memo_quarantined": [], "quarantine_violations": [], "warnings_by_token": [], "infos_by_token": []}`. The verdict proceeds; parity is observational-only in v0 (Phase A) so this never contributes to `lint_critical_flag` regardless — but the operator sees WHY the check did not fire (issue #375).
   - The call returns a `LintResult` with `warnings: list[Finding]`, `infos: list[Finding]`, `skipped: bool`, `reason: str | None`, and `memo_sibling: str | None`. Each `Finding` has `line` (1-based source line in whichever body the token appeared), `rule="deck_memo_parity"`, `severity="warning"` (or `"info"` if suppressed), `message` (a human-readable diagnostic naming the canary anchor), `token` (the normalized token surface form), and `side` (`"only_in_memo"`, `"only_in_deck"`, `"only_in_memo_economic"`, `"only_in_memo_quarantined"`, or `"quarantine_violation"` — see the quarantine-surface bullet below for the latter two).
   ...
   - **BRIEF-quarantine surface — `quarantine:` reframe + reverse violation (issue #914)**: before a token is promoted to `only_in_memo_economic`, the classifier ALSO consults a **quarantine corpus** built from the project `BRIEF.md`'s `quarantine:` key (`anvil.lib.parity._extract_quarantine_corpus`, project root = the parent of the deck thread root — same discovery convention as `load_rubric_overrides_for_slug` in step 5e). A project author lists literal figures/ranges a hard rule forbids porting (e.g. `quarantine: ["$400M", "20-40%"]`) — distinct from the free-form `hard_rules:` prose, this is the machine-matchable token surface. **Two effects, both additive**: (1) a quarantined `only_in_memo` token is **excluded from `only_in_memo_economic` promotion** and instead surfaces under `side="only_in_memo_quarantined"` with the message "correctly absent per BRIEF hard rule — do not port" — this reframes it away from the "should you port this?" decision framing the economic promotion carries, since the omission is deliberate and rule-mandated, not accidental drift; (2) a quarantined token found **anywhere in the deck body** raises a NEW `side="quarantine_violation"` finding — a real hard-rule violation this lint did not previously detect (the canary: a deck speaker note picking up a memo's quarantined `20-40%` unit-economics spread). **Why**: pre-#914, this lint's sharpest channel (the economic promotion) was actively recommending the hard-rule violation the BRIEF forbids for a deliberately-quarantined figure. **Invariants**: `set(only_in_memo_quarantined) ⊆ set(only_in_memo)`; a quarantined token is NEVER also `only_in_memo_economic` (quarantine reframing takes precedence over economic promotion for the same token); `quarantine_violation` findings are independent of `only_in_deck` — they fire on ANY deck-body presence, including a token shared with the memo; an empty/absent BRIEF `quarantine:` key makes this a no-op, byte-identical to pre-#914 behavior. **Derived-forms caveat (out of scope for this pass)**: the quarantine corpus matches literal tokens only — a quarantined percentage reappearing in the deck as its arithmetic derivation (e.g. a quarantined `20-40%` rate surfacing as a derived `$360-720/yr` dollar range) is NOT detected. A reviser porting content near a quarantined figure MUST manually check for derived forms.
   - Cache the `LintResult` for the `_summary.md` and `findings.md` writes below. **Do NOT OR this lint's findings into `lint_critical_flag`** — Phase A is observational only. The `only_in_memo_economic` and `only_in_memo_quarantined` subsets, and the `quarantine_violation` class, are ALL observational in v0 (warning severity, non-gating); the reviser-side framing (`deck-revise.md` step 7b) is what makes the subsets load-bearing in practice.
...
5f. **Run post-render text-layer-completeness lint (post-render)** — issue #983:
   - Invoke `anvil.skills.deck.lib.text_layer_completeness`'s `check_text_layer_completeness(<thread>.{N}/deck.pdf, <thread>.{N}/deck.md)` as a Python import — NOT as a filesystem path. The canonical consumer invocation is `uv run --project .anvil python -c "from anvil.skills.deck.lib.text_layer_completeness import check_text_layer_completeness; ..."`. The module resolves through the importable mirror at `.anvil/anvil/skills/deck/lib/text_layer_completeness.py` (post-#230).
   - **The third deterministic pre-flight gate, and why it is a distinct family member from 5b/5c.** The step-5b source-side `marp_lint` estimates vertical content cost from the markdown source before render; the step-5c post-render `auto_shrink_detector` compares each slide's rendered bbox against **peer-class medians**. Neither asks the one ground-truth question this gate asks: does the slide's last source line actually survive into its rendered PDF page's text layer? A caption pushed off a 720px slide's bottom edge answers "no" — and this check needs no peer class at all (unlike `auto_shrink_detector`, it works identically on a singleton `_class:` slide such as `title`), so it has no analog of the #965 singleton-carve-out gap.
   - **How it works**: for each slide, the module splits `deck.md` into per-slide source chunks by reusing `anvil.lib.marp_lint._split_slides` — the SAME `---`-based slide-splitting logic step 5b uses, so slide numbering stays byte-consistent across all three gates. It then walks each slide's source lines from the bottom up to find the last **checkable** line: blank lines and Marp/anvil directive comments (`<!-- _class: ... -->`, `<!-- anvil-lint-disable: ... -->`) are skipped, markdown decoration is stripped (image references removed entirely — alt text is not rendered as visible page text by the deck theme; link syntax unwrapped to its visible text; emphasis markers `*`/`_`/backtick dropped; leading list markers dropped), and the first surviving line that still carries alphanumeric content is the checked line. A slide with no checkable line (image-only or directive-only) is skipped — nothing to verify, and this does NOT count as a finding.
   - **PDF text extraction**: the module runs `pdftotext <deck.pdf> -` (poppler-utils) and splits the output on the form-feed byte poppler inserts after every page by default, yielding one text string per rendered page. Mirrors the `_which_pdftotext` / `_extract_pdf_text` precedent in `anvil/lib/render_gate.py` (the LaTeX-skill analog, ~line 826 / 840), including its Unicode-whitespace-normalization contract: poppler's `pdftotext` collapses Unicode `Zs`-category separator spaces (NBSP, thin space, narrow NBSP, …) to a plain ASCII space during extraction, so the module folds the same characters to ASCII on the *source* side before the containment check (`_normalize_whitespace`) — a source caption typed with a stray NBSP does not false-block.
   - **The comparison**: the normalized last-line text must appear (case-insensitively) as a substring of the normalized page text. Absence emits a `severity="error"` `Finding` (`rule="text-layer-completeness"`) naming the slide number, source line, and the exact missing text — the reviser does not need to open the rendered PDF to know what to fix.
   - **Escape hatch**: `<!-- anvil-lint-disable: text-layer-completeness -->` anywhere on a slide downgrades that slide's finding to `info` — the same whole-slide suppression shape `slide-content-overflow` uses (reuses `anvil.lib.marp_lint._collect_disabled_rules`, the generic rule-name-driven directive parser already shared across the `marp_lint` rule family). Use case: a deliberately partial off-slide decorative footer.
   - **Graceful-skip on missing `pdftotext`**: the module's first step calls its own `_which_pdftotext()` (a `shutil.which("pdftotext")` resolver, same shape as `render_gate.py`'s); if it returns `None`, the module returns `TextLayerCompletenessResult(skipped=True, reason="pdftotext not found on PATH (poppler-utils) — install poppler-utils to enable the text-layer-completeness gate.")` without raising. Record the skip as a `severity="info"` lint entry — the rest of `deck-review` proceeds normally (same pattern as the `auto_shrink` step 5c missing-deps skip).
   - **Graceful-skip on missing PDF**: if `deck.pdf` does not yet exist (the user hasn't run `deck-figures`), the module returns `TextLayerCompletenessResult(skipped=True, reason="deck.pdf not found at ... — run \`deck-figures\` before \`deck-review\` to render the PDF.")`. Record as an info-level skip; do not block.
   - **On `ImportError` / `ModuleNotFoundError`** (the `anvil.skills.deck.lib.text_layer_completeness` module itself is not importable — distinct from the graceful-skips above, which are runtime return values from a module that DID import): the reviewer MUST NOT silently skip. Record a single info-level entry in `findings.md` § Text-layer-completeness findings of the form `lint=unavailable (module not importable: <ImportError message>)` and set the `_summary.md.lint.text_layer_completeness` block to `{"ran": false, "skipped": true, "reason": "module not importable: <ImportError message>", "errors": 0, "warnings": 0, "infos": 0, "findings": []}`. The verdict proceeds without this gate contributing to `lint_critical_flag` — but the operator sees WHY the check did not fire (issue #375's convention).
   - The call returns a `TextLayerCompletenessResult` with `findings: list[Finding]` (the same `anvil.lib.marp_lint.Finding` type `figure_legibility.py` reuses — `slide`, `line`, `rule`, `severity`, `message`), `skipped: bool`, and `reason: str | None`.
   - Cache the `TextLayerCompletenessResult` for the `_summary.md` and `findings.md` writes below. Errors from this gate OR into `lint_critical_flag` alongside the `marp_lint` (step 5b) and `auto_shrink_detector` (step 5c) errors — `lint_critical_flag = (marp_lint.errors > 0) or (auto_shrink.errors > 0) or (text_layer_completeness.errors > 0)`.
6. **Score owned dimensions**:
   ...
   - **Dim 5 + Dim 6 refs back-check sub-step** (issue #166): enumerate `<thread>/refs/` and identify the **source-of-truth materials** present per SKILL.md §"Source-of-truth materials" (files named for their content — `cv.pdf`, `cv.md`, `founder-bio.md`, `transcript-*.md`, `filing-*.pdf`, `paper-*.pdf`, `email-loi-*.md` / `loi-*.md`, `quote-*.md`, `image-*.{png,jpg}`). The back-check applies to source-of-truth materials only; generic reference material (decks, transcripts the brief did not name as a source-of-truth, financial spreadsheets used only as drafter context) is out of scope for this sub-step and stays under the existing BRIEF-only cross-check. For each source-of-truth refs-document **type** present that is on-topic for dim 5 (traction-bearing files: LOIs, quotes, customer letters, traction-cited filings) or dim 6 (team-bearing files: CVs, founder bios, prior-outcome filings), pick at least one load-bearing claim in `deck.md` whose evidentiary basis is the document's subject and write a `comments.md` entry of the form:
     ...
     - **`CONTRADICTED`** — refs/ document contains a passage that **directly contradicts** the claim (e.g., Slide 10 says "Founder: 15+ years at Foundry Trust" but `refs/cv.pdf` shows "Foundry Trust 2018-2023" — five years, not fifteen); **2-point deduction** on the relevant dim AND a **critical-flag candidate**. For traction-bearing claims (dim 5), a CONTRADICTED verdict in a load-bearing context escalates to the existing **critical flag 1 (Fabricated traction)** — the underlying source-of-truth document shows the traction figure is not what the slide says. For team-bearing claims (dim 6), a CONTRADICTED verdict escalates to the existing **critical flag 2 (Fabricated team credentials)** — same canary failure mode the existing flag exists to catch (Foundry 15+ years founder bio error from issue #166's body propagated through TWO deck versions because no reviewer back-checked against the CV). No new flag is needed; the existing flags 1 and 2 are the natural escalation path.
     ...
     **Finding hygiene — resolve locations before writing them (issue #749).** When a `comments.md` entry names a specific `refs/<file>` location, passage, or verdict, the reviewer MUST have actually looked it up before writing it — the back-check discipline above applies to the reviewer's OWN comment prose, not just to the deck's claims. Do NOT name a `refs/<file>` anchor from memory or from a plausible-sounding guess. A finding that mis-cites its own remediation is a defective finding: the next reviser has every incentive to trust a finding's own citation rather than re-derive it under iteration pressure (see `commands/deck-revise.md` step 7 §"Findings are leads, not evidence"). This closes the review-sidecar leg of the two-generation citation-error chain documented in issue #749.
...
9. **Write `_summary.md`** as a JSON-in-markdown scorecard with a top-level `rubric` block (issue #346) sibling to `lint`. The `lint` block is populated from the cached `LintResult` returned by step 5b; the `rubric` block carries the rubric the reviewer scored against so a downstream consumer aggregating across versions does not need to walk back to `anvil/skills/deck/rubric.md` (which may have changed between v3 and v5 of a long thread that spanned the `/40 → /44 → /49` migrations):
   ...
   ```json
   {
     ...
     "lint": {
       ...
       "auto_shrink": {
         ...
         "uncrosscheckable_singletons": [
           { "slide": 1, "class_name": "title", "source_side_rule": "slide-content-overflow" }
         ]
       },
       "deck_memo_parity": {
         "ran": true,
       ...
       "text_layer_completeness": {
         ...
         "infos": 0,
         "findings": [
           { "slide": 6, "line": 41, "rule": "text-layer-completeness", "severity": "error", "message": "Slide 6's last source line is missing from its rendered PDF page's text layer: \"Figure 3: unaudited pilot conversion data, Q2 2026.\"..." }
         ]
       }
     },
     "rubric_overrides": {
   ...
   - The `lint.auto_shrink.uncrosscheckable_singletons` field (issue #965) is the singleton-class carve-out list defined in step 5c: one `{slide, class_name, source_side_rule}` entry per slide that carries a step-5b `severity="error"` finding AND whose `_class:` is a key in `skipped_classes` (too few peers for a median). It is `[]` whenever the intersection is empty — which is the common case — and MUST be present even then, so a reader can distinguish "no uncrosscheckable errors" from "this reviewer predates #965." **Observational only**: the field does NOT contribute to `critical_flag` (the underlying source-side error already does, unchanged) and does NOT alter `advance`. Its consumer is `commands/deck-revise.md` step 7c, which uses it to route those errors to the `deck-vision` v1 `vertical_overflow` cross-check instead of leaving the reviser with a blocking error and no sanctioned resolution.
   - The `deck_memo_parity` block is populated from the cached `LintResult` returned by step 5d. When the lint skipped (no memo sibling discoverable), the block shape is `{ "ran": false, "memo_sibling": null, "reason": "no memo sibling found at portfolio root; parity check inactive", "warnings": 0, "infos": 0, "only_in_memo": [], "only_in_deck": [], "only_in_memo_economic": [], "only_in_memo_quarantined": [], "quarantine_violations": [], "warnings_by_token": [], "infos_by_token": [] }`. The `ran: false` skip path MUST be recorded — the operator should see WHY the parity check did not fire (same skip-reason convention as `auto_shrink`). The `only_in_memo_economic` field (issue #553) is a top-level subset of `only_in_memo` — see the load-bearingness filter prose in step 5d for the classifier contract. The `only_in_memo_quarantined` field (a top-level subset of `only_in_memo`) and the `quarantine_violations` field (independent of `only_in_deck`) are the BRIEF-quarantine surface (issue #914) — see the quarantine-surface bullet in step 5d.
   - **`deck_memo_parity` findings do NOT contribute to `critical_flag` in v0** (Phase A ships warning-only) — this includes the `only_in_memo_economic` subset (issue #553) and the `only_in_memo_quarantined` / `quarantine_violations` classes (issue #914), all of which ship at warning severity and are observational. The block is observational: it surfaces drift in `findings.md` and the operator's revision priorities, but the `critical_flag` boolean is computed exactly as before (`marp_lint.errors > 0` OR `auto_shrink.errors > 0` OR `text_layer_completeness.errors > 0`). Phase B promotion to error severity (and therefore `advance: false`-gating) is a separate decision deferred per issue #200's Phase A / Phase B contract. The reviser-side framing (`deck-revise.md`) is what makes the economic subset load-bearing in practice — the reviser is taught to consult `only_in_memo_economic` BEFORE bulk-dismissing the broader `only_in_memo` set, and to treat `quarantine_violations` as an actionable hard-rule fix, not an ordinary drift item.
   - The `text_layer_completeness` block (issue #983) is populated from the cached `TextLayerCompletenessResult` returned by step 5f. When the gate skipped (missing `pdftotext` or `deck.pdf` not yet rendered), the block shape is `{ "ran": false, "skipped": true, "reason": "...", "errors": 0, "warnings": 0, "infos": 0, "findings": [] }` — the `ran: false` skip path MUST be recorded (same skip-reason convention as `auto_shrink`). Unlike `auto_shrink`, this gate has no per-class median/skipped-class machinery — it needs no peer comparison, so it carries no analog of `uncrosscheckable_singletons`.
   - When `lint.errors > 0` (sum of source-side `errors`, `auto_shrink.errors`, AND `text_layer_completeness.errors`), set `critical_flag: true` and append entries to `critical_flag_notes`:
     ...
     - text-layer completeness: `{ "type": "text_layer_incompleteness", "slide_refs": ["Slide 6"], "justification": "Slide 6's last source line is missing from its rendered PDF page's text layer — the line (a caption) was likely clipped off the slide's bottom edge. See lint.text_layer_completeness.findings for the per-slide breakdown." }`.
     All three flag categories live under the open-ended critical-flag bucket (per `rubric.md`'s "any other issue a sophisticated investor would catch and disqualify on" slot — the extension beyond the five standing flags) — a deck whose slides visibly read smaller than peer slides reads as unfinished, and a deck with a clipped line (in the observed real-thread case, a legally load-bearing notice) is not send-ready.
...
11. **Write `findings.md`** as itemized findings (deck-specific format the reviser uses for aggregation):
    ...
    **Singleton-class overflow errors — uncrosscheckable, with a named resolution (issue #965).** Distinct from the whole-detector skip above: the detector RAN, but skipped one `_class:` for want of peers (`min_peers_per_class`, default 3) while the step-5b source-side lint raised an error on a slide in that class. Emit this subsection whenever the cached `uncrosscheckable_singletons` list from step 5c is non-empty (omit it entirely when the list is empty — no noise on the common case):

    ```
    ## Singleton-class overflow errors (uncrosscheckable by the post-render detector)

    One entry per slide in `_summary.md.lint.auto_shrink.uncrosscheckable_singletons`. These slides carry a source-side `slide-content-overflow` **error** (which blocks advance, unchanged) that the post-render auto-shrink detector structurally cannot confirm or refute: their `_class:` has too few pages in this deck for a peer median, and the detector never flags a singleton class by design (`auto-shrink-fit-compression` is a peer-relative rule; an absolute-margin floor was considered and rejected in #965 — see `deck-review.md` step 5c). Resolve via `deck-vision` v1 `vertical_overflow`, NOT by opening the PDF by hand. `deck-revise.md` step 7c carries the reviser's playbook.

    1. **[error, uncrosscheckable]** Slide 1 (class 'title'): source-side `slide-content-overflow` error stands; the post-render detector skipped class 'title' (only 1 page(s) — minimum 3 required for a peer-median comparison). Vision cross-check: `acme-seed.1.vision/_review.json` — v1 `vertical_overflow` scored 5/5 with no finding on Slide 1, so the source-side error is **refuted** by the rendered-side critic; `deck-revise` may apply the per-slide escape hatch with that citation. Otherwise: trim per the source-side lint's suggested fix.
    ```

    Vision-sibling wording depends on what is actually on disk when this reviewer runs — never invent a v1 verdict:

    - Sibling present, v1 clean on the slide: `Vision cross-check: <thread>.{N}.vision/_review.json — v1 vertical_overflow reports no finding on Slide S (source-side error refuted; escape hatch justified for deck-revise).`
    - Sibling present, v1 reports a finding on the slide: `Vision cross-check: <thread>.{N}.vision/_review.json — v1 vertical_overflow reports "<finding text>" on Slide S (source-side error CONFIRMED; fix the slide, escape hatch NOT justified).`
    - Sibling absent (or present without a v1 score): `Vision cross-check: not available — no <thread>.{N}.vision/ sibling at this version. Run deck-vision on <thread>.{N} before the next deck-revise, or fix the slide per the source-side lint. The escape hatch is NOT justified without a rendered-side cross-check.`

    A third lint block (issue #983) sits under its own subsection: the post-render text-layer-completeness gate (step 5f). The subsection is **always present** (emitted even when the gate skipped) so the operator sees WHY the check did or did not fire. Errors block advance via the lint critical flag — a slide's last source line clipped off the render is the same "unfinished" verdict a human reviewer would reach:

    ```
    ## Text-layer-completeness findings (post-render, optional)

    Each entry comes from the `text-layer-completeness` gate (step 5f). For every slide, the gate confirms the slide's last checkable source line is present in that slide's rendered PDF page text (via `pdftotext`). Errors block advance via the lint critical flag — a missing line usually means a caption (or other trailing content) was pushed past the slide's bottom edge and clipped at render time.

    1. **[error]** Slide 6 (line 41): Slide 6's last source line is missing from its rendered PDF page's text layer: "Figure 3: unaudited pilot conversion data, Q2 2026." Suggested fix: constrain the figure (e.g. an explicit `h:NNNpx` clamp) or trim/move the caption so it fits; open the rendered PDF page to confirm before deciding. Suppress a deliberately off-slide line with `<!-- anvil-lint-disable: text-layer-completeness -->`.
    ```

    Or, when the gate was skipped (missing `pdftotext` on `PATH`, or `deck.pdf` not yet rendered):

    ```
    ## Text-layer-completeness findings (post-render, optional)

    _Skipped: <reason from TextLayerCompletenessResult.reason>._
    ```

    Or, when the gate ran cleanly (every slide's last line survived into its rendered page):

    ```
    ## Text-layer-completeness findings (post-render, optional)

    _No text-layer-completeness findings._
    ```

    A fourth lint block (issue #200, Phase A) sits under its own subsection. The parity lint is **always present** (subsection emitted even when the lint skipped) so the operator sees WHY the check did or did not fire. v0 ships warning-only — entries surface drift but do NOT block advance:
...
12. **Aggregate verdict** (this reviewer is the canonical verdict author):
    ...
    - For critical flag, take logical OR of all critic flags **including all three pre-flight lints** (source-side `marp_lint` from step 5b, post-render `auto_shrink_detector` from step 5c, AND post-render `text_layer_completeness` from step 5f). If this `_summary.md`'s own `lint.errors > 0` OR `lint.auto_shrink.errors > 0` OR `lint.text_layer_completeness.errors > 0`, the aggregated critical flag is true regardless of any other critic.
...
- **Never resolve an overflow error by looking at the PDF yourself.** The two pre-flight lints are the gate (step 5c §"the unified-gate contract"), and their one structural hole — a source-side `slide-content-overflow` error on a singleton `_class:` slide, which the post-render detector cannot peer-compare — has a named automated resolution: `deck-vision` v1 `vertical_overflow`. Record the intersection in `uncrosscheckable_singletons` and name the vision cross-check; do not substitute your own read of the render, and do not silently drop the error into the general lint list where the reviser will treat it as ordinary.
