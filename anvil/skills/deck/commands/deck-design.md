...
7b. **Additive-ness pass (generative-eligible only; issue #547)** — Imagine-then-review additive-ness gate. Resolve the thread's effective `imagery_policy` via `anvil/skills/deck/lib/imagegen.py::resolve_effective_imagery_policy(thread_dir, portfolio_path)` — the single source of truth for the resolution order (`<thread>/BRIEF.md` frontmatter `imagery_policy:` ∪ `.anvil/config.json` `deck.imagegen.default_policy` ∪ built-in default; issue #547). Pass the **thread root** (`<portfolio>/<thread>/`), NOT the project root — the project-level `BRIEF.md` one directory up is a distinct file (frontmatter `documents:` list) that never carries `imagery_policy`, and reading it instead of the thread-level file is the exact drift this function exists to prevent (issue #984). When the effective policy is `generative-eligible`, perform a per-slot additive-ness judgment on every generative image. Use the helper at `anvil/skills/deck/lib/imagegen_additive.py::gate_should_run` (passing the resolved `effective_policy` string — it does not re-resolve the policy itself) to decide whether the pass runs; use `collect_generative_slots` to enumerate the per-slot input bundles (slot name, PNG path, journal entry). For each slot:
