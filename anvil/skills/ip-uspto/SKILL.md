...
- **The prior-art critic does NOT do its own patent search.** It scores only what is on disk in `<thread>/prior-art/`. Searching is a separate skill — `anvil:ip-search` — which the operator may run by hand, or wire in as the opt-in, off-by-default step 0 above (`prior_art_search` in `<thread>/.anvil.json`). Either way the search is a drafting aid, never an attorney clearance search, and renders no positioning verdict: dimension 5 stays the critic's.
