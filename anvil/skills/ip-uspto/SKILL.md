...
| `ip-uspto-prior-art <thread> [--search]` | prior-art critic (`--search` opts into the pre-search step, issue #958) | latest `<thread>.{N}/` + `<thread>/prior-art/**` (+ `anvil:ip-search` when opted in) | `<thread>.{N}.priorart/` |
