...
- **Vision findings often require fixing `figures/src/*.py` or inline mermaid blocks, not `deck.md`.** A vision finding flagging mathtext or palette on a chart label is a matplotlib-script fix under `figures/`; a finding flagging illegible axis labels is a DPI/figsize/font-size fix in the same script; a finding flagging an illegible or overflowing mermaid diagram requires editing the inline ```mermaid block in `deck.md`. Vertical-overflow on text-heavy slides remains a `deck.md` fix. The `slides-revise` command surfaces this guidance to the reviser explicitly (its D6 reviser-guidance note).
