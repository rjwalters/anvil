...
| 9 | **Rhetorical economy** | 4 | Talk-level anti-bloat (distinct from per-slide density dim 4): could the whole talk land in 30 minutes? Do slides 23–28 carry weight the talk needs? Could the same arc reach the takeaway in fewer beats? Owned by `slides-review` (source-side judgment); NOT scored by `slides-vision`. See `anvil/lib/snippets/plain_language.md` (issue #1042) for the expanded guidance this dimension also draws on: unexplained jargon deducts here, but words spent walking the audience from plain language to the precise term do not. |
