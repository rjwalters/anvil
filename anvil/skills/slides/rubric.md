...
| 9 | **Rhetorical economy** | 4 | Talk-level anti-bloat (distinct from per-slide density dim 4): could the whole talk land in 30 minutes? Do slides 23–28 carry weight the talk needs? Could the same arc reach the takeaway in fewer beats? Owned by `slides-review` (source-side judgment); NOT scored by `slides-vision`. |
