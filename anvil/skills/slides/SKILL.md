...
3. **Density check**: the rehearser counts note words for spoken-time estimation; sidecar files make this trivial. When notes carry a verbatim script (see `slides-rehearse`'s "Cues vs. Script sections"), splitting the file into a short `## Cues` section (timed) and a `## Script` section (leave-behind text, not timed) keeps the estimate meaningful.
...
**`slides-rehearse`** — *time-budget + density check.* Mechanical pass: counts words per slide, estimates spoken-time-per-slide (default heuristic: 90s base + 30s per non-trivial figure + 1.5s per word of presenter notes, capped at 3 min/slide for technical depth slides). If a notes file has a `## Cues` heading, only that section's words are timed (fall back to the full file otherwise) — see "Cues vs. Script sections" in `slides-rehearse` for the script-style-notes convention this recognizes. Emits `<thread>.{N}.rehearse/timing.md` with per-slide and aggregate estimates, density violations, and recommended cuts; a caveat is prepended when a majority of slides saturate the 180s cap, since the aggregate becomes low-information at that point. Deterministic-first (regex/wordcount) with LLM judgment only for "is this figure trivial or non-trivial?" classification. Feeds Dimensions 4 and 8 directly.
