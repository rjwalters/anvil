...
5. **Build the claim inventory**: walk `report.md` and enumerate every quantitative claim, numeric assertion, named-entity attribution, and citation. Record each in `findings.md` with columns:
   ...
   **Finding hygiene — resolve the `Cited source` cell before writing it (issue #749).** The `Cited source` and `Verified?` cells are the auditor's OWN claim about where evidence lives, not a transcription of what the drafter wrote — the auditor MUST have actually opened the named `refs/<file>` (or, for external URLs, fetched it) and located the supporting passage before writing `yes`/`partial` into a row. Do NOT mark a row `yes` on the strength of a plausible-sounding source name; a `findings.md` row that mis-cites its own basis is a defective finding, and the reviser is instructed to trust it (see `commands/report-revise.md` step 7 §"Findings are leads, not evidence") — a lazy audit row propagates its own error into the next `report.md` unverified. This is the audit-sidecar leg of the two-generation citation-error chain documented in issue #749.
...
10b. **Pending-marker terminal gate (deterministic, unconditional) — issue #841**: invoke `uv run --project .anvil python -m anvil.lib.pending_marker <thread>.{N}/ --write-review --body report.md` (same tool and invocation shape as `report-review` step 4f). This re-runs the deterministic `[PENDING <source>]` marker gate as the **terminal check** before `AUDITED` — genuinely useful independent of `report-review` when the auditor runs first, and as defense-in-depth when it has already run. `--write-review` re-writes `<thread>.{N}.pending/_review.json` (deterministic-regeneration carve-out — a later pass supersedes an earlier one), carrying the specially-resolved `pending_dependency` flag whenever any active marker remains. **The terminal gate is the tool's exit code / `pass` field, NOT a critical flag**: it is visible-but-non-blocking, so it does not tank the audit `pass` field the way an unsupported claim does — instead, per step 11 below, `AUDITED` additionally requires the gate to have passed. A well-formed marker is an honest disclosure, not a defect — do NOT add a corresponding `findings.md` row as a mismatch, and do NOT fabricate a value to clear it; note any outstanding marker in `verdict.md`'s "Outstanding dependencies" list instead. **Tooling-absent fail-open**: manually scan `report.md` for the literal marker shape and hold the terminal gate on any found marker without fabricating, recording the fallback with a one-line note.
11. **Compute pass/fail**: `pass = (no critical flags) AND (all quantitative claims verified or partial-with-acceptable-rationale)`.

    **Terminal-state gate for `AUDITED` (deterministic, separate from `pass` — issue #841)**: even when `pass == true` AND `report-review`'s `advance == true`, the thread does NOT reach `AUDITED` while step 10b's pending gate has not passed (an active `[PENDING <source>]` marker remains anywhere in `report.md`). The thread stays at `REVIEWED+AUDITED` with an "Outstanding dependencies" note in `verdict.md` naming the still-pending source(s) — the audit genuinely is not done until the value lands; never fabricate a value to force `AUDITED`.
12. **Write `verdict.md`** in the format specified in `rubric.md`:
    ...
    - **Outstanding dependencies (conditional — issue #841)**: when step 10b's pending gate did not pass, list each still-pending source label and state explicitly this holds `AUDITED`, is NOT a scored defect, and NOT a critical flag. Omitted entirely when the gate passed cleanly.
