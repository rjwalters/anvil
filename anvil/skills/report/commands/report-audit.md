...
- **Quantify your coverage.** Report in `verdict.md` exactly how many quantitative claims you audited (e.g., "audited 18/18 quantitative claims; 14 verified, 2 partial, 2 unsupported"). If the report contains a quantitative claim you could not verify (because the source is not in `refs/` and you cannot access it), flag it explicitly in `findings.md` with `Verified? = n/a — source not accessible to auditor` and recommend the reviser either provide the source or remove the claim.
