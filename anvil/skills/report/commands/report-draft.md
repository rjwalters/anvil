...
6b. **Apply the plain-language-first framework (issue #1042/#1044)**: throughout the sections drafted in step 6, state a finding or recommendation in ordinary language before naming any specialist term, and introduce a technical term only when it adds precision or lets the recipient connect the finding to a named prior engagement or standard — per `anvil/lib/snippets/plain_language.md` §"The 7-point framework" (points 1–4). Keep the plain restatement alongside the term once introduced (point 4); a recipient reading only the executive summary should not need the term to follow the argument.
7. **Apply recipient calibration**: use `voice_notes` and `confidentiality_class` from `_project.md` to set jargon level, tone, and any redaction posture. A `restricted` confidentiality class triggers a placeholder warning at the top of the cover page (`[RESTRICTED — DO NOT REDISTRIBUTE]`); the skill does NOT enforce write-location restrictions in v0 (see SKILL.md open question on confidentiality handling). Recipient jargon calibration (this step) and the plain-language-first framework (step 6b) compose: the framework sets the floor (a term earns its place only when it adds precision), and `voice_notes` further tunes how much specialist vocabulary above that floor a given recipient can absorb.
