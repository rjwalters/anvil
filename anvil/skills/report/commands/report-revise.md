...
10b. **Record the evidence-drift baseline (issue #857)**: invoke `uv run --project .anvil python -m anvil.lib.evidence_drift record <project>/<thread>/ <project>/<thread>.{N+1}/` — the same call `report-draft` step 10b makes, re-baselined against the new version dir. Every revision re-records the current BRIEF/refs mtimes so `report-review` always compares against the freshest version's baseline. Purely additive; never fails the revise. **Tooling-unavailable fallback**: when `uv` is not on `PATH`, **or this step's write is blocked** (e.g. a consumer-repo hook that blocks Bash-channel writes — issue #1018), skip with a one-line notice. Do **not** hand-type a snapshot into `_progress.json` as a substitute — a hand-transcribed mtime float loses precision and can produce a false-positive drift report later. The next successful draft/revise pass records the baseline instead.
...
- **Critical flags trump everything — with ONE explicit exception (issue #841).** Any critic-side critical flag MUST be addressed — failing to do so is a worse outcome than declining a stylistic suggestion. **The exception is the specially-resolved `pending_dependency` flag** (from the `<thread>.{N}.pending/` sibling): it is NOT a defect to fix in prose. NEVER fabricate a value to clear a `[PENDING <source>]` marker — the ONLY correct resolution is replacing it with the real value once it genuinely exists. Until then, carry the marker forward verbatim into `<thread>.{N+1}/report.md` and note in `changelog.md` that the dependency is still outstanding (`Resolution: pending — <source> not yet resolved; marker carried forward`). Inventing a plausible-looking figure to make the marker "go away" is the single most severe error available in a customer-facing report.
