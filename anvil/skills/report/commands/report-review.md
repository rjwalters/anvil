...
- **Evidence-drift snapshot** (issue #857): the `metadata.evidence_snapshot` baseline `report-draft`/`report-revise` recorded in `<thread>.{N}/_progress.json` at draft/revise completion time (`anvil/lib/evidence_drift.py::record_evidence_snapshot`). Step 4g compares this baseline against the CURRENT on-disk mtimes of `<project>/<thread>/BRIEF.md` and `<project>/<thread>/refs/**` to detect thread-root evidence that changed after this version was written. Purely advisory (see step 4g) — a thread with no recorded baseline (pre-#857) is treated as clean, never a false positive.
...
4f. **Run pending-marker gate (deterministic, unconditional) — issue #841**: invoke `uv run --project .anvil python -m anvil.lib.pending_marker <thread>.{N}/ --write-review --body report.md` (the module's `<slug>.md`/`main.tex` auto-detect does not match this skill's fixed `report.md` body filename, so `--body` is required here). This detects well-formed `[PENDING <source>]` / `[PENDING: <source>]` placeholders (see `anvil/lib/snippets/pending_marker.md`) — a drafter's honest disclosure that a specific number (a client-supplied figure not yet returned, an analysis still running) is not yet available. `--write-review` writes `<thread>.{N}.pending/_review.json` via `staged_sidecar`, auto-discovered by `anvil/lib/critics.py::aggregate` with no aggregator change.
   - **A distinct, non-blocking flag — NOT an ordinary critical flag**: when any active marker remains, the sidecar carries one specially-resolved `pending_dependency`-typed `CriticalFlag` (the additive type from `anvil/lib/convergence.py`). It is **visible** in the aggregate — surfaced in `verdict.md` as an outstanding dependency — but it **never** forces `advance: false` (`convergence.blocking_critical_flags` filters it out) and **never** deducts a dimension score. Routing it through the ordinary critical-flag path would tell the reviser to *resolve* the marker in prose — i.e. invent the still-outstanding number — the exact fabrication this convention exists to prevent. **The terminal-state gate (`AUDITED`) is enforced separately in `report-audit`.**
   - **Not a defect — no dimension penalty**: per `rubric.md`'s pending-marker note, do NOT deduct dimension score because a well-formed marker is present.
   - **Cache the result** (`outstanding_sources`, `resolved_sources`, `pass`) for `verdict.md`'s "Outstanding dependencies" note (step 9). The tool automatically reads `<thread>/BRIEF.md` frontmatter `pending_sources:` for the reporting-aid `resolved_sources`/`outstanding_sources` split; this has NO effect on gating.
   - **Suppression**: a marker on a line carrying (or directly below) a `<!-- anvil-lint-disable: pending_marker -->` directive is recorded as a non-gating note, excluded from `outstanding_sources`. Handled inside the tool; no reviewer action needed.
   - **Tooling-absent fail-open (manual fallback)**: when `uv`/the module is unavailable, emit a notice, manually scan `report.md` for the literal `[PENDING <source>]` / `[PENDING: <source>]` shape (a source-less `[PENDING]` is malformed, not a marker), treat any found marker exactly as the tool would (outstanding dependency in `verdict.md`, never fabricate a value), and record the fallback with a one-line note in `comments.md`.

4g. **Check BRIEF/refs evidence drift (deterministic, advisory-only) — issue #857**: invoke `uv run --project .anvil python -m anvil.lib.evidence_drift check <project>/<thread>/ <thread>.{N}/`. This compares the CURRENT mtime of `<project>/<thread>/BRIEF.md` and the max mtime under `<project>/<thread>/refs/**` against the snapshot `report-draft`/`report-revise` recorded in `<thread>.{N}/_progress.json.metadata.evidence_snapshot` when this version was written. A drift result (`status: "EVIDENCE-DRIFT"`) means the thread's BRIEF and/or refs changed AFTER this version was drafted/revised — the version under review may have been drafted against evidence that has since been corrected, retracted, or superseded (the #857 censusapi incident: a `BRIEF.md` claim retracted after the v1 draft, discovered independently by a v8 reviewer only because it happened to re-read the BRIEF).
   - **Purely advisory — no gating, no schema wiring**: unlike step 4f's pending-marker gate, this check is NEVER routed through `anvil/lib/convergence.py`'s `CriticalFlag` machinery. It does not touch `advance`, does not deduct any dimension score, does not affect the `AUDITED` terminal-state gate (owned by `report-audit`), and writes no sidecar. Its only effect is the `verdict.md` note in step 9.
   - **Cache the result** (`status`, `changed_paths`) for `verdict.md`'s "Evidence drift" note (step 9).
   - **Bootstrap case** (`status: "NO-SNAPSHOT"`): treated identically to a clean result — no note in `verdict.md`.
   - **Tooling-absent ordering (fail-open)**: when `uv` is not on `PATH`, emit a one-line notice (`report-review: evidence-drift check skipped — uv not on PATH`) and omit the `verdict.md` note entirely — a missed opportunity to inform, not a defect, so there is no manual-fallback obligation.
...
5. **Score each dimension** (1–9 per rubric, /44 total, customer-facing weights):
   ...
   - **Rhetorical economy (D9)**: distinct from dim 1 *Executive summary clarity* (first-page clarity) and dim 7 *Format / presentation quality* (rendered polish). Dim 9 asks "is the WHOLE report load-bearing?" — sections that restate findings without adding evidence, appendices that quote interview transcripts verbatim where excerpts would land, recommendation lists padded with low-value items, methodology sections that pre-emptively defend against questions nobody is going to ask. Customer reports balloon under "more = more rigorous" pressure; dim 9 is the explicit countervailing pressure. **Plain-language framing (issue #1042/#1044)**: per `anvil/lib/snippets/plain_language.md` §"Interaction with dim 9 (Rhetorical economy)" (and `anvil/lib/snippets/rubric.md` §"Dim 9 — teaching-oriented length is not padding"), a finding stated in ordinary language before its technical name is introduced is teaching a non-specialist recipient, not padding the report — do not cite it as a D9 anti-pattern instance.
...
6. **Identify critical flags** (review-side; see `rubric.md` for the list and definitions):
   ...
   - **Unresolved pending marker (deterministic, issue #841)**: step 4f's gate already emitted the specially-resolved `pending_dependency` flag when any active `[PENDING <source>]` marker remains. Do NOT list it here among the critical flags that force `advance: false`, and do NOT deduct dimension score for it — it is surfaced instead as an outstanding dependency (step 9) and gates only the `AUDITED` terminal transition (`report-audit`).
7. **Compute total**: sum all dimension scores. `advance = (total >= 39) AND (no critical flags)`. "critical flags" here excludes the specially-resolved `pending_dependency` flag (`convergence.blocking_critical_flags` is the programmatic filter) — a pending marker never lowers `total` and never flips `advance` to false on its own. The `AUDITED` terminal transition additionally requires the pending gate to have passed — see `commands/report-audit.md`'s terminal-gate step.
...
9. **Write `verdict.md`** in the format specified in `rubric.md`:
   ...
   - **Outstanding dependencies (conditional — issue #841)**: when step 4f's `outstanding_sources` is non-empty, add a short "Outstanding dependencies" note listing each still-pending source label and, when `<thread>/BRIEF.md` declared `pending_sources`, which declared sources are already `resolved_sources`. State explicitly that these are honest disclosures holding the `AUDITED` transition (via `report-audit`'s terminal gate), NOT scored defects and NOT critical flags. Omitted entirely when no active markers are present.
   - **Evidence drift (conditional — issue #857)**: when step 4g reported `status: "EVIDENCE-DRIFT"`, add a short "Evidence drift" note naming which of `BRIEF.md` / `refs/**` changed since this version was drafted/revised (from `changed_paths`), and state explicitly this is **advisory only** — it does NOT change `advance`, does NOT change any dimension score, and does NOT gate the `AUDITED` transition. The note exists so a human (or `report-audit`, which reads this same `verdict.md`) knows to **re-weigh the current BRIEF/refs against the version under review** — the thread's evidence may have been corrected, retracted, or superseded after this version was written. Omitted entirely when step 4g reported `CLEAN` or `NO-SNAPSHOT` (byte-identical to pre-#857 `verdict.md`).
