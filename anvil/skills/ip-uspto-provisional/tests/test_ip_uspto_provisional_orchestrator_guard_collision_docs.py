...
Every ip-uspto-provisional command doc whose critic writes a
sidecar-manifest file (`findings.md` / `verdict.md` / `scoring.md` /
`comments.md`) into a `staged_sidecar` staging dir must carry a
one-sentence pointer to the sanctioned Bash-heredoc fallback for a harness
that pattern-matches and rejects those filenames on a `Write` —
documented once in ``anvil/lib/snippets/critics.md`` §"Orchestrator
output-file guard collisions" and cross-referenced from each command
doc's "Non-Python-driver ordering" step.

PR #970 (closing #962) propagated this breadcrumb to 31 command files
repo-wide, including the six ip-uspto-provisional sidecar-writing critic
docs below — already true on `main` by the time issue #999 was filed.
This test pins that coverage so a future doc edit cannot silently drop
the cross-reference from one of them, mirroring
...
``test_additive_gate_docs.py``: no PDF render, no schema parse.
"""
...
# The ip-uspto-provisional command docs whose critic writes findings.md
# (or a sibling sidecar-manifest file) into a staged_sidecar staging dir
# and already carry the cross-reference (landed by PR #970).
