...
- **Numeral-drift verification is NOT a substitute for this critic (issue #982).** The `review` critic's Dim-4 text-source check and TikZ mode's own compile-success signal are both text/compile-level — neither observes rendered pixels, so neither catches a numeral that is textually present yet visually collided, struck through, or clipped once rendered. `ip-uspto-provisional-figures` marks this gap explicitly (`phases.figures.metadata.visual_qa = "not-run"` in `<thread>.{N}/_progress.json` whenever it renders ≥1 TikZ figure) precisely because this critic is opt-in and not default-run — running THIS critic (writing `<thread>.{N}.vision/_review.json`) is what actually resolves that gap, not a re-reading of the numeral-drift result. `ip-uspto-provisional-finalize` checks for this critic's `_review.json` at package-assembly time and warns (non-blocking) when rendered figures exist but no vision pass was ever run — see `commands/ip-uspto-provisional-finalize.md` §"Visual QA check".
