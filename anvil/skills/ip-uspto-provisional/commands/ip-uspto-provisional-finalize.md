...
4. **Audit gate**: `<thread>.{N}.audit/_summary.md` records `passed: true`. (Verified in step 1.) **This is the only gate.** There is NO inventorship-lock gate (no `inventorship.md` in this skill) and NO pre-flight gate (the provisional pre-flight is a tracked follow-up — this command mirrors `ip-uspto-provisional-revise.md`'s "no pre-flight gate in Phase 1" posture and does NOT take a hard dependency on it). If the audit gate fails, exit with a `BLOCKED` notice naming the gate and the remedial action ("run `ip-uspto-provisional-audit <thread>` first, or address its blockers via `ip-uspto-provisional-revise`").
