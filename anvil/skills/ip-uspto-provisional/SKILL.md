...
<portfolio>/
  <thread>/                       Thread root with brief and reference material
    ...
    prior-art/                    Prior art (PDFs or markdown summaries): operator-supplied, plus — when the opt-in prior_art_search knob is set — anvil:ip-search output marked `source: "anvil:ip-search/<corpus>"`
    .anvil.json                   Optional per-thread overrides (max_iterations, critic set, prior_art_search)
...
| `ip-uspto-provisional-prior-art <thread> [--prior-art-search \| --no-prior-art-search]` | prior-art critic (+ opt-in `anvil:ip-search` step 0 when `prior_art_search` is set — OFF by default) | latest `<thread>.{N}/` + `<thread>/prior-art/**`; step 0 also reads `<thread>/BRIEF.md` + `<thread>/.anvil.json` | `<thread>.{N}.priorart/`; step 0 may add `<thread>/prior-art/<slug>.md` (never overwrites) |
...
**Optional prior-art search step** (issue #958): `<thread>/.anvil.json` may carry `{ "prior_art_search": true }` — a sibling top-level key to `max_iterations` / `critics`, accepting `true`/`false` or an object with `enabled` / `corpus` / `query` / `max_references` / `min_score`. When set, `ip-uspto-provisional-prior-art` runs `anvil:ip-search` for the thread (step 0, before it opens its sidecar) so dimension 5 scores against freshly-searched art instead of whatever happened to be hand-collected — closing the same "operator must remember" gap the mechanical pre-flight gate and the audit backstop close elsewhere. It is **NOT a critic** and does not join the `critics` array: it is a supply stage that writes only into `<thread>/prior-art/`, produces no sidecar, and contributes nothing to any score. Four properties are enforced in code (`anvil/skills/ip-search/lib/prior_art_step.py`), not just documented: **off by default** (absent the knob, no corpus query / API-key read / network call — byte-identical to a pre-#958 install; a malformed `.anvil.json` fails safe to off); **never overwrites** (`force` is pinned false and refused as an argument, so operator-authored references and hand-pasted claim text survive every re-run); **provenance-marked** (machine-fetched files carry `source: "anvil:ip-search/<corpus>"` frontmatter, mechanically partitionable from operator art); and **never blocks** (a degraded / errored / unavailable search is reported and stepped over, leaving the critic's "no prior art supplied → Dim 5 `null`" path exactly as it is today). Per-invocation `--prior-art-search` / `--no-prior-art-search` flags override the file in either direction. Full contract: `commands/ip-uspto-provisional-prior-art.md` §"Step 0".
...
- **The prior-art critic does NOT do its own patent search.** It positions the disclosure against whatever is on disk in `<thread>/prior-art/`. Searching is a separate skill — `anvil:ip-search` — which the operator may run by hand, or wire in as the opt-in, off-by-default step 0 (`prior_art_search` in `<thread>/.anvil.json`). Either way the search is a drafting aid, never an attorney clearance search, and renders no positioning verdict: dimension 5 stays the critic's.
