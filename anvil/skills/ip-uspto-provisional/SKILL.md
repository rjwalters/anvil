...
| `ip-uspto-provisional-prior-art <thread> [--search]` | prior-art critic (`--search` opts into the pre-search step, issue #958) | latest `<thread>.{N}/` + `<thread>/prior-art/**` (+ `anvil:ip-search` when opted in) | `<thread>.{N}.priorart/` |
...
| `ip-uspto-provisional-figures <thread>` | figurer (deterministic; stub-default, opt-in `--mode tikz`) | latest `<thread>.{N}/` (`spec.tex`, `drawings/drawing-descriptions.md`) | into `<thread>.{N}/drawings/` (stubs + illustrator brief; `fig-*.tex`/`fig-*.pdf` in TikZ mode, `_progress.json` `metadata.visual_qa: "not-run"` when figures rendered — issue #982, numeral-drift ≠ visual verification) |
...
**Optional prior-art pre-search wiring** (issue #958, **off by default**): `ip-uspto-provisional-prior-art` can invoke `anvil:ip-search` (issue #957) on its own, immediately before its supply check, instead of the operator running it as a separate step. Two equivalent triggers: the `--search` CLI flag on a single invocation, or `{ "prior_art_search": { "auto": true } }` in `<thread>/.anvil.json` to make it standing behavior for the thread (optional `corpus` / `max` / `min_score` keys pass through to `ip-search`'s own flags). Absent both triggers, behavior is byte-identical to before #958 — no network call. `ip-search`'s own contract governs safety: it never overwrites an existing `prior-art/*.md` without `--force` (never passed here), and it degrades gracefully with no API key (writes nothing, exits 0) — so this knob only ever adds candidate art before the priorart critic's existing "no prior art supplied → Dimension 5 `null`" path runs, never disturbing it. **All of the above is code-enforced, not just documented** (issue #978): `anvil/skills/ip-search/lib/prior_art_step.py`'s `run_step` resolves the knob and drives the search, with a structural (not merely a followed-prose) guarantee that the runner is never called when the knob is off. See `commands/ip-uspto-provisional-prior-art.md` §"Opt-in pre-search".
...
**Numeral-drift verification is NOT visual verification (issue #982).** The `review` critic's Dimension-4 text-source check ("every spec `\refnum{N}` appears in a drawing/stub, and vice versa") and TikZ mode's own compile-success signal are both text/compile-level checks — neither observes the rendered pixels, so neither can catch a numeral that is textually present yet visually collided, struck through, or clipped once rendered (a real filing shipped exactly that defect shape with a clean numeral-drift pass). Because `vision` is opt-in and not default-run, `ip-uspto-provisional-figures` writes `phases.figures.metadata.visual_qa = "not-run"` into `<thread>.{N}/_progress.json` whenever TikZ mode renders at least one figure, so a downstream reader never mistakes "figures rendered" for "layout visually verified." `ip-uspto-provisional-finalize` reads this signal at package-assembly time: if rendered (non-stub) figures exist at the AUDITED version and no `<thread>.{N}.vision/_review.json` is present, finalize emits a non-blocking WARNING in the counsel package (mirroring the existing stub-drawings warning) recommending a `vision` pass or a manual visual review before filing — see `commands/ip-uspto-provisional-finalize.md` §"Visual QA check".
...
- **The prior-art critic does NOT do its own patent search — by default.** Operator supplies prior art in `<thread>/prior-art/`, or runs `anvil:ip-search` (issue #957) to search for it. An opt-in knob (`--search` flag or `<thread>/.anvil.json` `prior_art_search.auto`, issue #958) wires `ip-search` into the critic itself, immediately before its supply check — still off by default, still never a positioning verdict; dimension 5 stays the critic's alone.
