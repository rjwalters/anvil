...
`$ARGUMENTS` is interpreted in one of **three modes** (A/B/C), chosen by inspection of the non-flag tokens and the presence of a `--prs` flag — plus a dedicated **build-everything sentinel** for the bare, sole token `all`. Before classifying, **strip all recognized flag tokens** (`--builders-per-wave N`, `--dry-run`, `--prs`, `--no-daemon`, `--claim-owned N`) from the token list — flags are honoured in their respective modes.

> **`--claim-owned N` (daemon self-claim marker).** Emitted only by `loom-daemon` when it dispatches this session as a child for issue `N` — never typed by an operator. It declares that the daemon flipped `loom:issue → loom:building` on `N` immediately before spawning THIS session, so the `loom:building` label this session observes on `N` is its **own** claim, not a competing worker's. Strip it (and its single numeric value) from the token list anywhere in `$ARGUMENTS` before mode classification, and store `CLAIM_OWNED=N`. A malformed or missing value is **fail-safe, not fatal**: treat `CLAIM_OWNED` as unset and continue — a daemon-only flag must never block the sweep it was meant to unblock. The companion env var `LOOM_SWEEP_CLAIM_OWNED=<N>` may be exported alongside; either signal alone is sufficient. When `CLAIM_OWNED` is set, this session is by construction a daemon-dispatched child: do NOT attempt any daemon delegation or `mcp__loom__*` probe — run the lifecycle in-process exactly as with `--no-daemon`.
...
1. **Verify the issue is open and not already in flight.**
   ...
   - **Step 1a — daemon self-claim check (runs first).** If `CLAIM_OWNED` (from the `--claim-owned N` flag or the `LOOM_SWEEP_CLAIM_OWNED` env var) equals this issue's number, the `loom:building` label on it is THIS session's own claim, made by the daemon immediately before spawning this child. Do NOT skip it as in-flight — proceed with the lifecycle (curate if needed, then build). This check applies only to the exact claimed issue number; every other issue uses the ordinary rule below.
   - If the issue already has `loom:building` (and Step 1a did not match), skip it — another shepherd or builder is working on it. Log a warning. Does NOT contribute to this wave.
